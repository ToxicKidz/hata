    ...
    def __new__(cls, separator, assigner):
        ...
        if separator is None:
            ...
            if separator_type is str:
                ...
                raise TypeError(
                    f'`separator` can be given as `None`, `str` or as `tuple` instance, got '
            ...
            if separator_type is str:
                ...
                if processed_separator.isspace():
                    raise ValueError(
                        f'`str` separator cannot be a space character`, meanwhile it is, got '
                        f'{separator!r}.'
                    )
                ...
                for index in range(2):
                    ...
                    if len(processed_element) != 1:
                        raise ValueError(
                            f'`tuple` separator\'s elements can be only `str` with length of `1`, '
        ...
        if assigner is None:
            ...
            if assigner_type is str:
                ...
                raise TypeError(
                    f'`assigner` can be given as `None` or `str` instance, got {assigner_type.__name__}.'
                )
        ...
        if separator_type is str:
            ...
            rp = re.compile(
                f'[{escaped_separator}\s]*((?:([^\s{assigner_escaped}]+?)\s*{assigner_escaped}\s+)?(.+?))\s*(?:$|[{escaped_separator})]+)',
                re.M | re.S,
            )
            ...
            if start == end:
                ...
                rp = re.compile(
                    f'\s*(?:([^\s{assigner_escaped}]+?)\s*{assigner_escaped}\s+)?(?:(?:{escaped_separator}(.+?)(?:$|{escaped_separator}))|(?:(.+?)(?:$|[{escaped_separator}\s]+)))',
                    re.M | re.S,
                )
                ...
                rp = re.compile(
                    f'\s*(?:([^\s{assigner_escaped}]+?)\s*{assigner_escaped}\s+)?(?:(?:{separator_start_escaped}(.+?)(?:$|{separator_end_escaped}))|(?:(.+?)(?:$|[{separator_start_escaped}\s]+)))',
                    re.M | re.S,
                )
...
DEFAULT_SEPARATOR = ContentParameterParser(
    DEFAULT_PARAMETER_SEPARATOR, DEFAULT_PARAMETER_ASSIGNER
)
...
ConverterFlag.sticker_all = ConverterFlag.sticker_default.update_by_keys(
    everywhere=True
)
...
class ConverterSetting:
    ...
    def __new__(
        ...
        if type(requires_part) is not bool:
            raise TypeError(
                f'`requires_part` can be given as `bool`, got {requires_part.__class__.__name__}.'
            )
        ...
        if type(uses_flags) is not bool:
            raise TypeError(
                f'`uses_flags` can be given as `bool`, got {uses_flags.__class__.__name__}.'
            )

        if type(default_flags) is not ConverterFlag:
            raise TypeError(
                f'`default_flags` should have be given as `{ConverterFlag.__name__}`, got '
                f'{default_flags.__class__.__name__}.'
            )

        if type(all_flags) is not ConverterFlag:
            raise TypeError(
                f'`all_flags` should have be given as `{ConverterFlag.__name__}`, got '
                f'{all_flags.__class__.__name__}.'
            )

        if (alternative_type_name is not None) and (
            type(alternative_type_name) is not str
        ):
            raise TypeError(
                f'`alternative_type_name` should have be given as `None` or as `str`, got '
                ...
                if flags & CONVERTER_FLAG_EVERYWHERE:
                    if flags & CONVERTER_FLAG_PROFILE:
                        ...
                        if guild is not None:
                            try:
                                user = await command_context.client.guild_user_get(
                                    guild, id_
                                )
                            except BaseException as err:
                                if not (
            ...
            if flags & CONVERTER_FLAG_EVERYWHERE:
                try:
                    ...
                    if not (
                        isinstance(err, ConnectionError)
                        or isinstance(err, DiscordException)
                        and (err.code == ERROR_CODES.unknown_sticker)
                    ):
                        raise
                else:
                    return sticker
...
async def _message_converter_m_id(
    command_context, content_parser_parameter_detail, message_id
):
...
class ContentParserParameter:
    ...
    def __new__(cls, parameter, index):
        ...
        while True:
            ...
            if isinstance(annotation, tuple):
                ...
                if annotation_length not in (2, 3):
                    raise ValueError(
                        f'`tuple` annotation\'s length can be either `2` or `3`, got '
                ...
                if type(annotation_tuple_description) is str:
                    ...
                    raise TypeError(
                        f'`annotation` description can be `str` instance, got '
                ...
                if annotation_length == 3:
                    ...
                    if type(annotation_tuple_name) is str:
                        ...
                        raise TypeError(
                            f'`annotation` name can be `str` instance, got '
            ...
            raise TypeError(
                f'`annotation` can be either given as `None`, `type`, `str`, `tuple, or `set`, got '
        ...
        if details_length == 0:
            detail = ContentParserParameterDetail(
                CONVERTER_NONE, CONVERTER_NONE.default_type
            )
            details = None
    ...
    def set_converter_setting(self, converter_setting):
        ...
        self.detail = ContentParserParameterDetail(
            converter_setting, converter_setting.default_type
        )
    ...
    async def parse(self, command_context, part):
        ...
        if detail is None:
            for detail in self.details:
                parsed = await detail.converter_setting.converter(
                    command_context, detail, part
                )
                if parsed is not None:
                    break
            else:
...
class CommandContentParser:
    ...
    async def parse_content(self, command_context, index):
        ...
        for parameter_parsing_state in iter_no_part_parameter_states(
            parameter_parsing_states
        ):
            parsed_value = await parameter_parsing_state.content_parser_parameter.parse(
        ...
        while True:
            ...
            if get_is_all_parameter_parser_state_satisfied(parameter_parsing_states):
                parameter_parsing_state = get_rest_parameter_state(
                    parameter_parsing_states
                )
            ...
            if keyword is None:
                parameter_parsing_state = get_next_non_filled_parameter_state(
                    parameter_parsing_states
                )
                ...
                parsed_value = (
                    await parameter_parsing_state.content_parser_parameter.parse(
                        command_context, part
                    )
                )
                parameter_parsing_state.add_parsed_value(parsed_value, None)
            else:
                keyword = raw_name_to_display(keyword)
                parameter_parsing_state = get_keyword_parameter_state(
                    parameter_parsing_states, keyword
                )
                if parameter_parsing_state is None:
                    continue
...
class GenericParameterParsingState(ParameterParsingStateBase):
    ...
    def add_parsed_value(self, parsed_value, keyword):
        ...
        if parsed_value is not None:
            ...
            if (
                content_parser_parameter.is_positional
                or content_parser_parameter.is_keyword
            ):
                self.satisfied = True
...
def content_parser_parameter_postprocessor_try_find_context(command_context_parser):
    ...
    for parameter in parameters:
        detail = parameter.detail
        if detail is not None:
            if detail.converter_setting is CONVERTER_NONE and parameter.name in (
                'ctx',
                'context',
                'command_context',
            ):
...
def content_parser_parameter_postprocessor_try_find_message_and_client(
    command_context_parser,
):
    """
    ...
    converter_setting = detail.converter_setting
    if (converter_setting is not CONVERTER_NONE) and (
        converter_setting is not CONVERTER_CLIENT
    ):
        return
    ...
    converter_setting = detail.converter_setting
    if (converter_setting is not CONVERTER_NONE) and (
        converter_setting is not CONVERTER_MESSAGE
    ):
        return
