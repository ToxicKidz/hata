...
def _validate_hidden_if_checks_fail(hidden_if_checks_fail):
    ...
    if hidden_if_checks_fail is None:
        ...
        hidden_if_checks_fail = preconvert_bool(
            hidden_if_checks_fail, 'hidden_if_checks_fail'
        )
...
class Command:
    ...
    def __new__(
        ...
        aliases, route_to = _check_maybe_route(
            'aliases', aliases, route_to, _validate_aliases
        )
        category, route_to = _check_maybe_route(
            'category', category, route_to, _validate_category
        )
        checks, route_to = _check_maybe_route(
            'checks', checks, route_to, validate_checks
        )
        error_handlers, route_to = _check_maybe_route(
            'error_handlers', error_handlers, route_to, validate_error_handlers
        )
        hidden, route_to = _check_maybe_route(
            'hidden', hidden, route_to, _validate_hidden
        )
        hidden_if_checks_fail, route_to = _check_maybe_route(
            'hidden_if_checks_fail',
            hidden_if_checks_fail,
            route_to,
        ...
        if route_to:
            ...
            category_hint = [
                _generate_category_hint_from(category) for category in category
            ]
        ...
        if function is None:
            ...
            content_parser, function = CommandContentParser(
                function, separator, assigner
            )
    ...
    def copy(self):
        ...
        if command_categories is not None:
            command_categories = {
                command_category.copy() for command_category in command_categories
            }
        ...
        new.command_category_name_to_command_category = (
            command_category_name_to_command_category
        )
    ...
    def _add_command_category(self, command_category):
        ...
        command_categories = self._command_categories
        command_category_name_to_command_category = (
            self.command_category_name_to_command_category
        )
        if command_categories is None:
    ...
    def create_event(self, command, *args, **kwargs):
        ...
        **kwargs : Keyword parameters
        ...
        if isinstance(command, (Command, Router)):
            raise TypeError(
                f'`{Command.__name__}` and `{Router.__name__}` instances cannot be added as sub-commands, '
                f'got {command!r}.'
            )
