...
def iter_sub_commands(command_category, command_name_trace):
    ...
    Parameters
    ----------
    command_category : ``Command`` or ``CommandCategory``
...
def generate_command_parameter_representation(command_parameter, separator, assigner):
    ...
    if command_parameter.is_kwargs:
        return (
            f'{PARAMETER_OPTIONAL_START}...{assigner}...~1{PARAMETER_OPTIONAL_END}{separator}'
    ...
    if command_parameter.is_args:
        return (
            f'{PARAMETER_OPTIONAL_START}{parameter_name}~1{PARAMETER_OPTIONAL_END}{separator}'
...
def generate_command_parameters(command_function):
    ...
    Returns
    -------
    command_parameters : `None` or `str`
    ...
    for command_parameter in content_parser._parameters:
        parameter_representation = generate_command_parameter_representation(
            command_parameter, parameter_separator, parameter_assigner
        )
...
class SubterraneanHelpCommand:
    ...
    | command_context       | ``CommandContext``            |
    +-----------------------+-------------------------------+
    | command_or_category   | ``Command`` or ``Category``   |
    +-----------------------+-------------------------------+
    ...
    def __new__(cls, color=None, prefix=None, embed_postprocessor=None):
        ...
        embed_postprocessor : `None` or `callable`, Optional
            ...
            | command_context       | ``CommandContext``            |
            +-----------------------+-------------------------------+
            | command_context       | ``Command`` or ``Category``   |
            +-----------------------+-------------------------------+
    ...
    async def lookup_by_name(self, command_context, name):
        ...
        try:
            ...
            if (category is None) or (
                not await run_category_checks(command_context, category)
            ):
        ...
        if commands:
            ...
            await self.list_commands(
                command_context, command_names, category.display_name
            )
        else:
            await self.command_not_found(command_context, name)
    ...
    async def show_command(self, command_context, command):
        ...
        if isinstance(description, str):
            await self.show_string_description(
                command_context, command.display_name, description
            )
            return
        ...
        if callable(description):
            ...
            if (
                issubclass(embed_type, (tuple, set))
                or hasattr(embed_type, '__getitem__')
                and hasattr(embed_type, '__len__')
            ):
                await Pagination(
                    command_context.client,
                    command_context.message.channel,
    ...
    async def list_categories(self, command_context):
        ...
        embeds = [
            Embed('Categories', page).add_field(
    ...
    async def list_category(
        self, command_context, category, display_category_name=True
    ):
        """
    ...
    async def list_commands(self, command_context, command_names, category_name):
        ...
        field_name = (
            f'Use `{command_context.prefix}help <command>` for more information.'
        )
    ...
    async def list_category_indexed(self, command_context, name):
        ...
        categories = iter(
            sorted(
                command_context.client.command_processor.categories,
