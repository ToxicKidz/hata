    ...
    def __new__(
        ...
        *,
        ...
        if name_type is str:
            ...
            raise TypeError(
                f'`name` can be give as `str` instance, got {name_type.__name__}.'
            )

        if not isinstance(hidden, bool):
            raise TypeError(
                f'`hidden` can be given as `bool` instance, got {hidden.__class__.__name__}.'
            )

        if not isinstance(hidden_if_checks_fail, bool):
            raise TypeError(
                f'`hidden_if_checks_fail` can be given as `bool` instance, got '
