...
async def send_response(command_context, response):
    ...
    if isinstance(response, str):
        return await command_context.client.message_create(
            command_context.channel, response
        )

    if isinstance(response, EmbedBase) or is_only_embed(response) and response:
        return await command_context.client.message_create(
            command_context.channel, embed=response
        )
    ...
    if response:
        response = await command_context.client.message_create(
            command_context.channel, response
        )
