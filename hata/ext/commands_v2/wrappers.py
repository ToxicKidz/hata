__all__ = (
    'CommandCooldownWrapper',
    'CommandCheckWrapper',
    'CommandConverterConfigurerWrapper',
    'CommandWrapper',
)
...
class CommandConverterConfigurerWrapper(CommandWrapper):
    ...
    def __new__(cls, annotation, flags=None, **modifiers):
        ...
        if detail is None:
            raise TypeError(
                f'`annotation` can be given either as `type` or `str` instance, got '
        ...
        if (flags is not None) and not isinstance(flags, ConverterFlag):
            if isinstance(flags, int):
                ...
                raise TypeError(
                    f'`flag` can be given as `None`, `{ConverterFlag.__name__}` or as other `int` '
        ...
        if modifiers:
            ...
            for key, value in modifiers.items():
                if not isinstance(value, bool):
                    raise TypeError(
                        f'Modifier value can only be given as `bool` instance, got {key!r}={value!r}.'
                    )
