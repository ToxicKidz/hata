...
class CooldownHandler:
    ...
    def __new__(cls, for_, reset, limit=1, weight=1):
        ...
        if for_type is str:
            ...
            raise TypeError(
                f'`for_` can be given as `str` instance, got {for_type.__name__}.'
            )
        ...
        if reset_type is not float:
            try:
                ...
                raise TypeError(
                    f'The given reset is not `float`, neither other numeric convertable to it, got '
        ...
        if weight_type is int:
            ...
            raise TypeError(
                f'`weight` can be given as `int` instance, got {weight_type.__name__}.'
            ) from None
