...
assert (len(AUTO_DASH_APPLICABLES) == 0) or (
    AUTO_DASH_APPLICABLES != AUTO_DASH_APPLICABLES[0]
), (
...
class CommandWrapper:
    ...
    def __repr__(self):
        ...
        return (
            f'{self.__class__.__name__}(wrapped={self.wrapped!r}, wrapper={self.wrapper!r}, '
...
async def process_command_coroutine(client, channel, coro):
        ...
        while True:
            ...
            try:
                ...
                if (response is None) or (not isinstance(response, (str, EmbedBase))):
                    ...
                    try:
                        response_message = await client.message_create(
                            channel, response
                        )
                    except BaseException as err:
                        response_message = None
    ...
    if (
        ...
        and isinstance(response, (str, EmbedBase))
        and channel.cached_permissions_for(client).can_send_messages
    ):
...
class Command:
    ...
    def _validate_category(category):
        ...
        if category is None:
            ...
            if category_type is Category:
                ...
                raise TypeError(
                    f'`category` should be `None`, type `str` or `{Category.__name__}`, got '
    ...
    def _validate_parser_failure_handler(parser_failure_handler):
        ...
        if parser_failure_handler is not None:
            parser_failure_handler = check_parameter_count_and_convert(
                parser_failure_handler,
                5,
                name='parser_failure_handler',
                can_be_async_generator=True,
                error_message='`parser_failure_handler` expected 5 parameters (client, message, command, content, args).',
            )
            ...
            if isinstance(aliases, list):
                for alias in aliases:
                    if not isinstance(alias, str):
                        raise TypeError(
                            f'A non `str` instance alias is given: {alias!r}, got {aliases!r}.'
                        )

                    if not alias:
                        raise ValueError(
                            f'An alias cannot be empty string, got {aliases!r}.'
                        )
                ...
                raise TypeError(
                    'Aliases can be given as `str`, or `list` of `str` instances, got '
    ...
    def __new__(
        ...
        # Check for routing
        route_to = 0
        name, route_to = cls._check_maybe_route(
            'name', name, route_to, cls._validate_name
        )
        description, route_to = cls._check_maybe_route(
            'description', description, route_to, None
        )
        aliases, route_to = cls._check_maybe_route(
            'aliases', aliases, route_to, cls._validate_aliases
        )
        category, route_to = cls._check_maybe_route(
            'category', category, route_to, cls._validate_category
        )
        checks, route_to = cls._check_maybe_route(
            'checks', checks, route_to, validate_checks
        )
        parser_failure_handler, route_to = cls._check_maybe_route(
            'parser_failure_handler',
            parser_failure_handler,
            route_to,
            cls._validate_parser_failure_handler,
        )
        ...
        if route_to:
            ...
            description = route_value(
                description, route_to, default=default_description
            )
            ...
            for index in range(route_to):
                name[index], aliases[index], alters[index] = cls._generate_alters_from(
                    name[index], aliases[index]
                )

            category_hint = [
                cls._generate_category_hint_from(category) for category in category
            ]

            description = [
                cls._generate_description_from(command, description)
                if ((description is None) or (description is not default_description))
    ...
    parser_failure_handler = property(
        _get_parser_failure_handler,
        _set_parser_failure_handler,
...
def test_name_rule(rule, rule_name, nullable):
    ...
    if rule_type is not FunctionType:
        raise TypeError(
            f'`{rule_name}` should have been given as `{FunctionType.__name__}` instance, got '
    ...
    if analyzed.is_async():
        raise TypeError(
            f'`{rule_name}` should have been given as an non async function instance, got '
            f'{rule!r}.'
        )
...
class CommandProcesser(EventWaitforBase):
    ...
    def __new__(
        ...
        if default_category_name is not None:
            ...
            if default_category_name_type is str:
                ...
                raise TypeError(
                    f'`default_category_name` should have been passed as `None`, or as `str` instance, '
    ...
    def get_category(self, category_name):
        ...
        if category_name is None:
            ...
            if category_name_type is str:
                ...
                raise TypeError(
                    f'`category_name` can be given as `None` or as `instance`, got '
    ...
    def _set_default_category_name(self, default_category_name):
        ...
        if default_category_name is None:
            ...
            if (actual_default_category_name is not None) and (
                default_category_name == actual_default_category_name
            ):
                return
        ...
        if other_category is not None:
            raise ValueError(
                f'There is already a category added with name: `{default_category_name!r}`.'
            )
    ...
    default_category_name = property(
        _get_default_category_name, _set_default_category_name
    )
    ...
    def create_category(self, name, checks=None, description=None):
        ...
        if category is not None:
            raise ValueError(
                f'There is already a category added with that name: `{name!r}`'
            )
    ...
    def delete_category(self, category):
        ...
        if category_type is Category:
            ...
            if category_type is str:
                ...
                raise TypeError(
                    f'Expected type `str` or `{Category.__class__.__name__} as `category`, got '
                    f'{category_type.__name__}.'
                )
    ...
    def update_prefix(self, prefix, ignorecase=None):
        ...
        while True:
            if callable(prefix):
                ...
                if non_reserved_positional_parameter_count != 1:
                    raise TypeError(
                        f'If `prefix` is given as a `callable`, got {callable!r}, then it should accept '
                ...
                if analyzed.is_async():
                    ...
                    async def prefix_filter(message):
                        ...
                        if isinstance(practical_prefix, str):
                            ...
                            escaped_prefix = '|'.join(
                                re.escape(prefix_) for prefix_ in practical_prefix
                            )
            ...
            if isinstance(prefix, str):
                ...
                for prefix_ in prefix:
                    if not isinstance(prefix_, str):
                        raise TypeError(
                            f'`prefix` can be given as `str`, `callable` or as `list`, `tuple` or `set` '
                ...
                PREFIX_RP = re.compile(
                    '|'.join(re.escape(prefix_) for prefix_ in prefix), flag
                )
                ...
                raise TypeError(
                    f'Prefix can be only `callable`, `str` or `tuple` / `list` of `str` instances,  got '
    ...
    def create_event(
        ...
        if (name is not None) and isinstance(name, str):
            ...
            if name == 'default_event':
                func = check_parameter_count_and_convert(
                    func,
                    2,
                    name='default_event',
                    can_be_async_generator=True,
            ...
            if name == 'command_error':
                func = check_parameter_count_and_convert(
                    ...
                    name='command_error',
                    can_be_async_generator=True,
                    error_message='`command_error` expected 5 parameters (client, message, command, content, exception).',
                )
        ...
        for alter in alters:
            ...
            if would_overwrite is not None:
                error_message_parts.append(
                    ' The command already overwrites an another one with the same name: `'
                )
    ...
    def _remove_command(self, func, name=None):
        ...
        if (name is None) or (name == func.name):
            ...
            if not found_alters:
                raise ValueError(
                    f'The passed command `{func!r}` is not added with any of it\'s own names as a '
                    f'command.'
                )
        ...
        if aliases is None:
            raise ValueError(
                f'The passed name `{name!r}` is not the name, neither an alias of the command '
                f'`{func!r}`.'
            )
        ...
        try:
            ...
            raise ValueError(
                f'At the passed name `{name!r}` there is no command removed, so it cannot be '
                f'deleted either.'
            )
    ...
    def delete_event(self, func, name=None):
        ...
        if name is not None:
            ...
            if name_type is str:
                ...
                raise TypeError(
                    f'`name` can be `None` or `str` instance, got {name_type.__name__}.'
                )
        ...
        if isinstance(func, Router):
            for func_maybe in func:
                if not isinstance(func_maybe, Command):
                    raise TypeError(
                        f'`func` was given as `{Router.__name__}` instance, but it contains not only '
        ...
        if name is None:
            raise TypeError(
                f'`name` should have been passed as `str`, if `func` is not passed as '
        ...
        if not compare_converted(command.command, func):
            raise ValueError(
                f'The passed `{name!r}` (`{func!r}`) command is not the same as the already loaded one: '
                f'`{command!r}`'
            )
        ...
        if result is None:
                    ...
                    if command_error is not None:
                        ...
                        if checks is None:
                            ...
                            for check in checks:
                                ...
                                coro = command_error(
                                    client, message, command, content, err
                                )
                                await process_command_coroutine(
                                    client, message.channel, coro
                                )
            ...
            try:
                ...
                if command_error is not None:
                    ...
                    if checks is None:
                        coro = command_error(
                            client, message, command_name, content, err
                        )
                        await process_command_coroutine(client, message.channel, coro)
                        return
                    else:
    ...
    def __repr__(self):
        ...
        result = [
            ...
            repr(self.prefix),
            ', command count=',
            repr(self.command_count),
            ', mention_prefix=',
            repr(self.mention_prefix),
        ]
    ...
    def _set_default_event(self, default_event):
        default_event = check_parameter_count_and_convert(
            default_event,
            2,
            name=default_event,
            can_be_async_generator=True,
    ...
    default_event_checks = property(
        _get_default_event_checks, _set_default_event_checks, _del_default_event_checks
    )
    ...
    def _set_command_error(self, command_error):
        command_error = check_parameter_count_and_convert(
            command_error,
            5,
            name='command_error',
            can_be_async_generator=True,
            error_message='`command_error` expected 5 parameters (client, message, command, content, err).',
        )
    ...
    command_error_checks = property(
        _get_command_error_checks, _set_command_error_checks, _del_command_error_checks
    )
    ...
    def _set_invalid_command(self, invalid_command):
        invalid_command = check_parameter_count_and_convert(
            invalid_command,
            4,
            name='invalid_command',
            can_be_async_generator=True,
    ...
    invalid_command = property(
        _get_invalid_command, _set_invalid_command, _del_invalid_command
    )
    ...
    invalid_command_checks = property(
        _get_invalid_command_checks,
        _set_invalid_command_checks,
        _del_invalid_command_checks,
    )
    del (
        _get_invalid_command_checks,
        _set_invalid_command_checks,
    ...
    category_name_rule = property(
        _get_category_name_rule, _set_category_name_rule, _del_category_name_rule
    )
    ...
    command_name_rule = property(
        _get_command_name_rule, _set_command_name_rule, _del_command_name_rule
    )
