...
def setup_ext_commands(client, prefix=None, lite=False, **kwargs):
    ...
    **kwargs : Keyword parameters
    ...
    if (not lite) and (prefix is None):
        raise TypeError(
            f'`prefix` parameter is required if `lite` is given as `False`.'
        )
    ...
    if lite:
        ...
        while True:
            ...
            if type(event_message_create) is asynclist:
                for index in range(list.__len__(event_message_create)):
                    ...
                    if isinstance(event, CommandProcesser):
                        raise RuntimeError(
                            f'The client already has a `{CommandProcesser.__name__}` instance added as '
                            f'event.'
                        )
            ...
            if isinstance(event_message_create, CommandProcesser):
                raise RuntimeError(
                    f'The client already has a `{CommandProcesser.__name__}` instance added as event.'
                )
