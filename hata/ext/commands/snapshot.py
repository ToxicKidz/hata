...
def take_command_processer_snapshot(client):
    ...
    if (command_processer is None) or (
        not isinstance(command_processer, CommandProcesser)
    ):
        collected = None
    else:

        element_category_name_rule = command_processer.category_name_rule
        element_command_error = (
            command_processer.command_error,
            command_processer.command_error_checks,
        )
        element_default_event = (
            command_processer.default_event,
            command_processer.default_event_checks,
        )
        element_invalid_command = (
            command_processer.invalid_command,
            command_processer.invalid_command_checks,
        )
        element_precheck = command_processer.precheck
        element_mention_prefix = command_processer.mention_prefix
        element_prefix = (
            command_processer._ignorecase,
            command_processer.prefix,
            command_processer.prefix_filter,
        )
        ...
        collected = (
            element_category_name_rule,
            element_command_error,
            element_default_event,
            element_invalid_command,
            element_precheck,
            element_mention_prefix,
            element_prefix,
            element_default_category_name,
            element_categories,
            element_commands,
        )
...
def calculate_command_processer_snapshot_difference(client, snapshot_old, snapshot_new):
    ...
    *elements_new, element_categories_new, element_commands_new = snapshot_new
    ...
    snapshot_difference = (
        *(None if e1 == e2 else e1 for e1, e2 in zip(elements_old, elements_new)),
        element_category_difference,
        element_command_difference,
    )
...
def revert_command_processer_snapshot(client, snapshot_difference):
    ...
    (
        element_category_name_rule_difference,
        element_command_error_difference,
        element_default_event_difference,
        element_invalid_command_difference,
        element_precheck_difference,
        element_mention_prefix_difference,
        element_prefix_difference,
        element_default_category_name_difference,
        element_category_difference,
        element_command_difference,
    ) = snapshot_difference
    ...
    if element_default_category_name_difference is not None:
        command_processer.default_category_name = (
            element_default_category_name_difference
        )
    ...
    if element_category_difference is not None:
        ...
        if categories_old is not None:
            for category in categories_old:
                try:
                    command_processer.create_category(
