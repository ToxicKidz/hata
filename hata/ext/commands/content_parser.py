...
class ContentParameterSeparator:
    ...
    def __new__(cls, separator):
        ...
        if separator_type is str:
            ...
            raise TypeError(
                f'`separator` should have be given as `str` or as `tuple` instance, got '
        ...
        if separator_type is str:
            if len(processed_separator) != 1:
                raise ValueError(
                    f'`str` separator length can be only `1`, got {separator!r}.'
                )
            ...
            if len(processed_separator) != 2:
                raise ValueError(
                    f'`tuple` separator length can be only `2`, got {separator!r}.'
                )
            ...
            for index in range(2):
                ...
                if element_type is str:
                    ...
                    raise TypeError(
                        f'`tuple` separator\'s elements can be only `str` instances, meanwhile it\'s '
                        f'element under index `{index}` is type {element_type.__name__!r}.'
                    )

                if len(processed_element) != 1:
                    raise ValueError(
                        f'`tuple` separator\'s elements can be only `str` with length of `1`, meanwhile '
                        f'it\'s element under index `{index}` is not, got {element!r}.'
                    )

                if processed_element.isspace():
                    raise ValueError(
                        f'`tuple` separator\'s elements cannot be space character`, meanwhile it\'s '
        ...
        if separator_type is str:
            ...
            rp = re.compile(
                f'[^{escaped}\S]*(?:{escaped}[^{escaped}\S]*)+', re.M | re.S
            )
            ...
            if start == end:
                ...
                rp = re.compile(
                    f'(?:{escaped}(.+?){escaped}|(\S+))[^{escaped}\S]*', re.M | re.S
                )
                ...
                rp = re.compile(
                    f'(?:{start_escaped}(.+?){end_escaped}|(\S+))[^{start_escaped}\S]*',
                    re.M | re.S,
                )
...
ConverterFlag.user_default = ConverterFlag().update_by_keys(
    mention=True, name=True, id=True
)
ConverterFlag.user_all = ConverterFlag.user_default.update_by_keys(
    everywhere=True, profile=True
)
ConverterFlag.client_default = ConverterFlag().update_by_keys(
    mention=True, name=True, id=True
)
ConverterFlag.client_all = ConverterFlag.client_default.update_by_keys(everywhere=True)
ConverterFlag.role_default = ConverterFlag().update_by_keys(
    mention=True, name=True, id=True
)
ConverterFlag.role_all = ConverterFlag.role_default.update_by_keys(everywhere=True)
ConverterFlag.channel_default = ConverterFlag().update_by_keys(
    mention=True, name=True, id=True
)
ConverterFlag.channel_all = ConverterFlag.channel_default.update_by_keys(
    everywhere=True
)
ConverterFlag.emoji_default = ConverterFlag().update_by_keys(
    mention=True, name=True, id=True
)
...
ConverterFlag.message_all = ConverterFlag.message_default.update_by_keys(
    everywhere=True
)
...
class ParserContext(ParserContextBase):
    ...
    def __new__(cls, flagged_annotation):
        ...
        if type(type_) is tuple:
            raise TypeError(
                f'`flagged_annotation` cannot be given as `tuple`, when creating a `{cls.__name__}` '
...
class ChainedArgsParserContext(ParserContextBase):
    ...
    def __new__(cls, flagged_annotations):
        ...
        if (type(flagged_annotations) is not tuple) or (len(flagged_annotations) < 2):
            raise TypeError(
                f'`flagged_annotations` cannot be given as not `tuple`, or as `tuple` with 1 (or less) '
    ...
    async def __call__(self, content_parser_ctx):
        ...
        while True:
            for parser_context in parser_contexts:
                result = await parser_context.converter(
                    parser_context, content_parser_ctx
                )
...
class ConverterSetting:
    ...
    def __new__(
        ...
        if converter_type is not FunctionType:
            raise TypeError(
                f'`converter` should have been given as `{FunctionType.__name__}` instance, got '
        ...
        if not analyzed.is_async():
            raise TypeError(
                f'`converter` should have been given as an async function instance, got '
                f'{converter!r}.'
            )
        ...
        if analyzed.accepts_args():
            raise TypeError(
                f'`converter` should accept not expect args, meanwhile it does.'
            )

        if analyzed.accepts_kwargs():
            raise TypeError(
                f'`converter` should accept not expect kwargs, meanwhile it does.'
            )
        ...
        if uses_flags_type is bool:
            ...
            if uses_flags in (0, 1):
                ...
                raise TypeError(
                    f'`uses_flags` was given as `int` instance, but not as `0`, or `1`, got '
                    f'{uses_flags_type.__name__}. Next time please pass a `bool` instance.'
                )
        else:
            raise TypeError(
                f'`uses_flags` should have been given as `bool` instance, got {uses_flags_type.__name__}.'
            )

        default_flags_type = default_flags.__class__
        if default_flags_type is not ConverterFlag:
            raise TypeError(
                f'`default_flags` should have be given as `{ConverterFlag.__name__}` instance, got '
        ...
        if all_flags_type is not ConverterFlag:
            raise TypeError(
                f'`all_flags` should have be given as `{ConverterFlag.__name__}` instance, got '
        ...
        if alternative_type_name is not None:
            ...
            if alternative_type_name_type is str:
                ...
                raise TypeError(
                    f'`alternative_type_name` should have be given as `None` or as `str` instance, got '
                    f'{alternative_type_name_type.__class__}.'
                )

        if (default_type is not None) and (not isinstance(default_type, type)):
            raise TypeError(
                f'`default_type` was not given as `None`, neither as `type` instance, got '
        ...
        if alternative_types is None:
            ...
            if not hasattr(alternative_types_type, '__iter__'):
                raise TypeError(
                    f'`alternative_types` can be given as `None` or as `iterable` of `type`, got '
                    ...
                    try:
                        ...
                        try:
                            ...
                            if not (
                                ...
                                or isinstance(err, DiscordException)
                                and err.code
                                in (
    ...
    async def user_converter(parser_ctx, content_parser_ctx):
                ...
                if flags & CONVERTER_FLAG_EVERYWHERE:
                    if flags & CONVERTER_FLAG_PROFILE:
                        ...
                        if guild is not None:
                            try:
                                user = await content_parser_ctx.client.guild_user_get(
                                    guild, id_
                                )
                            except BaseException as err:
                                if not (
        ...
        if flags & CONVERTER_FLAG_NAME:
            ...
            if guild is None:
                ...
                try:
                    user = await content_parser_ctx.client.guild_user_search(
                        guild, part
                    )
                except BaseException as err:
                    if not (
            ...
            if len(part) > 6 and part[-5] == '#':
                try:
                    ...
                    for client in clients:
                        if (client.discriminator == discriminator) and (
                            client.name == name_
                        ):
                            return client
...
async def channel_converter(parser_ctx, content_parser_ctx):
    ...
    if flags & CONVERTER_FLAG_ID:
        ...
        if parsed is not None:
            ...
            if flags & CONVERTER_FLAG_EVERYWHERE:
                ...
                if guild is None:
                    if (
                        (channel_type is None) or isinstance(channel, channel_type)
                    ) and channel.id == id_:
                        return channel
    ...
    if flags & CONVERTER_FLAG_NAME:
        ...
        if guild is None:
            if (
                (channel_type is None) or isinstance(channel, channel_type)
            ) and channel.has_name_like(part):
                return channel
        else:
...
async def _message_converter_cm_id(
    parser_ctx, content_parser_ctx, channel_id, message_id
):
    channel = content_parser_ctx.message.channel
    message = MESSAGES.get(message_id, None)
    if message is not None:
        ...
        if parser_ctx.flags & CONVERTER_FLAG_EVERYWHERE:
            ...
            if (
                ...
                if (guild is None)
                else (message.channel.guild is guild)
            ):
                return message
    ...
    guild = channel.guild
    if (
        (message_channel is channel)
        if (guild is None)
        else (message_channel.guild is guild)
    ):
        client = content_parser_ctx.client
        if channel.cached_permissions_for(client).can_read_message_history:
            try:
...
async def message_converter(parser_ctx, content_parser_ctx):
    ...
    if parser_ctx.flags & CONVERTER_FLAG_ID:
        ...
        if parsed is not None:
            ...
            return await _message_converter_m_id(
                parser_ctx, content_parser_ctx, message_id
            )
...
async def invite_converter(parser_ctx, content_parser_ctx):
    ...
    try:
        ...
        if not (
            isinstance(err, ConnectionError)
            or isinstance(err, DiscordException)
            and err.code == ERROR_CODES.unknown_invite  # Invite not exists
        ):
            raise
...
def validate_default_code(default_code):
    ...
    if default_code_type is str:
        ...
        if not analyzed.is_async():
            raise TypeError(
                f'`default_code` should have been given as `str`, or as an `async-callable` `function`, '
        ...
        if analyzed.accepts_args():
            raise TypeError(
                f'`default_code` should accept not expect args, meanwhile it does.'
            )

        if analyzed.accepts_kwargs():
            raise TypeError(
                f'`default_code` should accept not expect kwargs, meanwhile it does.'
            )
        ...
        raise TypeError(
            f'`default_code` can be given as `str` instance, identifying a predefined default code '
    ...
    try:
        ...
        raise LookupError(
            f'`default_code` was given as `str` instance, but not as 1 of the predefined default codes, '
            f'got {default_code!r}'
        ) from None
...
def validate_annotation_type(annotation):
    ...
    if annotation_type is str:
        ...
        if (annotation_type is type) or issubclass(annotation_type, type):
            if annotation not in CONVERTER_SETTING_TYPE_RELATION_MAP:
                raise LookupError(
                    f'`annotation` was given as `type` instance, but there is no parser for it, got '
        ...
        raise TypeError(
            f'Expected `str` or `type` instance as `annotation`, got {annotation_type.__name__}.'
        )
    ...
    try:
        ...
        raise LookupError(
            f'`annotation` was given as `str` instance, but there is no parser for it, got '
            f'{annotation!r}.'
        ) from None
...
def validate_annotation_type_flags(annotation, flags):
    ...
    if not isinstance(annotation, type):
        raise TypeError(
            f'`annotation` should have be given as `type` instance, got {annotation.__class__.__name__}.'
        )

    if not isinstance(flags, ConverterFlag):
        raise TypeError(
            f'`flags` should have be given as `{ConverterFlag.__name__}` instance, got '
    ...
    try:
        ...
        raise LookupError(
            f'The given `annotation` has no settings linked to it, got: {annotation.__name__}.'
        ) from None
    ...
    if setting.uses_flags:
        if flags:
            ...
            if not new_flags:
                raise TypeError(
                    f'Flags was given as `{flags!r}`, meanwhile the {annotation!r} annotation\'s setting '
        ...
        if flags:
            raise TypeError(
                f'The annotation {annotation!r}\'s setting do not allows flags, meanwhile given: '
                f'{flags!r}.'
            )
    ...
    def __new__(cls, annotation, flags=None):
        ...
        if type(annotation) is Converter:
            if annotation.default_type:
                raise TypeError(
                    f'`annotation` is given as `{Converter.__name__}` instance with default set, got '
                    f'{annotation!r}.'
                )
    ...
    if isinstance(annotation, tuple):
        ...
        for sub_annotation in unnest_tuple(annotation):
            ...
            try:
                ...
                flagged_annotation.flags = ConverterFlag(
                    actual_flagged_annotation.flags | flagged_annotation.flags
                )
        ...
        if result_length < 2:
            if result_length == 0:
                raise TypeError(
                    f'`annotation` is given as a `tuple`, but it contains no real annotation, got '
                    f'{annotation!r}.'
                )
            if result_length == 1:
                result = result[0]
    ...
    def __repr__(self):
        ...
        if type(annotation) is tuple:
            for flagged_annotation in annotation:
                try:
                    setting = CONVERTER_SETTING_TYPE_RELATION_MAP[
                        flagged_annotation.annotation
                    ]
                except KeyError:
                    # ????
                    continue
...
class ContentParserParameterHinter:
    ...
    def __repr__(self):
        ...
        return (
            f'{self.__class__.__name__}(default={self.default!r}, default_type={self.default_type!r}, annotation='
...
class CommandContentParser:
    ...
    def __new__(cls, func, separator):
        ...
        if analyzer.is_async() or analyzer.is_async_generator():
            ...
            analyzer.can_instance_to_async_callable()
            or analyzer.can_instance_to_async_generator()
        ):
            real_analyzer = CallableAnalyzer(func.__call__, as_method=True)
            if (not real_analyzer.is_async()) and (
                not real_analyzer.is_async_generator()
            ):
                raise TypeError(
                    f'`func` is not `async-callable` and cannot be instanced to `async` either, got '
                    f'{func!r}.'
                )
        ...
        keyword_only_parameter_count = (
            real_analyzer.get_non_default_keyword_only_parameter_count()
        )
        ...
        if client_parameter.has_annotation and (
            client_parameter.annotation is not Client
        ):
            raise TypeError(
        ...
        if message_parameter.has_annotation and (
            message_parameter.annotation is not Message
        ):
            raise TypeError(
    ...
    async def get_args(self, client, message, content):
        ...
        content_parser_context = ContentParserContext(
            self._separator, client, message, content
        )
...
class ContentParser(CommandContentParser):
    ...
    def __new__(cls, func=None, handler=None, is_method=False, separator=None):
        ...
        if handler is not None:
            handler = check_parameter_count_and_convert(
                handler,
                6,
                name='handler',
                error_message='`ContentParser` expects to pass `6` parameters to it\'s `handler`: client, message, content_parser, '
    ...
    async def __call__(self, *args):
        ...
        if self._is_method:
            ...
            if args_count < 2 or args_count > 3:
                raise TypeError(
                    f'{self!r} expects 2-3 positional parameters to be given, got {args_count}.'
                )
