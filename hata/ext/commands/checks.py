...
def validate_checks(checks_):
    ...
    if checks_ is None:
        ...
        for index, check in enumerate(checks_):
            ...
            raise TypeError(
                f'`checks` element {index} was not given as `{_check_base.__name__}`, got '
        ...
        raise TypeError(
            f'`checks_` should have been given as `None`, `{_check_base.__name__}` instance or as '
...
def _convert_handler(handler):
    ...
    if handler is not None:
        handler = check_parameter_count_and_convert(
            ...
            4,
            name='handler',
            error_message='`handler` expects to pass 4 parameters (client, message, command, check).',
        )
    return handler
...
def _convert_permissions(permissions):
    ...
    if permission_type is Permission:
        ...
        raise TypeError(
            f'`permissions` should have been passed as a `{Permission.__name__}` object or as an '
...
class _or_op_check(_check_base):
    ...
    def __new__(cls, check_1, check_2):
        ...
        if check_type is not None:
            ...
            if issubclass(check_1_type, guild_only) or issubclass(
                check_2_type, guild_only
            ):
            ...
            # Connectible cases?
            if issubclass(check_1_type, CHECKS_OR_ONLY_TYPES) or issubclass(
                check_2_type, CHECKS_OR_ONLY_TYPES
            ):
            ...
            # Entity or?
            if issubclass(check_1_type, CHECKS_ENTITY_TYPES) and issubclass(
                check_2_type, CHECKS_ENTITY_TYPES
            ):
                entities = []
...
class _and_op_check(_check_base):
    ...
    def __new__(cls, check_1, check_2):
        ...
        if check_type is not None:
            # Connectible cases?
            if issubclass(check_1_type, CHECKS_AND_ONLY_TYPES) or issubclass(
                check_2_type, CHECKS_AND_ONLY_TYPES
            ):
...
class has_any_role(_check_base):
    ...
    def __new__(cls, roles, handler=None):
        ...
        if not hasattr(roles_type, '__iter__'):
            raise TypeError(
                f'`roles` can be given as `iterable` of (`str`, `int` or `{Role.__name__}`), got '
...
class is_any_guild(_check_base):
    ...
    def __new__(cls, guilds, handler=None):
        ...
        if not hasattr(guild_type, '__iter__'):
            raise TypeError(
                f'`guilds` can be given as `iterable` of (`str`, `int` or `{Guild.__name__}`), got '
...
class custom(_check_base):
    ...
    def __new__(cls, function, handler=None):
        ...
        non_reserved_positional_parameter_count = (
            analyzer.get_non_reserved_positional_parameter_count()
        )
...
class is_any_channel(_check_base):
    ...
    def __new__(cls, channels, handler=None):
        ...
        if not hasattr(channels_type, '__iter__'):
            raise TypeError(
                f'`channels` can be given as `iterable` of (`str`, `int` or `{ChannelBase.__name__}`), '
...
class is_in_category(_check_base):
    ...
    def __new__(cls, category, handler=None):
        ...
        """
        category_id = instance_or_id_to_snowflake(
            category, (ChannelCategory, Guild), 'category'
        )
        handler = _convert_handler(handler)
...
class is_in_any_category(_check_base):
    ...
    def __new__(cls, categories, handler=None):
        ...
        if not hasattr(categories_type, '__iter__'):
            raise TypeError(
                f'`categories` can be given as `iterable` of (`str`, `int`, `{ChannelCategory.__name__}`, '
...
CHECKS_AND_ONLY_TYPES = (guild_only,)
...
CHECK_AND_TABLE = {
    ...
    frozenset(
        (guild_only, owner_or_has_guild_permissions)
    ): owner_or_has_guild_permissions,
    frozenset((is_channel, is_channel)): is_any_channel,
    frozenset((is_channel, is_any_channel)): is_any_channel,
    frozenset((is_any_channel, is_any_channel)): is_any_channel,
