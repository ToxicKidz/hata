...
from .utils import (
    PROTECTED_NAMES,
    _iter_extension_names,
    _validate_entry_or_exit,
    ...
    for extension_name in extension_names:
        try:
            ...
            raise ExtensionError(
                f'No extension was added with name: `{extension_name}`.'
            ) from None
    ...
    **extension.py**
    ...
    def default_entry_point(self, default_entry_point):
        if not _validate_entry_or_exit(default_entry_point):
            raise TypeError(
                f'`{self.__class__.__name__}.default_entry_point` expected `None`, `str` or a `callable`, '
    ...
    def default_exit_point(self, default_exit_point):
        if not _validate_entry_or_exit(default_exit_point):
            raise TypeError(
                f'`{self.__class__.__name__}.default_exit_point` expected `None`, `str` or a `callable`, '
