...
class PrettyIgnorePush:
    ...
    def __repr__(self):
        return (
            f'<{self.__class__.__name__} back={self.back} text_length={len(self.text)}>'
        )
...
def str_message(message, index=None, **kwargs):
    ...
    result.append(
        f'- channel {channel.id} ({channel.__class__.__name__}, {channel.type})', 1
    )
...
def str_reaction_mapping(
    reactions, index=None, **kwargs
):  # ignore index, 1 message can have only 1
    result = PrettyBlock()
...
def str_message_application(
    application, index=None, **kwargs
):  # ignore index, 1/message
    result = PrettyBlock()
    ...
    if len(description) > 32:
        result.append(
            f'- description: {description[:26]}...(+{len(description)-26})', 1
        )
    else:
...
def write_guild_channel_extras(
    channel, result, write_parents, write_overwrites, kwargs
):
    guild = channel.guild
    if write_parents:
        ...
        if overwrites:
            if write_overwrites:
                ...
                for index, overwrite in enumerate(overwrites, 1):
                    result.append(
                        str_PermissionOverwrite(overwrite, index=index, **kwargs), 2
                    )
            else:
                result.append(f'- overwrites count: {len(overwrites)}', 1)


def str_channel_text(
    channel, index=None, write_parents=True, overwrites=False, **kwargs
):
    result = PrettyBlock()
    if index is None:
        start = ''
        ...
        start = f'{index}.: '
    result.append(
        f'{start}ChannelText ({"news" if channel.type else "text"} {channel.type}) : ({channel.id})'
    )
...
def str_channel_category(
    ...
    if channels:
        result.append(
            str_list(
                channels, write_parents=write_parents, overwrites=overwrites, **kwargs
            ),
            1,
        )
...
def str_channel_guild_undefined(
    channel, index=None, write_parents=True, overwrites=False, **kwargs
):
    result = PrettyBlock()
    if index is None:
...
def str_guild(guild, index=None, **kwargs):
    ...
    if features:
        result.append(
            f'- features : {", ".join(feature.value for feature in features)}', 1
        )
    ...
    if public_updates_channel is not None:
        result.append(
            f'-public updates channel : {public_updates_channel.name} {public_updates_channel.id}',
            1,
        )
    ...
    if created_at > DISCORD_EPOCH_START:
        ...
        if max_age is not None:
            if relativedelta is None:
                ...
                result.append(
                    '- time left : '
                    f'{elapsed_time(relativedelta(created_at+timedelta(0, max_age), datetime.utcnow()))}',
                    1,
                )
...
def str_list(list_, index=None, mixed=True, name=None, **kwargs):
    ...
    if mixed:
        for index, value in enumerate(list_, 0):
            result.append(
                PRETTY_PRINTERS[value.__class__.__name__](value, index=index, **kwargs),
                0,
            )
    else:
...
def str_AuditLogEntry(entry, index=None, **kwargs):
    ...
    if target is not None:
        result.append(
            f'- target: {target.name} {target.id} ({target.__class__.__name__})', 1
        )
    reason = entry.reason
        ...
        for change in changes:
            ...
            for value in (change.before, change.after):
                if value is None:
                    ...
                    if attr == 'role':
                        text = ', '.join(
                            f'{element.name} {element.id}' for element in value
                        )
                    elif attr == 'overwrites':
                        text = ', '.join(
                            f'PermissionOverwrite of {element.target.name} ({element.target.id})'
...
def str_integration(integration, index=None, **kwargs):
    ...
    result.append(
        str_integration_account(integration.account, head_line=False, **kwargs), 1
    )
    ...
    if application is not None:
        ...
        result.append(
            str_integration_application(application, head_line=False, **kwargs), 1
        )
...
def str_voice_state(state, index=None, **kwargs):
    ...
    if requested_to_speak_at is not None:
        result.append(
            f'- requested to speak at: {requested_to_speak_at:{DATETIME_FORMAT_CODE}}',
            1,
        )
...
def str_application(application, index=None, **kwargs):
    ...
    if executables is not None:
        ...
        for index, executable in enumerate(executables, 1):
            ...
            result.append(
                f'{index}.: name={executable.name!r}, os={executable.os!r}{parameters_r}{is_launcher_r}',
                2,
            )
    ...
    if third_party_skus is not None:
        ...
        for index, sku in enumerate(third_party_skus, 1):
            result.append(
                f'{index}.: distributor={sku.distributor!r}, id={sku.id!r} sku={sku.sku!r}',
                2,
            )
...
def str_team(team, index=None, **kwargs):
    ...
    if members:
        ...
        for index, member in enumerate(members, 1):
            result.append(
                f'{index}.: state={member.state.name}, permissions={member.permissions!r}',
                2,
            )
...
def str_message_interaction(message_interaction, index=None, **kwargs):
    ...
    result.append(
        f'- type: {message_interaction_type.name} ({message_interaction_type.value})'
    )
