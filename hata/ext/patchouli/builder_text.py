    ...
    for element in graved:
        if type(element) is str:
            ...
            if words and words[-1] == ' ':
                if (not add_space_before) and (
                    len(words) > 1 and words[-2][-1] in DO_NOT_ADD_SPACE_AFTER
                ):
                    del words[-1]
...
EMBED_SIZED_BUILDER_CONTEXT = BuilderContext(
    INDENT_SIZE_DEFAULT, SPACE_CHAR_UNICODE, 79, graved_to_escaped_words
)
TEXT_BUILDER_CONTEXT = BuilderContext(
    INDENT_SIZE_DEFAULT, SPACE_CHAR_DEFAULT, 120, graved_to_single_graved_words
)
TEXT_SOURCE_BUILDER_CONTEXT = BuilderContext(
    INDENT_SIZE_DEFAULT, SPACE_CHAR_DEFAULT, 120, graved_to_source_words
)
        ...
        for index in range(longest):
            ...
            for column_index in range(len(line)):
                ...
                if type(part) is str:
                    if index:
                        result_line.append(
                            builder_context.space_char * sizes[column_index]
                        )
                    else:
                        result_line.append(part)
                        result_line.append(
                            builder_context.space_char
                            * (sizes[column_index] - len(part))
                        )
                        continue
                ...
                if len(part) <= index:
                    ...
                    result_line.append(part)
                    result_line.append(
                        builder_context.space_char * (sizes[column_index] - len(part))
                    )
...
class TableConverter:
    ...
    def __new__(cls, table, indent_level, optimal_fit, builder_context):
        ...
        if sum(longest_lengths) <= fillable_chars:
            ...
            size_factors = [
                (
                    ((average_lengths[index] << 1) + longest_lengths[index])
                    * shortest_lengths[index]
                )
                ** 0.5
                for index in range(x)
            ]
            ...
            sizes = [
                ceil(size_factors[index] * factor + shortest_lengths[index])
                for index in range(x)
            ]
        ...
        generated_lines = [
            TableLine(indent_level, line, sizes, builder_context) for line in lines
        ]
...
class CodeBlockConverter:
    ...
    def _do_interactive_break(self, number_of_rows):
        ...
        while True:
            ...
            code_block_1 = self._create_remove_empty_lines(
                lines[:index], self.starter, self.ender
            )
            code_block_2 = self._create_remove_empty_lines(
                lines[index:], self.starter, self.ender
            )
            break
    ...
    def _do_line_break(self, number_of_rows):
        ...
        """
        code_block_1 = self._create_remove_empty_lines(
            self.lines[:number_of_rows], self.starter, self.ender
        )
        code_block_2 = self._create_remove_empty_lines(
            self.lines[number_of_rows:], self.starter, self.ender
        )
...
def listing_converter(listing, indent_level, optimal_fit, builder_context):
    ...
    for listing_element in listing.elements:
        yield from ListingHeadConverter(
            listing_element.head, indent_level, optimal_fit, builder_context
        )
        content = listing_element.content
        if content is None:
            continue

        yield from sub_section_converter(
            content,
            indent_level + 1,
            optimal_fit - builder_context.indent_size,
            builder_context,
        )
...
def sub_section_converter(sub_section, indent_level, optimal_fit, builder_context):
    ...
    for element in sub_section:
        ...
        yield from converter(
            element, local_indent_level, local_optimal_fit, builder_context
        )
...
class SectionConverter:
    ...
    def __new__(cls, section, builder_context):
        ...
        parts = [
            ...
            *sub_section_converter(
                section_parts, 0, builder_context.max_line_length, builder_context
            ),
        ]
