...
from .graver import (
    build_graves_on_subsection,
    GravedListing,
    GravedListingElement,
    GravedTable,
    GravedDescription,
    GravedCodeBlock,
    DocWarning,
...
def parse_sections(lines):
    ...
    while True:
        ...
        if (
            (limit - index) > 2
            and (SECTION_NAME_RP.fullmatch(line) is not None)
            and (SECTION_UNDERLINE_RP.fullmatch(lines[index + 1]) is not None)
        ):
...
def parse_section(lines):
    ...
    while True:
        ...
        for detector, builder in (
            ...
            sys.stderr.write(
                f'Could not detect anything, skipping. Line :{lines[index]}\n'
            )
            index += 1
            continue
...
def parse_docstring(text, path):
    ...
    if lines[0][0] in (' ', '\t'):
        DocWarning(
            path,
            'Docstring starts with space or tab character. Might contain `\\n` or mixed indents.',
        )
        return None
...
class DocString:
    ...
    @classmethod
    def _create_attribute_docstring_part(
        cls, attr_name, attr_separator, attr_head, attr_body
    ):
        """
    ...
    def attribute_docstring_for(self, name):
        ...
        if attribute_sections is None:
            self._attribute_sections = attribute_sections = get_attribute_docs_from(
                self.sections
            )
