...
class QualPath:
    ...
    def __new__(cls, *paths):
        ...
        *paths : `str` or ``QualPath`` instances
        ...
        for path in paths:
            ...
            raise TypeError(
                f'`path` passed neither as `str`, or `{cls.__name__} instance: {path.__class__.__name__}.'
            )
    ...
    def _do_sub(self, other):
        ...
        while True:
            if (self_parts[index] == other_to_find) and (
                self_parts[index:] == other_parts[: limit - index]
            ):
