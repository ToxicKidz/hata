...
def build_graves(text):
    ...
    while True:
        ...
        if grave_start == len(text):
            warnings.append(
                f'Not ended grave character: {get_part_around(text, grave_start)!r}'
            )
            break
        ...
        if grave_start == grave_end:
            if grave_end != 0:
                warnings.append(
                    f'Empty ungraved section: {get_part_around(text, grave_end)!r}'
                )
        else:
        ...
        if grave_end == -1:
            ...
            content.append(section)
            warnings.append(
                f'Not ended grave section: {get_part_around(text, len(section))!r}'
            )
            break
        ...
        if double_grave:
            if grave_end > len(text) - 2 or text[grave_end + 1] != '`':
                ...
                content.append(section)
                warnings.append(
                    f'Not ended double grave section: {get_part_around(text, len(section))!r}'
                )
            else:
                if grave_start == grave_end:
                    warnings.append(
                        f'Empty double grave: {get_part_around(text, grave_end)!r}'
                    )
                else:
