    ...
    def __init__(self, object_, path=None):
        ...
        for section_name, section_content in docs.sections:
            if section_name is None:
                section_parts[SECTION_TYPE_DEFAULT] = SimpleSection(
                    None, section_content, object_, path
                )
                continue
            ...
            try:
                ...
                section_parts[section_type_value] = SimpleSection(
                    section_name, section_content, object_, path
                )
                continue
...
class AttributeSection:
    ...
    def __init__(self, title, mentioned_names, object_, path=None):
        ...
        if docs is None:
            ...
            relations = {
                mentioned_name: docs.attribute_docstring_for(mentioned_name)
    ...
    def serialize(self):
        ...
        for attribute_name in attribute_names:
            ...
            yield from sub_section_serializer(
                attribute_content, object_, parent_path, create_relative_sectioned_link
            )
            yield '</div>'
            continue
        ...
        yield from sub_section_serializer(
            extra_content, object_, parent_path, create_relative_sectioned_link
        )
        return
    ...
    def __init__(self, object_, path=None):
        ...
        if docs is not None:
            for section_name, section_content in docs.sections:
                ...
                try:
                    section_type_value = SECTION_NAME_TYPE_DEFAULT_RELATIONS[
                        section_name
                    ]
                except KeyError:
                    pass
                ...
                if section_name in ('Attributes', 'Instance Attributes'):
                    mentioned_names = get_attribute_section_mentioned_names(
                        section_content
                    )
                    section_parts[SECTION_TYPE_INSTANCE_ATTRIBUTES] = AttributeSection(
                        section_name, mentioned_names, object_
                    )
                    continue
                ...
                if section_name in ('Class Attributes', 'Type Attributes'):
                    ...
                    section_parts[SECTION_TYPE_OTHER_PARAMETERS] = AttributeSection(
                        section_name, mentioned_names, object_
                    )
                    continue
                ...
                if ATTRIBUTE_SECTION_NAME_RP.fullmatch(section_name) is not None:
                    ...
                    existing.append(
                        AttributeSection(section_name, mentioned_names, object_, path)
                    )
                    continue
                ...
                existing.append(
                    SimpleSection(section_name, section_content, object_, path)
                )
                continue
        ...
        for instance_attribute_name, _ in collected_instance_attributes:
            ...
            if section_attributes_i is None:
                section_parts[SECTION_TYPE_INSTANCE_ATTRIBUTES] = AttributeSection(
                    'Attributes', {instance_attribute_name}, object_
                )
            else:
        ...
        if collected_properties:
            collected_properties = [
                item for item in sorted(collected_properties, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_PROPERTIES] = UnitSection(
                'Properties', object_, collected_properties, path
            )

        if collected_methods:
            collected_methods = [
                item for item in sorted(collected_methods, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_METHODS] = UnitSection(
                'Methods', object_, collected_methods, path
            )

        if collected_classes:
            collected_classes = [
                item for item in sorted(collected_classes, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_CLASSES] = UnitSection(
                'Classes', object_, collected_classes, path
            )
...
class ModuleSerializer:
    ...
    def __init__(self, object_, path=None):
        ...
        if collected_functions:
            collected_functions = [
                item[1] for item in sorted(collected_functions, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_FUNCTIONS] = UnitListerSection(
                'Functions', object_, collected_functions, path
            )

        if collected_classes:
            collected_classes = [
                item[1] for item in sorted(collected_classes, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_CLASSES] = UnitListerSection(
                'Classes', object_, collected_classes, path
            )

        if collected_modules:
            collected_modules = [
                item[1] for item in sorted(collected_modules, key=item_sort_key)
            ]
            section_parts[SECTION_TYPE_MODULES] = UnitListerSection(
                'Modules', object_, collected_modules, path
            )
