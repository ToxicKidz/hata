...
from .converters import (
    get_component_command_parameter_converters,
    RegexMatcher,
    check_component_converters_satisfy_string,
...
def _validate_custom_id(custom_id):
    ...
    if isinstance(custom_id, str):
        ...
        if (custom_id_length < 1) or (
            custom_id_length > COMPONENT_CUSTOM_ID_LENGTH_MAX
        ):
            raise ValueError(
...
def _validate_custom_ids(custom_id):
    ...
    if isinstance(custom_id, (str, Pattern)):
        ...
        for sub_custom_id in custom_id:
            ...
            raise TypeError(
                f'`custom_id` contains a non `st` element, got: {custom_id.__class__.__name__}.'
            )
        ...
        raise TypeError(
            f'`custom_id` can be given as `str`, (`list`, `set`) of `str`, got '
...
def split_and_check_satisfaction(custom_ids, parameter_converters):
    ...
    if regex_custom_ids is not None:
        regex_custom_ids = tuple(
            RegexMatcher(regex_custom_id) for regex_custom_id in regex_custom_ids
        )
    ...
    if regex_custom_ids is not None:
        for regex_custom_id in regex_custom_ids:
            check_component_converters_satisfy_regex(
                parameter_converters, regex_custom_id
            )
...
class ComponentCommand:
    ...
    def __new__(cls, func, custom_id):
        ...
        # Check for routing.
        route_to = 0
        custom_id, route_to = _check_maybe_route(
            'name', custom_id, route_to, _validate_custom_ids
        )

        command, parameter_converters = get_component_command_parameter_converters(
            command
        )
        ...
        if route_to:
            ...
            for custom_id in custom_id:
                string_custom_ids, regex_custom_ids = split_and_check_satisfaction(
                    custom_id, parameter_converters
                )
    ...
    async def __call__(self, client, interaction_event, regex_match):
        ...
        for parameter_converter in self._parameter_converters:
            parameter = await parameter_converter(
                client, interaction_event, regex_match
            )
        ...
        await process_command_coroutine(
            client, interaction_event, False, command_coroutine
        )
