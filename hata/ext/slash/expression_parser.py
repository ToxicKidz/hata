    ...
    if index == end:
        ...
        try:
            ...
            raise EvaluationError(
                array, index, end, 'Unknown decimal integer postfix.'
            ) from None
...
def merge_2_tokens(token_1, token_2, value):
    ...
    """
    return Token(
        token_1.array, token_1.start, token_2.end, VARIABLE_EVALUATED, None, value
    )
...
CAN_EXECUTE_BINARY_AND = frozenset((OPERATION_BINARY_AND_ID,))


CAN_EXECUTE_BINARY_XOR = frozenset((OPERATION_BINARY_XOR_ID,))


CAN_EXECUTE_BINARY_OR = frozenset((OPERATION_BINARY_OR_ID,))
...
def evaluate_2_sided_power(token_1, token_2, token_3):
    ...
    if (
        ...
        and value_1.bit_length() * value_2.bit_length() > LIMIT_INTEGER_BIT_LENGTH
    ):
        raise EvaluationError(
            token_1.array,
            token_1.start,
            ...
            for parser in parsers:
                if not isinstance(parser, ParserBase):
                    raise AssertionError(
                        f'`{ParserSequence}.__new__` creates with an iterable of not only '
...
class EvaluationError(SlashCommandError):
    ...
    def _create_pretty_repr(self):
        ...
        if middle <= EXCEPTION_MESSAGE_MAX_LINE_LENGTH:
            ...
            repr_parts.append(' ' * 3)
            repr_parts.append(
                '^' * (middle - EXCEPTION_MESSAGE_MAX_LINE_LENGTH + start + 3)
            )
        else:
...
def parse_cycle(state):
    ...
    while True:
        ...
        if not JUST_PARSE(state):
            raise EvaluationError(
                state.array, state.index, state.index + 1, 'Unexpected character.'
            )
...
def check_parentheses(state):
    ...
    for token in tokens:
        ...
        if token_id == OPERATION_PARENTHESES_END_ID:
            ...
            raise EvaluationError(
                token.array, token.start, token.end, 'Never opened parentheses.'
            )

    if parentheses_starts:
        token = parentheses_starts[-1]
        raise EvaluationError(
            token.array, token.start, state.end, 'Never closed parentheses.'
        )
...
def evaluate_two_sided_operations(tokens):
    ...
    for evaluable_tokens in CAN_EXECUTE_TWO_SIDED_ORDERED:
        ...
        while index < limit:
            ...
            token = EVALUATE_2_SIDED_OPERATION[token_id](
