...
def take_slasher_snapshot(client):
    ...
    if (slasher is None) or (not isinstance(slasher, Slasher)):
        ...
        if (collected_slash_commands is None) and (
            collected_component_commands is None
        ):
            collected = None
        else:
...
def calculate_slasher_snapshot_difference(client, snapshot_old, snapshot_new):
    ...
    if (slash_command_snapshot_old is not None) or (
        slash_command_snapshot_new is not None
    ):
    ...
    if (component_command_snapshot_old is None) or (
        component_command_snapshot_new is None
    ):
        removed_component_commands = component_command_snapshot_old
        added_component_commands = component_command_snapshot_new
    else:
        removed_component_commands = (
            component_command_snapshot_old - component_command_snapshot_new
        )
        added_component_commands = (
            component_command_snapshot_new - component_command_snapshot_old
        )
