...
from ...discord.interaction.application_command import (
    ...
    APPLICATION_COMMAND_DESCRIPTION_LENGTH_MIN,
    APPLICATION_COMMAND_DESCRIPTION_LENGTH_MAX,
    APPLICATION_COMMAND_NAME_LENGTH_MIN,
...
def _validate_1_guild(guild):
    ...
    if isinstance(guild, Guild):
        ...
        raise TypeError(
            f'`guild` was not given neither as `{Guild.__class__.__name__}`, neither as `int` '
...
def _validate_guild(guild):
    ...
    if guild is None:
        ...
        if not guild_ids:
            raise ValueError(
                f'`guild` cannot be given as empty container, got {guild!r}.'
            )
...
class SlashCommand:
    ...
    def _register_guild_and_application_command_id(
        self, guild_id, application_command_id
    ):
        """
        ...
        if registered_application_command_ids is None:
            registered_application_command_ids = (
                self._registered_application_command_ids
            ) = {}

        registered_application_command_ids[guild_id] = application_command_id

    def _unregister_guild_and_application_command_id(
        self, guild_id, application_command_id
    ):
        """
        ...
        if registered_application_command_ids is not None:
            try:
                maybe_application_command_id = registered_application_command_ids[
                    guild_id
                ]
            except KeyError:
                pass
    ...
    def _exhaust_application_command_ids(self):
        ...
        if registered_application_command_ids is not None:
            while registered_application_command_ids:
                ...
                ) = registered_application_command_ids.popitem()
    ...
    def __new__(
        ...
        description, route_to = _check_maybe_route(
            'description', description, route_to, None
        )
        show_for_invoking_user_only, route_to = _check_maybe_route(
            'show_for_invoking_user_only',
            show_for_invoking_user_only,
            route_to,
            _validate_show_for_invoking_user_only,
        )
        is_global, route_to = _check_maybe_route(
            'is_global', is_global, route_to, _validate_is_global
        )
        guild_ids, route_to = _check_maybe_route(
            'guild', guild, route_to, _validate_guild
        )
        is_default, route_to = _check_maybe_route(
            'is_default', is_default, route_to, _validate_is_default
        )
        ...
        if route_to:
            ...
            show_for_invoking_user_only = route_value(
                show_for_invoking_user_only, route_to
            )
            ...
            if (
                sub_name_length < APPLICATION_COMMAND_NAME_LENGTH_MIN
                or sub_name_length > APPLICATION_COMMAND_NAME_LENGTH_MAX
            ):
                raise ValueError(
                    f'`name` length is out of the expected range '
                    f'[{APPLICATION_COMMAND_NAME_LENGTH_MIN}:'
        ...
        if command is None:
            ...
            command, parameter_converters = get_slash_command_parameter_converters(
                command, parameter_configurers
            )
        ...
        if route_to:
            ...
            for (
                ...
                if is_global and (guild_ids is not None):
                    raise TypeError(
                        f'`is_guild` and `guild` contradict each other, got is_global={is_global!r}, '
                        f'guild={guild!r}'
                    )
    ...
    def copy(self):
        ...
        if sub_commands is not None:
            sub_commands = {
                category_name: category.copy()
                for category_name, category in sub_commands.items()
            }
        ...
        if overwrites is not None:
            overwrites = {
                guild_id: overwrite.copy() for guild_id, overwrite in overwrites.items()
            }
    ...
    def _add_slash_command(self, command):
        ...
        """
        sub_commands = self._sub_commands
        if len(sub_commands) == APPLICATION_COMMAND_OPTIONS_MAX and (
...
class SlashCommandCategory:
    ...
    def copy(self):
        ...
        sub_commands = {
            category_name: category.copy()
            for category_name, category in self._sub_commands.items()
        }
