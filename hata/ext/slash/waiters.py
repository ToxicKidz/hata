...
def get_client_from_interaction_event(interaction_event):
    ...
    try:
        ...
        raise RuntimeError(
            f'The message or interaction is bound to a 3rd party application, got: '
            f'{interaction_event!r}.'
        ) from err
...
async def get_interaction_client_and_message(event_or_message, timeout):
    ...
    if isinstance(event_or_message, Message):
        ...
        raise TypeError(
            f'`event_or_message` can be either `{Message.__name__}` or `{InteractionEvent.__name__}` '
...
async def wait_for_component_interaction(event_or_message, *, timeout=None, check=None):
    ...
    RuntimeError
        ...
        - The given message message has no bound interaction.
    """
    client, message = await get_interaction_client_and_message(
        event_or_message, timeout
    )
    return await ComponentInteractionWaiter(client, message, check, timeout)


async def iter_component_interactions(
    event_or_message, *, timeout=None, check=None, count=-1
):
    """
    ...
    component_interaction_iterator = ComponentInteractionIterator(
        client, message, check, timeout, count
    )
    try:
        while True:
