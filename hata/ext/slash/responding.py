...
async def get_request_coroutines(
    client, interaction_event, show_for_invoking_user_only, response
):
    """
    ...
    if isinstance(response, InteractionResponse):
        for request_coro in response.get_request_coroutines(
            client, interaction_event, show_for_invoking_user_only
        ):
            yield request_coro
...
async def process_command_coroutine_generator(
    client, interaction_event, show_for_invoking_user_only, coroutine_generator
):
    """
...
async def process_command_coroutine(
    client, interaction_event, show_for_invoking_user_only, coroutine
):
    """
...
class InteractionResponse:
    ...
    def __init__(
        ...
        *,
        ...
        if force_new_message is not ...:
            warnings.warn(
                f'`{self.__class__.__name__}.__init__`\'s `show_for_invoking_user_only` parameter is '
    ...
    def get_request_coroutines(
        self, client, interaction_event, show_for_invoking_user_only
    ):
        """
        ...
        if interaction_event_type is INTERACTION_TYPE_APPLICATION_COMMAND:
            ...
            if message is not ...:
                ...
                if message is None:
                    yield client.interaction_response_message_edit(
                        interaction_event, **response_parameters
                    )
                else:
                    yield client.interaction_followup_message_edit(
                        interaction_event, message, **response_parameters
                    )
                return
            ...
            response_parameters = self._get_response_parameters(
                ('allowed_mentions', 'content', 'embed', 'components')
            )
...
def SlashResponse(*args, **kwargs):
    ...
    """
    warnings.warn(
        f'`SlashResponse` is deprecated and will be removed in 2021 August. Please use'
