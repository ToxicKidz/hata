...
from .waiters import (
    ...
    get_client_from_interaction_event,
    Timeouter,
)
...
class ComponentDescriptor(ComponentDescriptorState):
    ...
    def __new__(cls, source, sub_components):
        ...
        self = ComponentDescriptorState.__new__(
            ComponentDescriptorState, source, sub_components
        )
    ...
    def get_component_proxy(self, instance):
        ...
        try:
            ...
            component_proxy = component_proxy_cache[self._identifier] = ComponentProxy(
                self, instance
            )
...
def validate_check(check):
    ...
    if min_ > 2:
        raise TypeError(
            f'`check` should accept `2` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 2:
        if max_ < 2:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`check` should accept `2` parameters, meanwhile the given callable expects '
...
def validate_invoke(invoke):
    ...
    if min_ > 2:
        raise TypeError(
            f'`invoke` should accept `2` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 2:
        if max_ < 2:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`invoke` should accept `2` parameters, meanwhile the given callable expects '
...
def validate_initial_invoke(initial_invoke):
    ...
    if min_ > 1:
        raise TypeError(
            f'`initial_invoke` should accept `1` parameters, meanwhile the given callable expects at '
...
def validate_close(close):
    ...
    if min_ > 2:
        raise TypeError(
            f'`close` should accept `2` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 2:
        if max_ < 2:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`close` should accept `2` parameters, meanwhile the given callable expects '
    ...
    def __new__(cls, class_attributes):
        ...
        if timeout is None:
            ...
            try:
                ...
                raise TypeError(
                    f'`timeout` cannot be converted to `float`, got {timeout.__class__.__name__}; '
                    f'{timeout!r}'
                ) from err
...
class MenuType(type):
    ...
    def __new__(cls, class_name, class_parents, class_attributes):
        ...
        for attribute_name, attribute_value in _iter_attributes(
            class_parents, class_attributes
        ):
            if isinstance(attribute_value, ComponentDescriptor):
                container = component_descriptors
                secondary = tracked_components
                component_hasher = ComponentSourceIdentityHasher(
                    attribute_value._source_component
                )
        ...
        for component_name, component_value in component_groups:
            ...
            try:
                ...
                for sub_component in _iter_sub_components(component_value):
                    ...
                    try:
                        sub_component_descriptor = all_component_descriptor[
                            sub_component_hasher
                        ]
                    except KeyError:
                        sub_component_descriptor = ComponentDescriptor(
                            sub_component, None
                        )
                        all_component_descriptor[
                            sub_component_hasher
                        ] = sub_component_descriptor

                    sub_component_descriptors.append(sub_component_descriptor)

                component_descriptor = ComponentDescriptor(
                    component_value, sub_component_descriptors
                )
                all_component_descriptor[component_hasher] = component_descriptor
...
class Menu(metaclass=MenuType):
    ...
    async def __new__(cls, interaction_event, *args, **kwargs):
        ...
        while True:
            ...
            if isinstance(interaction_event, tuple) and (len(interaction_event) == 2):
                ...
                if isinstance(target_client, Client) and isinstance(
                    target_channel, ChannelTextBase
                ):
                    target_message = None
                    is_interaction = False
                    break
            ...
            raise TypeError(
                f'`interaction_event` can be given as `{InteractionEvent.__name__}`, '
        ...
        if is_interaction and interaction_event.is_unanswered():
            await target_client.interaction_application_command_acknowledge(
                interaction_event
            )
        ...
        if is_interaction:
            message = await target_client.interaction_folloup_message_create(
                interaction_event, **kwargs
            )
        else:
    ...
    async def __call__(self, interaction_event):
        ...
        if (gui_state == GUI_STATE_EDITING) or (
            GUI_STATE_EDITING == GUI_STATE_CANCELLING
        ):
        ...
        try:
            await client.interaction_component_message_edit(
                interaction_event, **kwargs, components=self._components
            )
        except BaseException as err:
            self.cancel(err)
            return
        ...
        if isinstance(exception, CancelledError):
            try:
                ...
                await client.events.error(
                    client, f'{self!r}._handle_close_exception', err
                )
    ...
    def embed(self, embed):
        if (embed is not None) and (not isinstance(embed, EmbedBase)):
            raise TypeError(
                f'`embed` can be given as `None` or as `{EmbedBase.__name__}` instance, got '
                ...
                if isinstance(raw_sub_component, ComponentProxy):
                    ...
                    if raw_sub_component.type is ComponentType.row:
                        ...
                        for sub_component in _iter_sub_components(raw_sub_component):
                            sub_component_hasher = ComponentSourceIdentityHasher(
                                sub_component
                            )
                    ...
                    for raw_nested_sub_component in raw_sub_component:
                        if isinstance(raw_nested_sub_component, ComponentProxy):
                            ...
                            raise TypeError(
                                f'Nested-sub components can be either `{ComponentProxy.__name__}` or  '
