...
class SlashCommandParameterConversionError(SlashCommandError):
    ...
    def __init__(self, parameter_name, received_value, excepted_type, expected_values):
        ...
        Exception.__init__(
            self, parameter_name, received_value, excepted_type, expected_values
        )
...
async def default_slasher_exception_handler(
    client, interaction_event, command, exception
):
    """
    ...
    if isinstance(exception, SlashCommandError):
        ...
        interaction_event.type is InteractionType.application_command
    ) and interaction_event.is_unanswered():
    ...
    if forward is not None:
        try:
            await client.interaction_response_message_create(
                interaction_event, forward, show_for_invoking_user_only=True
            )
...
async def _render_slash_command_exception(client, command, exception):
    ...
    await client.events.error(
        client, f'`Slasher` while calling `{command_name}`', exception
    )


async def handle_command_exception(
    exception_handlers, client, interaction_event, command, exception
):
    """
    ...
    if exception_handlers is not None:
        for exception_handler in exception_handlers:
            try:
                handled = await exception_handler(
                    client, interaction_event, command, exception
                )
            except BaseException as err:
