...
class SetupFunction:
    ...
    def __new__(
        cls, extension_name, setup_function, required_parameters, optional_parameters
    ):
        """
...
def register_setup_function(
    extension_name, setup_function, required_parameters, optional_parameters
):
    """
    ...
    EXTENSION_SETUP_FUNCTIONS.setdefault(
        setup_function.extension_short_name, setup_function
    )
...
def get_and_validate_setup_functions(extensions, kwargs):
    ...
    if extensions is not None:
        if isinstance(extensions, str):
            ...
            if iter_ is None:
                raise TypeError(
                    f'`extensions` can be given as `str` or as `iterable` of `str`-s, got '
            ...
            for extension in iter_(extensions):
                if type(extension) is str:
                    ...
                    raise TypeError(
                        f'`extensions` contains a non-`str` element, got '
            ...
            if required_parameters is not None:
                ...
                for required_parameter in required_parameters:
                    if required_parameter not in kwargs:
                        raise RuntimeError(
                            f'`{required_parameter!r}` parameter is required by '
