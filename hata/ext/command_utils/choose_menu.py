...
class ChooseMenu(PaginationBase):
    ...
    async def __new__(
        ...
        *,
        ...
        if (prefix is not None) and (len(prefix) > 100):
            raise ValueError(
                f'Please pass a prefix, what is shorter than 100 characters, got {prefix!r}.'
            )
            ...
            if message is None:
                if received_interaction:
                    ...
                    message = await client.interaction_followup_message_create(
                        channel, embed=self._render_embed()
                    )
                else:
    ...
    def _render_embed(self):
        ...
        embed.add_footer(
            f'Page {current_page}/{page_limit}, {start} - {end} / {limit}, selected: {selected+1}'
        )
        return embed
    ...
    async def __call__(self, client, event):
        ...
        if event.emoji not in (
            self.EMOJIS if len(self.choices) > 10 else self.EMOJIS_RESTRICTED
        ):
            return
            ...
            if emoji is self.SELECT:
                ...
                try:
                    ...
                    await client.events.error(
                        client, f'{self!r}.__call__ when calling {selector!r}', err
                    )
                return
