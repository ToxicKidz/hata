...
def validate_check(check):
    ...
    if min_ > 1:
        raise TypeError(
            f'`check` should accept `1` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 1:
        if max_ < 1:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`check` should accept `1` parameters, meanwhile the given callable expects '
...
def validate_invoke(invoke):
    ...
    if min_ > 1:
        raise TypeError(
            f'`invoke` should accept `1` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 1:
        if max_ < 1:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`invoke` should accept `1` parameters, meanwhile the given callable expects '
...
def validate_initial_invoke(initial_invoke):
    ...
    if min_ > 0:
        raise TypeError(
            f'`initial_invoke` should accept `0` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 0:
        if max_ < 0:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`initial_invoke` should accept `0` parameters, meanwhile the given callable '
...
def validate_close(close):
    ...
    if min_ > 1:
        raise TypeError(
            f'`close` should accept `1` parameters, meanwhile the given callable expects at '
    ...
    if min_ != 1:
        if max_ < 1:
            if not analyzer.accepts_args():
                raise TypeError(
                    f'`close` should accept `1` parameters, meanwhile the given callable expects '
...
class UserMenuFactory:
    ...
    def __new__(cls, klass):
        ...
        if not isinstance(klass, type):
            raise TypeError(
                f'`klass` can be given as `type` instance, got {klass.__class__.__name__}.'
            )
        ...
        if (close_emoji is not None) and (not isinstance(close_emoji, Emoji)):
            raise TypeError(
                f'`close_emoji can be either `None` or `{Emoji.__name__}` instance, got '
        ...
        if emojis is not None:
            if not isinstance(emojis, (tuple, list)):
                raise TypeError(
                    f'`emojis` can be either `None`, `list` or `tuple` instance, got '
            ...
            for emoji in emojis:
                if not isinstance(emoji, Emoji):
                    raise TypeError(
                        f'`emojis` contains non `{Emoji.__name__}` element, got '
            ...
            if emojis_length == 0:
                ...
                raise ValueError(
                    f'`emojis` can contain up to `20` emojis, got {emojis_length!r}.'
                )
        ...
        if allow_third_party_emojis is None:
            ...
            allow_third_party_emojis = preconvert_bool(
                allow_third_party_emojis, 'allow_third_party_emojis'
            )
    ...
    async def __new__(cls, factory, client, channel, *args, message=None, **kwargs):
        ...
        **kwargs : Keyword parameters
        ...
        if isinstance(channel, ChannelTextBase):
            ...
            raise TypeError(
                f'`channel` can be given only as `{ChannelTextBase.__name__}`, `{Message.__name__}` '
        ...
        if close is not None:
            try:
                ...
                await client.events.error(
                    client, f'{self!r}.handle_close_exception', err
                )
