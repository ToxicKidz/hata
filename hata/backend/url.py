...
from urllib.parse import (
    SplitResult,
    parse_qsl as parse_query_string_list,
    urljoin as url_join,
    urlsplit as url_split,
...
class URL:
    ...
    def __new__(cls, value='', encoded=False):
        ...
        if isinstance(value, str):
            ...
            raise TypeError(
                f'`value` should have be given as `{cls.__name__}`, `str` or `{SplitResult.__name__}` '
    ...
    def __truediv__(self, name):
        ...
        if path == '/':
            ...
            new_path = '/'.join(parts)
        return URL(
            self._value._replace(path=new_path, query='', fragment=''), encoded=True
        )
    ...
    def query(self):
        ...
        """
        return multidict(
            parse_query_string_list(self.query_string, keep_blank_values=True)
        )
    ...
    def parent(self):
        ...
        if path == '/':
            ...
            parts = path.split('/')
            value = self._value._replace(
                path='/'.join(parts[:-1]), query='', fragment=''
            )
    ...
    def with_scheme(self, scheme):
        ...
        if not isinstance(scheme, str):
            raise TypeError(
                f'`scheme` can be given as `str` instance, got {scheme.__class__.__name__}.'
            )
    ...
    def with_user(self, user):
        ...
        if user is None:
            ...
            raise TypeError(
                f'`user` can be given as `None` or `str` instance, got  {user.__class__.__name__}.'
            )
        ...
        return URL(
            value._replace(
                netloc=self._make_netloc(user, password, value.hostname, value.port)
            ),
            encoded=True,
        )
    ...
    def with_password(self, password):
        ...
        if password is None:
            ...
            raise TypeError(
                f'`password` can be given as `None` or `str` instance, got {password.__class__.__name__}.'
            )
        ...
        return URL(
            value._replace(
                netloc=self._make_netloc(
                    value.username, password, value.hostname, value.port
                )
            ),
            encoded=True,
        )
    ...
    def with_host(self, host):
        ...
        if not isinstance(host, str):
            raise TypeError(
                f'`host` can be given as `str` instance, got {host.__class__.__name__}.'
            )
        ...
        if not host:
            raise ValueError(
                '`host` was given as empty string, but removing host is not allowed.'
            )
        ...
        return URL(
            value._replace(
                netloc=self._make_netloc(
                    value.username, value.password, host, value.port
                )
            ),
            encoded=True,
        )
    ...
    def with_port(self, port):
        ...
        if (port is not None) and (not isinstance(port, int)):
            raise TypeError(
                f'`port` can be given as `None` or `int` instance, got {port.__class__.__name__}.'
            )
        ...
        return URL(
            value._replace(
                netloc=self._make_netloc(
                    value.username, value.password, value.hostname, port
                )
            ),
            encoded=True,
        )
    ...
    def with_fragment(self, fragment):
        ...
        if fragment is None:
            ...
            raise TypeError(
                f'`fragment` can be given as `None` or `str` instance, got {fragment.__class__.__name__}.'
            )

        return URL(
            self._value._replace(fragment=quote(fragment, safe='?/:@')), encoded=True
        )
    ...
    def with_name(self, name):
        ...
        return URL(
            self._value._replace(path='/'.join(parts), query='', fragment=''),
            encoded=True,
        )
    ...
    def join(self, other):
        ...
        if not isinstance(other, URL):
            raise TypeError(
                f'`url` can be given as `{URL.__name__}` instance, got {other.__class__.__name__}.'
            )
    ...
    def human_repr(self):
        ...
        return url_unsplit(
            SplitResult(
                self.scheme,
                self._make_netloc(
                    self.user, self.password, self.host, self._value.port
                ),
                self.path,
                self.query_string,
                self.fragment,
