...
if hasattr(module_socket, 'TCP_NODELAY'):
    ...
    def _set_nodelay(socket):
        if (
            socket.family in (module_socket.AF_INET, module_socket.AF_INET6)
            and socket.type == module_socket.SOCK_STREAM
            and socket.proto == module_socket.IPPROTO_TCP
        ):
                ...
                try:
                    ...
                    if err.reason == 'PROTOCOL_IS_SHUTDOWN':
                        ...
                        if errno == ssl.SSL_ERROR_WANT_READ:
                            ...
                            errno == ssl.SSL_ERROR_WANT_WRITE
                            or errno == ssl.SSL_ERROR_SYSCALL
                        ):
                            need_ssl_data = False
                        else:
                            raise
...
class _SSLProtocolTransport:
    ...
    def write(self, data):
        ...
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError(
                f'`data` expecting a `bytes-like`, got {data.__class__.__name__}.'
            )
...
class SSLProtocol:
    ...
    def connection_made(self, transport):
        ...
        self.ssl_pipe = _SSLPipe(
            self._ssl_context, self.server_side, self.server_hostname
        )
    ...
    def connection_lost(self, exception):
        ...
        if self._session_established:
            ...
            self.loop.call_soon(
                app_protocol.__class__.connection_lost, app_protocol, exception
            )
        ...
        try:
            for _ in range(len(write_backlog)):
                ...
                if data:
                    ...
                    # to write
                    if offset:
                        ssl_data = self.ssl_pipe.do_handshake(
    ...
    def _fatal_error(self, exception, message='Fatal error on transport.'):
        ...
        """
        if not isinstance(
            exception, (BrokenPipeError, ConnectionResetError, ConnectionAbortedError)
        ):
...
class _SelectorSocketTransport:
    ...
    def write(self, data):
        ...
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError(
                f'data parameter must should be `bytes-like`, got {data.__class__.__name__}.'
            )
        if self._at_eof:
    ...
    def _maybe_resume_protocol(self):
        ...
        """
        if (not self.protocol_paused) or (
...
class _SelectorDatagramTransport:
    ...
    def send_to(self, data, maybe_address=None):
        ...
        if not isinstance(data, (bytes, bytearray, memoryview)):
            raise TypeError(
                f'data parameter must be a bytes-like object, not {type(data).__name__!r}'
            )
        ...
        if address is not None:
            if (maybe_address is not None) or (maybe_address != address):
                raise ValueError(
                    f'Invalid address: must be `None` or `{address!r}`, got {maybe_address!r}.'
                )
