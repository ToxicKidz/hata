...
class Formdata:
    ...
    def add_field(
        self, name, value, content_type=None, filename=None, transfer_encoding=None
    ):
        """
            ...
            if __debug__:
                if not isinstance(filename, str):
                    raise AssertionError(
                        f'`filename` can be given as `None` or `str` instance, got '
        ...
        if content_type is not None:
            if __debug__:
                if not isinstance(content_type, str):
                    raise AssertionError(
                        '`content_type` can be `None` or `str` instance, got '
        ...
        if transfer_encoding is not None:
            if __debug__:
                if not isinstance(transfer_encoding, str):
                    raise AssertionError(
                        '`transfer_encoding` can be `None` or `str` instance, got: '
    ...
    def _gen_form_urlencoded(self, encoding):
        ...
        return BytesPayload(
            urlencode(data, doseq=True, encoding=encoding).encode(),
    ...
    def _gen_form_data(self, encoding):
        ...
        for type_options, headers, value in self.fields:
            try:
                ...
                raise TypeError(
                    f'Can not serialize value type: {value.__class__.__name__}, headers: {headers!r}, '
                    f'value: {value!r}.'
                ) from err

            if type_options:
                part.set_content_disposition(
                    'form-data', type_options.kwargs(), quote_fields=self.quote_fields
                )
