...
try:
    ...
    SSL_CONTEXT_UNVERIFIED.options |= (
        module_ssl.OP_NO_SSLv2 | module_ssl.OP_NO_SSLv3 | module_ssl.OP_NO_COMPRESSION
    )
    ...
    def release(self):
        ...
        if protocol is not None:
            ...
            self.connector.release(
                self.key, protocol, should_close=protocol.should_close()
            )
...
class ConnectorBase:
    ...
    __slots__ = (
        '__weakref__',
        'acquired_protocols',
        'acquired_protocols_per_host',
        'alive_protocols_per_host',
        'cleanup_handle',
        'closed',
        'cookies',
        'force_close',
    ...
    def _cleanup(self):
        ...
        if alive_protocols_per_host:
            self.cleanup_handle = self.loop.call_later_weak(
                KEEP_ALIVE_TIMEOUT,
                self._cleanup,
            )
...
class HostInfoCont:
    ...
    def __new__(cls, host, address_infos):
        ...
        self.host_infos = [
            HostInfo.from_address_info(host, address_info)
    ...
    def __repr__(self):
        ...
        return (
            f'<{self.__class__.__name__} host_infos={self.host_infos!r}, index={self.index!r}, '
...
class TCPConnector(ConnectorBase):
    ...
    async def resolve(self, host, port):
        ...
        """
        try:
            infos = await self.loop.get_address_info(
                host, port, type=module_socket.SOCK_STREAM, family=self.family
            )
        except BaseException as err:
            return err
                ...
                if type(host_infos) is not HostInfoCont:
                    if isinstance(host_infos, OSError):
                        raise ConnectionError(
                            request.connection_key, host_infos
                        ) from host_infos
                    else:
                        raise host_infos
