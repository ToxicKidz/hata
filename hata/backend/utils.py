...
class RemovedDescriptor:
    ...
    def __get__(self, obj, type_):
        ...
        if name is None:
            raise RuntimeError(
                f'{self.__class__.__name__} is not initialized correctly yet.'
            )
        ...
        if obj is None:
            ...
            error_message = (
                f'{obj.__class__.__name__!r} object has no attribute {name!r}'
            )
...
class KeepType:
    ...
    def __call__(self, new_class):
        ...
        for attribute_name in dir(new_class):
            ...
            if hasattr(object, attribute_name) and (
                attribute_value is getattr(object, attribute_name)
            ):
                continue
...
class cached_property:
    ...
    def __new__(cls, fget):
        ...
        if not name_type_correct:
            raise TypeError(
                f'`fget` has no attribute or has, but non string attribute `__name__`, got '
...
def _modulize_function(old, globals_, source_module, module_name, module_path):
    ...
    new = FunctionType(
        old.__code__, globals_, old.__name__, old.__defaults__, old.__closure__
    )
    new.__module__ = module_path
    qualname = old.__qualname__
    if (
        (qualname is not None)
        and (len(qualname) > len(module_name))
        and qualname[len(module_name)] == '.'
        and qualname.startswith(module_name)
    ):
...
def _modulize_type(klass, globals_, source_module, module_name, module_path):
    ...
    if (
        (qualname is None)
        or (len(qualname) <= len(module_name))
    ...
    for name in dir(klass):
        ...
        if value_type is FunctionType:
            value = _modulize_function(
                value, globals_, source_module, module_name, module_path
            )
...
class HybridValueDictionary(dict):
    ...
    def __setitem__(self, key, value):
        ...
        dict.__setitem__(
            self,
            key,
            (value_weakreferable, value_or_reference),
        )
...
def added_json_serializer(obj):
    ...
    raise TypeError(
        f'Object of type {obj_type.__name__!r} is not JSON serializable.',
    )
...
def to_json(data):
    ...
    """
    return dump_to_json(
        data, separators=(',', ':'), ensure_ascii=True, default=added_json_serializer
    )
