...
class HTTPRequestHandler(ProtocolBase):
    ...
    async def _try_receive_request(self):
        ...
        try:
            ...
            headers = imultidict()
            body = (
                f'Http version: {version[0]}.{version[1]} is not supported.\n'.encode()
            )
...
MAYBE_REGEX_PATH_PART_RP = re.compile(
    '<(?:(string|int|float|uuid)\:)?([a-zA-Z_][a-zA-Z_0-9]*)>'
)
...
def _validate_subdomain(subdomain):
    ...
    if subdomain is not None:
        if type(subdomain) is str:
            ...
            raise TypeError(
                f'`subdomain` can be as `None` or as `str` instance, got {subdomain.__class__.__name__}.'
            )
...
def _validate_method(method):
    ...
    if method is not None:
        if type(method) is str:
            ...
            raise TypeError(
                f'`method` can be given as `None` or `str` instance, got {method.__class__.__name__}.'
            )

        method = method.upper()
        if method not in METHOD_ALL:
            raise ValueError(
                f'`method` can be given any of `{METHOD_ALL!r}`, got {method!r}.'
            )
...
def _validate_methods(methods):
    ...
    if methods is None:
        ...
        if not isinstance(methods, (list, tuple, set)):
            raise TypeError(
                f'`methods` can be either given as `None`, `list`, `tuple`, `set` instance, got '
        ...
        for index, method in enumerate(methods):
            if type(method) is str:
                ...
                raise TypeError(
                    f'`methods` element `{index}` should have be given as `None` or `str` instance, got '
            ...
            if method not in METHOD_ALL:
                raise ValueError(
                    f'`method` element `{index}` should have be given any of `{METHOD_ALL!r}`, got '
                    f'{method!r}.'
                )
...
def _validate_import_name(import_name):
    ...
    if type(import_name) is str:
        ...
        raise TypeError(
            f'`import_name` can be given as `str` instance, got {import_name.__class__.__name__}.'
        )
...
def _validate_root_path(root_path, import_name):
    ...
    if root_path is None:
        ...
        try:
            ...
            raise ImportError(
                f'Exception occurred while finding loader for {import_name!r} ({type(err)}{err})'
            ) from err
        ...
        if type(root_path) is str:
            ...
            raise TypeError(
                f'`root_path` can be given as `str` instance, got {root_path.__class__.__name__}.'
            )
...
def _validate_url_prefix(url_prefix):
    ...
    if url_prefix is None:
        ...
        if type(url_prefix) is str:
            ...
            raise TypeError(
                f'`url_prefix` can be given as `str` instance, got {url_prefix.__class__.__name__}.'
            )

        url_prefix_processed = tuple(
            maybe_typed_rule_part(rule_part) for rule_part in URL(url_prefix).path
        )
        if url_prefix_processed:
            for parameter_type, parameter_name in url_prefix_processed:
                if parameter_type == PARAMETER_TYPE_PATH:
                    raise TypeError(
                        f'Only last rule part can be `path` type, got {url_prefix!r}.'
                    )
        else:
            url_prefix_processed = None
    ...
    def add_rule(self, rule, request_methods, parameters, subdomain):
        ...
        self.rules.append(
            Rule(
                ...
                self.keyword_parameter_names,
                self.kwargs_parameter_supported,
                self.endpoint,
                request_methods,
                parameters,
                subdomain,
...
ROUTE_METHOD_NOT_ALLOWED = Rule(
    (),
    _handler_method_not_allowed,
    None,
    None,
    False,
    'method_not_allowed',
...
ROUTE_NOT_FOUND_NOT_ALLOWED = Rule(
    (), _handler_not_found, None, None, False, 'not found', None, None, None
)
...
def _analyze_handler(handler, handler_name, expected_parameter_count):
    ...
    if not analyzed.is_async():
        raise TypeError(
            f'`{handler_name}` can be given as `async-callable`, got {analyzed.__class__.__name__}; '
            f'{analyzed!r}.'
        )
    ...
    if non_reserved_positional_parameter_count != expected_parameter_count:
        raise TypeError(
            f'`{handler_name}` should accept `{expected_parameter_count}` non reserved positional '
    ...
    if non_default_keyword_only_parameter_count:
        raise TypeError(
            f'`{handler_name}` should accept `0` keyword only parameters, meanwhile it expects '
...
class AppBase:
    ...
    def __new__(
        cls, import_name, template_folder, root_path, static_folder, static_url_path
    ):
        """
        Parameters
        ----------
        ...
        if (static_url_path is None) and (static_folder is not None):
            if os.path.isabs(static_folder):
                ...
                static_url_path = '/'.join(
                    os.path.normpath(static_folder).split(os.path.sep)
                )
    ...
    def add_url_rule(self, rule, *args, provide_automatic_options=None, **options):
        ...
        **options : keyword parameters
        ...
        if view_func is None:
            raise TypeError(
                f'`view_func` can be `async-callable`, `{Rule.__name__}` or `{RuleFolder.__name__}` '
                f'instance, got `None`.'
            )
        ...
        if isinstance(view_func, (Rule, RuleFolder)):
            ...
            if not analyzed.is_async():
                raise TypeError(
                    f'`view_func` can be given as `async-callable`, got {view_func.__class__.__name__}; '
                    f'{view_func!r}.'
                )

            if analyzed.accepts_args():
                raise TypeError(
                    f'`view_func` should not accept `*args` meanwhile it does, got {view_func!r}.'
                )
            ...
            if view_func_positional_parameter_names is not None:
                view_func_positional_parameter_names = tuple(
                    view_func_positional_parameter_names
                )

            if view_func_keyword_parameter_names is not None:
                view_func_keyword_parameter_names = tuple(
                    view_func_keyword_parameter_names
                )
...
class BlueprintState:
    ...
    def __new__(cls, blueprint, options):
        ...
        if not isinstance(blueprint, Blueprint):
            raise TypeError(
                f'`blueprint` can be only as `{Blueprint.__name__}` instance, got '
        ...
        if options is None:
            ...
            if options:
                raise TypeError(
                    f'`options` contains unexpected parameters: {options!r}.'
                )
...
def _register_rules_from_state_stack(
    application, blueprint_state_stack, router, router_by_subdomain
):
    """
    Registers the
    ...
    def __new__(cls, application):
        ...
        while blueprint_state_generator_stack:
            ...
            if blueprint in blueprint_stack:
                raise RuntimeError(
                    f'Recursive blueprint hook found: {blueprint_stack!r} is followed by {blueprint!r}.'
                )
        ...
        while blueprint_state_generator_stack:
            ...
            try:
                ...
                _register_rules_from_state_stack(
                    application, blueprint_state_stack, router, router_by_subdomain
                )
