...
class HTTPStreamWriter:
    ...
    async def write(self, chunk):
        ...
        if self.chunked:
            chunk = b''.join(
                [len(chunk).__format__('x').encode('ascii'), b'\r\n', chunk, b'\r\n']
            )
        ...
        if compressor is None:
            if self.chunked:
                if chunk:
                    chunk = b''.join(
                        [
                            len(chunk).__format__('x').encode('ascii'),
                            b'\r\n',
                            chunk,
                            b'\r\n0\r\n\r\n',
                        ]
...
class ReadProtocolBase:
    ...
    async def read(self, n=-1):
        ...
        """
        try:
            return await self.set_payload_reader(
                self._read_until_eof() if n < 0 else self._read_exactly(n)
            )
    ...
    async def read_exactly(self, n):
        ...
        if n < 1:
            ...
            raise ValueError(
                f'`.read_exactly` size can not be less than zero, got {n}.'
            )
    ...
    def _read_until_CRLF(self):
        ...
        while True:
            ...
            if n < 0:
                raise PayloadError(
                    f'Header line exceeds max line length: {MAX_LINE_LENGTH!r} by {-n!r} and CRLF still '
                    f'not found.'
                )
    ...
    async def read_multipart(self, headers):
        ...
        is_first = True
        while True:
            is_more, headers, data = await self.set_payload_reader(
                self._read_multipart(boundary, is_first)
            )
            if data is not None:
                try:
                    ...
                    if transfer_encoding == 'base64':
                        ...
                        raise PayloadError(
                            f'Unknown transfer encoding: {transfer_encoding!r}'
                        )
    ...
    def _read_http_response(self):
        ...
        try:
            ...
            return RawResponseMessage(
                HttpVersion(int(major), int(minor)), int(status), reason, headers
            )
        except EOFError as err:
        ...
        while True:
            ...
            if len(chunk) == offset:
                ...
                if end > offset:
                    ...
                    if middle <= 0:
                        raise PayloadError(
                            f'Invalid header line: {chunk[offset:end]!r}'
                        )
    ...
    def get_payload_reader_task(self, message):
        ...
        if length is not None:
            if length.isdigit():
                ...
                raise PayloadError(
                    f'{CONTENT_LENGTH} must be a non negative int, got: {length!r}.'
                )
        ...
        if isinstance(message, RawRequestMessage) and (
            message.method == METHOD_CONNECT
        ):
            message.upgraded = True
            return self._read_until_eof()

        if (
            isinstance(message, RawResponseMessage)
            and message.status >= 199
            and (length is None)
        ):
            if message.chunked:
                decompressor = self._decompressor_for(message.encoding)
...
class ProtocolBase(ReadProtocolBase):
    ...
    def write_http_response(self, status, headers, version=HttpVersion11, body=None):
        ...
        result = [
            f'HTTP/{version.major}.{version.minor} {status.value} {status.phrase}\r\n'
        ]
        extend = result.extend
        for k, v in headers.items():
            extend((k, ': ', v, '\r\n'))
