...
class SyncWait:
    ...
    def set_exception(self, exception):
        ...
        if self._exception is None:
            ...
            if (
                type(exception) is CancelledError
                and type(self._exception) is not CancelledError
            ):
                self._exception = exception
...
class SyncQueue:
    ...
    def __init__(self, iterable=None, max_length=None, cancelled=False):
        ...
        self._results = (
            ...
            if iterable is None
            else deque(iterable, maxlen=max_length)
        )
        self._waiter = None
                ...
                try:
                    ...
                    if isinstance(err, StopIteration):
                        exception = RuntimeError(
                            f'{err.__class__.__name__} cannot be raised to a Future'
                        )
                        exception.__cause__ = err
                        err = exception
                        exception = None

                    future._loop.call_soon_thread_safe(
                        future.__class__.set_exception_if_pending, future, err
                    )
                else:
                    future._loop.call_soon_thread_safe(
                        future.__class__.set_result_if_pending, future, result
                    )
                    result = None
...
class _execution_ended_cb:
    ...
    def __call__(self, future):
        ...
        parent = self.parent
        future._loop.call_soon_thread_safe(
            parent.__class__._execution_ended, parent, self.executor
        )
...
class Executor:
    ...
    def __repr__(self):
        ...
        return (
            f'<{self.__class__.__name__} free={self.free_executor_count}, used={self.used_executor_count}, keep='
    ...
    def create_future(self):
        ...
        if not isinstance(local_thread, EventThread):
            raise RuntimeError(
                f'{self!r}.create_future was not called from {EventThread.__name__}, but from '
