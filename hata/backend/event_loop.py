...
class Handle:
    ...
    def _run(self):
        ...
        try:
            ...
            current_thread().render_exc_async(
                err,
                [
...
class CyclerCallable:
    ...
    def __new__(cls, func, priority):
        ...
        if priority_type is not int:
            try:
                ...
                raise TypeError(
                    f'The given `priority` is not `int`, neither other numeric convertable to it, got '
                    f'{priority_type.__name__}.'
                ) from None
    ...
    def __repr__(self):
        ...
        return (
            f'{self.__class__.__name__}(func={self.func!r}, priority={self.priority!r})'
        )
        ...
        *funcs : `callable`
        ...
        if cycle_time_type is not float:
            try:
                ...
                raise TypeError(
                    f'The given `cycle_time` is not `float`, neither other numeric convertable to it, got '
                    f'{cycle_time_type.__name__}.'
                ) from None
        ...
        if current_thread() is loop:
            ...
            handle = loop.call_soon_thread_safe_lazy(
                loop.__class__.call_later, loop, cycle_time, cls._run, self
            )
...
def _ip_address_info(host, port, family, type_, protocol):
    ...
    if protocol not in (0, module_socket.IPPROTO_TCP, module_socket.IPPROTO_UDP) or (
        host is None
    ):
        return
...
sys.set_asyncgen_hooks(
    firstiter=_async_generator_first_iteration_hook,
    finalizer=_async_generator_finalizer_hook,
)
        ...
        def _select(self, r, w, _, timeout=None):
            try:
                ...
                for iter_r, iter_w in sharded:
                    try:
                        result_r, result_w, result_x = select(
                            iter_r, iter_w, iter_w, 0.0
                        )
                    except OSError:
                        remove = []
                ...
                if (not collected_r) and (not collected_w):
                    ...
                    result_r, result_w, result_x = select(
                        [default_reader], EMPTY, EMPTY, timeout
                    )
                    collected_r.extend(result_r)
...
class EventThread(Executor, Thread, metaclass=EventThreadType):
    ...
    __slots__ = (
        ...
        '__weakref__',
        '_async_generators',
        '_async_generators_shutdown_called',
        '_self_write_socket',
        '_ready',
        '_scheduled',
        '_self_read_socket',
        ...
        if __debug__:
            if current_thread() is self:
                raise AssertionError(
                    f'`{self.__class__.__name__}.run` should not be called from itself.'
                )
    ...
    def _make_socket_transport(
        self, socket, protocol, waiter=None, *, extra=None, server=None
    ):
        """
    ...
    def _make_ssl_transport(
        ...
        *,
        ...
        ssl_protocol = SSLProtocol(
            self, protocol, ssl, waiter, server_side, server_hostname
        )
    ...
    def _start_serving(
        self, protocol_factory, socket, ssl=None, server=None, backlog=100
    ):
        """
    ...
    def _accept_connection(self, protocol_factory, socket, ssl, server, backlog):
        ...
        for _ in range(backlog):
            try:
                ...
                coro = self._accept_connection_task(
                    protocol_factory, connection_socket, extra, ssl, server
                )
                Task(coro, self)

    async def _accept_connection_task(
        self, protocol_factory, connection_socket, extra, ssl, server
    ):
        """
    ...
    async def connect_accepted_socket(self, protocol_factory, socket, *, ssl=None):
        ...
        return await self._create_connection_transport(
            socket, protocol_factory, ssl, '', True
        )
    ...
    async def create_connection(
        ...
        *,
            ...
            for family, type_, protocol, canonical_name, address in infos:
                try:
                    socket = module_socket.socket(
                        family=family, type=type_, proto=protocol
                    )
        ...
        return await self._create_connection_transport(
            socket, protocol_factory, ssl, server_hostname, False
        )

    async def _create_connection_transport(
        self, socket, protocol_factory, ssl, server_hostname, server_side
    ):
        """
        Open a streaming transport connection to a given address specified by `host` and `port`.
    ...
    def _ensure_resolved(
        self, address, *, family=0, type=module_socket.SOCK_STREAM, protocol=0, flags=0
    ):
        """
        ...
        if info is None:
            return self.get_address_info(
                host, port, family=family, type=type, protocol=protocol, flags=flags
            )
    ...
    async def socket_connect(self, socket, address):
        ...
        """
        if not hasattr(module_socket, 'AF_UNIX') or (
            socket.family != module_socket.AF_UNIX
        ):
            resolved = self._ensure_resolved(
                address, family=socket.family, protocol=socket.proto
            )
    ...
    def _socket_connect_cb(self, future, socket, address):
        ...
        try:
            err_number = socket.getsockopt(
                module_socket.SOL_SOCKET, module_socket.SO_ERROR
            )
            if err_number != 0:
        ...
        *,
        ...
        if socket is not None:
            ...
            if (
                ...
                error_message_parts = [
                    'Socket modifier keyword parameters can not be used when `socket` is given: '
                ]
            ...
            if (local_address is None) and (remote_address is None):
                ...
                if __debug__:
                    if local_address is not None:
                        if not isinstance(local_address, (str, bytes)):
                            raise TypeError(
                                '`local_address` should be given as `None` or as `str` or `bytes` '
                    ...
                    if remote_address is not None:
                        if not isinstance(remote_address, (str, bytes)):
                            raise TypeError(
                                '`remote_address` should be given as `None` or as `str` or `bytes` '
                                f'instance, if `family` is given as ``AF_UNIX`, got '
                ...
                if (
                    ...
                    and (
                        local_address[0]
                        != (0 if isinstance(local_address, bytes) else '\x00')
                    )
                ):
                    try:
                        ...
                        sys.stderr.write(
                            f'Unable to check or remove stale UNIX socket {local_address!r}: {err!s}.\n'
                        )
                ...
                if local_address is not None:
                    ...
                    for (
                        ...
                        address_infos[(it_family, it_protocol)] = (
                            it_socket_address,
                            None,
                        )
                ...
                for key, (
                    address_value_local,
                    address_value_remote,
                ) in address_infos.items():
                    ...
                    address_info.append(
                        (*key, address_value_local, address_value_remote)
                    )
            ...
            for family, protocol, local_address, remote_address in address_info:
                try:
                    socket = module_socket.socket(
                        family=family, type=module_socket.SOCK_DGRAM, proto=protocol
                    )
                    ...
                    if allow_broadcast:
                        socket.setsockopt(
                            module_socket.SOL_SOCKET, module_socket.SO_BROADCAST, 1
                        )
        ...
        waiter = Future(self)
        transport = _SelectorDatagramTransport(
            self, socket, protocol, remote_address, waiter, None
        )
    ...
    def _create_server_get_address_info(self, host, port, family, flags):
        ...
        return self._ensure_resolved(
            (host, port), family=family, type=module_socket.SOCK_STREAM, flags=flags
        )
    ...
    async def create_server(
        ...
        *,
        ...
        if (ssl is not None) and (type(ssl) is not SSLContext):
            raise TypeError(
                f'`ssl` can be given as `None` or as ``SSLContext``, got {ssl.__class__.__name__}.'
            )

        if (host is not None) or (port is not None):
            if socket is not None:
                raise ValueError(
                    '`host` and `port` parameters are mutually exclusive with `socket`.'
                )

            if type(reuse_address) is not bool:
                raise TypeError(
                    '`reuse_address` can be `None` or type `bool`, got '
            ...
            if (host is None) or (host == ''):
                ...
                for host in host:
                    ...
                    raise TypeError(
                        '`host` is passed as iterable, but it yielded at least 1 not `None`, or `str` '
                        f'instance; `{host!r}`'
                    )
            else:
                raise TypeError(
                    '`host` should be `None`, `str` instance or iterable of `None` or of `str` instances, '
                    f'got {host!r}'
                )
            ...
            futures = {
                self._create_server_get_address_info(host, port, family, flags)
                for host in hosts
            }
                ...
                while True:
                    ...
                    for future in done:
                        ...
                        for info in infos:
                            ...
                            if reuse_address:
                                socket.setsockopt(
                                    module_socket.SOL_SOCKET,
                                    module_socket.SO_REUSEADDR,
                                    True,
                                )
                            ...
                            if (
                                _HAS_IPv6
                                and (socket_family == module_socket.AF_INET6)
                                and hasattr(module_socket, 'IPPROTO_IPV6')
                            ):
                                socket.setsockopt(
                                    module_socket.IPPROTO_IPV6,
                                    module_socket.IPV6_V6ONLY,
                                    True,
                                )
                            try:
        ...
        async def subprocess_shell(
            ...
            *,
            ...
            if not isinstance(command, (bytes, str)):
                raise TypeError(
                    f'`cmd` must be `bytes` or `str` instance, got {command.__class__.__name__}.'
                )
