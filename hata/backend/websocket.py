...
class WebSocketCommonProtocol(ProtocolBase):
    ...
    async def send(self, data):
        ...
        if isinstance(data, (bytes, bytearray, memoryview)):
            ...
            raise TypeError(
                f'Data must be `bytes-like` or `str`, got: {data.__class__.__name__}.'
            )
    ...
    async def ensure_open(self):
        ...
        if state is CLOSED:
            raise ConnectionClosed(
                self.close_code, None, self.close_reason
    ...
    async def transfer_data(self):
        ...
        try:
            ...
            exception = ConnectionClosed(
                self.close_code or 1000, err, self.close_reason
            )
            ...
            exception = ConnectionClosed(
                self.close_code or 1000, None, self.close_reason
            )
    ...
    async def read_message(self):
        ...
        if frame.op_code == WS_OP_TEXT:
            ...
            raise WebSocketProtocolError(
                f'Unexpected op_code, got {frame.op_code!r}, expected {WS_OP_TEXT!r} or '
                f'{WS_OP_BINARY!r}.'
            )
        ...
        while True:
            ...
            if frame.op_code != WS_OP_CONT:
                raise WebSocketProtocolError(
                    f'Unexpected op_code, got {frame.op_code!r}, expected {WS_OP_CONT!r}.'
                )
    ...
    async def read_data_frame(self, max_size):
        ...
        while True:
            ...
            frame = await self.set_payload_reader(
                self._read_websocket_frame(self.is_client, max_size)
            )
    ...
    async def __new__(
        ...
        **websocket_kwargs : Keyword parameters
        ...
        if available_extensions is not None:
            request_headers[SEC_WEBSOCKET_EXTENSIONS] = build_extensions(
                available_extensions
            )

        if available_subprotocols is not None:
            request_headers[SEC_WEBSOCKET_PROTOCOL] = build_subprotocols(
                available_subprotocols
            )

        if extra_request_headers is not None:
            # we use especially items, so we check that
            if isinstance(extra_request_headers, imultidict) or hasattr(
                type(extra_request_headers), 'items'
            ):
                for name, value in extra_request_headers.items():
                    request_headers[name] = value
            else:
                raise TypeError(
                    '`extra_response_headers` should be `dict-like` with `.items` method, got '
        ...
        async with http_client.request(METHOD_GET, url, request_headers) as response:
            ...
            connections = []
            received_connections = response_headers.get_all(
                CONNECTION,
            )
            ...
            expected_key = b64encode(
                hashlib.sha1((sec_key + WS_KEY).encode()).digest()
            ).decode()
            ...
            if received_subprotocols is not None:
                ...
                for received_subprotocol in received_subprotocols:
                    parsed_subprotocol_values.extend(
                        parse_subprotocols(received_subprotocol)
                    )
            ...
            WebSocketCommonProtocol.__init__(
                self, loop, url.host, url.port, is_ssl=is_ssl, **websocket_kwargs
            )
...
class WSServerProtocol(WebSocketCommonProtocol):
    ...
    def __init__(self, server):
        ...
        WebSocketCommonProtocol.__init__(
            self, server.loop, host, port, is_ssl=is_ssl, **websocket_kwargs
        )
    ...
    async def __new__(
        ...
        **server_kwargs : Keyword parameters
        ...
        if extra_response_headers is None:
            ...
            raise TypeError(
                f'`extra_response_headers` should be `None` or a dict-like with \'.items\' method, got '
