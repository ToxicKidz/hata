...
class UnixWritePipeTransport:
    ...
    async def __new__(cls, loop, pipe, protocol, extra=None):
        ...
        if not (is_char or is_fifo or is_socket):
            raise ValueError(
                'Pipe transport is only for pipes, sockets and character devices, got '
                f'{pipe.__class__.__name__}; {pipe!r}.'
            )
...
class AsyncProcess:
    ...
    __slots__ = (
        ...
        '_paused',
        '_pending_calls',
        '_subprocess_stderr_protocol',
        '_subprocess_stdin_protocol',
        '_subprocess_stdout_protocol',
        'closed',
        'loop',
        'process_id',
        'process',
    ...
    def _pipe_connection_lost(self, fd, exception):
        ...
        if pending_calls is None:
            ...
            pending_calls.append(
                (self.__class__._do_pipe_connection_lost, (fd, exception))
            )
    ...
    def _try_finish(self):
        ...
        if pending_calls is None:
            ...
            pending_calls.append(
                (
                    self.__class__._do_connection_lost,
                    (
                        self,
                        None,
                    ),
