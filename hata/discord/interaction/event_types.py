...
from ..core import (
    KOKORO,
    INTERACTION_EVENT_RESPONSE_WAITERS,
...
RESPONSE_FLAG_IS_DEFERRING_OR_DEFERRED = (
    RESPONSE_FLAG_DEFERRING | RESPONSE_FLAG_DEFERRED
)
RESPONSE_FLAG_IS_RESPONDING_OR_RESPONDED = (
    RESPONSE_FLAG_RESPONDING | RESPONSE_FLAG_RESPONDED
)
RESPONSE_FLAG_IS_ACKNOWLEDGING_OR_ACKNOWLEDGED = (
    RESPONSE_FLAG_IS_ACKNOWLEDGING | RESPONSE_FLAG_IS_ACKNOWLEDGED
)
...
class ApplicationCommandInteraction(DiscordEntity):
    ...
    def __new__(cls, data, guild, cached_users):
                ...
                if resolved_user_datas:
                    ...
                    for user_id, user_data in resolved_user_datas.items():
                        if resolved_guild_profile_datas is None:
                            ...
                            guild_profile_data = resolved_guild_profile_datas.get(
                                user_id, None
                            )
                        ...
                        if (
                            (guild_profile_data is not None)
                            and (cached_users is not None)
                            and (user not in cached_users)
                        ):
        ...
        if (option_datas is None) or (not option_datas):
            ...
            options = [
                ApplicationCommandInteractionOption(option_data)
...
class InteractionEvent(DiscordEntity, EventBase, immortal=True):
    ...
    def __new__(cls, data):
        ...
        if interaction_type is None:
            ...
            interaction, cached_users = interaction_type(
                data['data'], guild, cached_users
            )
