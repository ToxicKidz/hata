    ...
    if components is None:
        ...
        if len(components) > COMPONENT_SUB_COMPONENT_LIMIT:
            raise AssertionError(
                f'A `component.components` can have maximum 5 sub-components, got '
        ...
        for component in components:
            if not isinstance(component, ComponentBase):
                raise AssertionError(
                    f'`component` can be given as `{ComponentBase.__name__}` instance, got '
        ...
        raise AssertionError(
            f'`components` can be given as `None`, `tuple` or `list`, got '
...
def _debug_component_custom_id(custom_id):
    ...
    if custom_id is None:
        ...
        if custom_id_length > COMPONENT_CUSTOM_ID_LENGTH_MAX:
            raise AssertionError(
                f'`custom_id`\'s max length can be {COMPONENT_CUSTOM_ID_LENGTH_MAX!r}, got '
                f'{len(custom_id)!r}; {custom_id!r}.'
            )
    else:
        raise AssertionError(
            f'`custom_id` can be given either as `None` or as `str` instance, got '
...
def _debug_component_label(label):
    ...
    if label is None:
        ...
        raise AssertionError(
            f'`label` can be given either as `None` or as `str` instance, got '
...
def _debug_component_enabled(enabled):
    ...
    if not isinstance(enabled, bool):
        raise AssertionError(
            f'`enabled` can be given as `bool` instance, got {enabled.__class__.__name__}.'
        )
...
def _debug_component_url(url):
    ...
    if url is None:
        ...
        raise AssertionError(
            f'`url` can be given either as `None` or as `str` instance, got '
...
def _debug_component_value(value):
    ...
    if not isinstance(value, str):
        raise AssertionError(
            f'`value` can be given either as  `str` instance, got '
...
def _debug_component_description(description):
    ...
    if description is None:
        ...
        raise AssertionError(
            f'`description` can be given either as `None` or as `str` instance, got '
    ...
    if isinstance(options, (tuple, list)):
        for option in options:
            if not isinstance(option, ComponentSelectOption):
                raise AssertionError(
                    f'`option` can be given as `{ComponentSelectOption.__name__}` instance, got '
        ...
        raise AssertionError(
            f'`options` can be given as `None`, `tuple` or `list`, got '
            f'{options.__class__.__name__}.'
        )

    if (option_length < COMPONENT_OPTION_LENGTH_MIN) or (
        option_length > COMPONENT_OPTION_LENGTH_MAX
    ):
        raise AssertionError(
            f'`options` can be in range '
...
def _debug_component_placeholder(placeholder):
    ...
    if placeholder is None:
        ...
        raise AssertionError(
            f'`placeholder` can be given as `None or `str` instance, got '
...
def _debug_component_min_values(min_values):
    ...
    if not isinstance(min_values, int):
        raise AssertionError(
            f'`min_values` can be given as `int` instance, got {min_values.__class__.__name__}.'
        )

    if (min_values < COMPONENT_OPTION_MIN_VALUES_MIN) or (
        min_values > COMPONENT_OPTION_MIN_VALUES_MAX
    ):
        raise AssertionError(
...
def _debug_component_max_values(max_values):
    ...
    if not isinstance(max_values, int):
        raise AssertionError(
            f'`max_values` can be given as `int` instance, got {max_values.__class__.__name__}.'
        )

    if (max_values < COMPONENT_OPTION_MAX_VALUES_MIN) or (
        max_values > COMPONENT_OPTION_MAX_VALUES_MAX
    ):
        raise AssertionError(
...
class ComponentSelect(ComponentBase):
    ...
    def from_data(cls, data):
        ...
        self.options = [
            ComponentSelectOption.from_data(option_data) for option_data in option_datas
        ]
...
COMPONENT_DYNAMIC_SERIALIZERS = {
    ...
    'style': lambda style: style.value
...
def create_component(component_data):
    ...
    component : ``ComponentBase``
        the created component instance.
    """
    return COMPONENT_TYPE_VALUE_TO_TYPE.get(
        component_data['type'], ComponentDynamic
    ).from_data(component_data)
