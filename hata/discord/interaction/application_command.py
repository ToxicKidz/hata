...
APPLICATION_COMMAND_PERMISSION_OVERWRITE_TYPE_USER = (
    ApplicationCommandPermissionOverwriteType.user
)
APPLICATION_COMMAND_PERMISSION_OVERWRITE_TYPE_ROLE = (
    ApplicationCommandPermissionOverwriteType.role
)
...
class ApplicationCommand(DiscordEntity, immortal=True):
    ...
    def __new__(cls, name, description, *, allow_by_default=True, options=None):
        ...
        if __debug__:
            ...
            if (
                name_length < APPLICATION_COMMAND_NAME_LENGTH_MIN
                or name_length > APPLICATION_COMMAND_NAME_LENGTH_MAX
            ):
                raise AssertionError(
            ...
            if not isinstance(allow_by_default, bool):
                raise AssertionError(
                    f'`allow_by_default` can be given as `bool` instance, got '
        ...
        if options is None:
            ...
            if __debug__:
                if not isinstance(options, (tuple, list)):
                    raise AssertionError(
                        f'`options` can be given as `None` or (`list` or `tuple`) of '
                ...
                if __debug__:
                    ...
                    for index, option in enumerate(options_processed):
                        if not isinstance(option, ApplicationCommandOption):
                            raise AssertionError(
                                f'`options` was given either as `list` or `tuple`, but it\'s element '
    ...
    def add_option(self, option):
        ...
        if __debug__:
            if not isinstance(option, ApplicationCommandOption):
                raise AssertionError(
                    f'`option` can be given as {ApplicationCommandOption.__name__} instance, got '
                    f'{option.__class__.__name__}.'
                )
    ...
    def _update_no_return(self, data):
        ...
        try:
            ...
            if (option_datas is None) or (not option_datas):
                ...
                options = [
                    ApplicationCommandOption.from_data(option_data)
...
class ApplicationCommandOption:
    ...
    def __new__(
        ...
        *,
        ...
        type_ = preconvert_preinstanced_type(
            type_, 'type_', ApplicationCommandOptionType
        )

        if __debug__:
            if not isinstance(default, bool):
                raise AssertionError(
                    f'`default` can be given as `bool` instance, got {default.__class__.__name__}.'
                )

            if not isinstance(required, bool):
                raise AssertionError(
                    f'`required` can be given as `bool` instance, got {required.__class__.__name__}.'
                )
        ...
        if choices is None:
            ...
            if __debug__:
                if not isinstance(choices, (tuple, list)):
                    raise AssertionError(
                        f'`choices` can be given as `None` or (`list` or `tuple`) of '
            ...
            if __debug__:
                ...
                for index, choice in enumerate(choices_processed):
                    if not isinstance(choice, ApplicationCommandOptionChoice):
                        raise AssertionError(
                            f'`choices` was given either as `list` or `tuple`, but it\'s element '
            ...
            if __debug__:
                ...
                for index, option in enumerate(options_processed):
                    ...
                    if option.type is ApplicationCommandOptionType.sub_command_group:
                        raise AssertionError(
                            f'`options` element {index}\'s type is cub-command group option, but'
        ...
        if choices_processed is not None:
            if type_ is ApplicationCommandOptionType.string:
                ...
                raise TypeError(
                    f'`choices` is bound to string and integer option type, got choices={choices!r}, '
                    f'type={type_!r}.'
                )
            ...
            for index, choice in enumerate(choices):
                if not isinstance(choice.value, expected_choice_type):
                    raise TypeError(
                        f'`choices` element\'s {index!r} value\'s type is not '
    ...
    def add_choice(self, choice):
        ...
        if isinstance(choice, ApplicationCommandOptionChoice):
            ...
            if len(choice) != 2:
                raise TypeError(
                    f'If `choice` is given as `tuple` it\'s length should be `2` representing a '
            ...
            raise TypeError(
                f'`choice` can be given as {ApplicationCommandOptionChoice.__name__} instance or a `tuple` '
        ...
        if type_ is ApplicationCommandOptionType.string:
            ...
            raise TypeError(
                f'`choice` is bound to string and integer choice type, got choice={choice!r}, '
                f'self={self!r}.'
            )

        if not isinstance(choice.value, expected_choice_type):
            raise TypeError(
                f'`choice` value\'s type is not `{expected_choice_type.__name__}` as expected from the '
    ...
    def from_data(cls, data):
        ...
        if (choice_datas is None) or (not choice_datas):
            ...
            choices = [
                ApplicationCommandOptionChoice.from_data(choice_data)
                for choice_data in choice_datas
            ]
        self.choices = choices
...
class ApplicationCommandOptionChoice:
    ...
    def __new__(cls, name, value):
        ...
        if __debug__:
            ...
            name_length = len(name)
            if (
                name_length < APPLICATION_COMMAND_CHOICE_NAME_LENGTH_MIN
                or name_length > APPLICATION_COMMAND_CHOICE_NAME_LENGTH_MAX
            ):
                raise AssertionError(
                    f'`name` length can be in range '
            ...
            if isinstance(value, int):
                ...
                if (
                    value_length < APPLICATION_COMMAND_CHOICE_VALUE_LENGTH_MIN
                    or value_length > APPLICATION_COMMAND_CHOICE_VALUE_LENGTH_MAX
                ):
                    raise AssertionError(
                ...
                raise AssertionError(
                    f'`value` type can be either `str` or `int`, got {value.__class__.__name__}.'
                )
...
class ApplicationCommandPermission:
        ...
        if overwrites is None:
            ...
            if __debug__:
                if not isinstance(overwrites, (list, set, tuple)):
                    raise AssertionError(
                        f'`overwrites` can be given either as `None` or as `list`, `set`, `tuple`'
    ...
    def from_data(cls, data):
        ...
        if overwrite_datas:
            overwrites = [
                ApplicationCommandPermissionOverwrite.from_data(overwrite_data)
                for overwrite_data in overwrite_datas
            ]
    ...
    def __repr__(self):
        ...
        repr_parts = [
            ...
            repr(self.application_command_id),
            ' guild_id=',
            repr(self.guild_id),
            ', overwrite count=',
        ]
        ...
        repr_parts.append(overwrites_count)
        repr_parts.append('>')
    ...
    def add_overwrite(self, overwrite):
        ...
        if __debug__:
            if not isinstance(overwrite, ApplicationCommandPermissionOverwrite):
                raise AssertionError(
                    f'`overwrite` can be given as {ApplicationCommandPermissionOverwrite.__name__} '
...
class ApplicationCommandPermissionOverwrite:
    ...
    def __new__(cls, target, allow):
        ...
        if target_lookup_failed:
            raise TypeError(
                f'`target` can be given either as {Role.__name__}, {ClientUserBase.__name__}, '
        ...
        if __debug__:
            if not isinstance(allow, bool):
                raise AssertionError(
                    f'`allow` can be given as `bool` instance, got {allow.__class__.__name__}.'
                )
