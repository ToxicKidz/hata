...
class Application(DiscordEntity, immortal=True):
    ...
    def _update_no_return(self, data, set_owner=False):
        ...
        if (developers_data is None) or (not developers_data):
            ...
            developers = [
                ApplicationSubEntity(developer_data)
        ...
        if (publishers_data is None) or (not publishers_data):
            ...
            publishers = [
                ApplicationSubEntity(publisher_data)
        ...
        if (executables_data is None) or (not executables_data):
            ...
            executables = [
                ApplicationExecutable(executable_data)
        ...
        if (third_party_skus_data is None) or (not third_party_skus_data):
            ...
            third_party_skus = [
                ThirdPartySKU(third_party_sku_data)
                for third_party_sku_data in third_party_skus_data
            ]
        ...
        if kwargs:
            ...
            try:
                ...
                if not isinstance(owner, (ClientUserBase, Team)):
                    raise TypeError(
                        f'`owner` can be given as `{ClientUserBase.__name__}`  or as '
