...
def _check_name_should_break(name):
    ...
    if type(name) is not str:
        raise TypeError(
            f'`name` should be `None` or type `str`, got `{name.__class__.__name__}`.'
        )
...
def check_parameter_count_and_convert(
    ...
    if analyzer.is_async() or (
        analyzer.is_async_generator() if can_be_async_generator else False
    ):
        min_, max_ = analyzer.get_non_reserved_positional_parameter_range()
        if min_ > expected:
            raise TypeError(
                f'A `{name}` should accept `{expected!r}` parameters, meanwhile the given callable expects '
        ...
        raise TypeError(
            f'A `{name}` should accept `{expected}` parameters, meanwhile the given callable expects up to '
            f'`{max_!r}`, got `{func!r}`.'
        )
    ...
    if analyzer.can_instance_to_async_callable() or (
        ...
        if sub_analyzer.is_async():
            ...
            if min_ > expected:
                raise TypeError(
                    f'A `{name}` should accept `{expected!r}` parameters, meanwhile the given callable '
            ...
            raise TypeError(
                f'A `{name}` should accept `{expected}` parameters, meanwhile the given callable after '
    ...
    if error_message is None:
        error_message = (
            f'Not async callable type, or cannot be instance to async: `{func!r}`.'
        )
    ...
    for element in iterable:
        if type(element) is EventListElement:
            if type_ is not None:
                element = type_.from_args_kwargs(
                    element.func, element.args, element.kwargs
                )
        ...
        if isinstance(element, type_):
            ...
            if isinstance(element, tuple):
                ...
                if element_len > 3 or element_len == 0:
                    raise ValueError(
                        f'Expected `tuple` with length 1 or 2, got `{element!r}`.'
                    )
                ...
                if element_len == 1:
                    ...
                    if (args is not None) and not isinstance(args, tuple):
                        raise ValueError(
                            f'Expected `None` or `tuple` instance at index 1 at element: `{element!r}`'
                        )
                    ...
                    if element_len == 2:
                        ...
                        if kwargs is not None:
                            if type(kwargs) is not dict:
                                raise ValueError(
                                    f'Expected `None` or `dict` instance at index 2 at element: '
                                    f'`{element!r}`'
                                )
...
def create_event_from_class(constructor, klass, parameter_names, name_name, event_name):
    ...
    return constructor(
        *(parameters_by_name[parameter_name][0] for parameter_name in parameter_names)
    )
...
class _EventHandlerManager:
    ...
    def from_class(self, klass):
        ...
        """
        from_class_constructor = getattr(
            type(self.parent), 'create_event_from_class', None
        )
        ...
        if type(iterable) is eventlist:
            ...
            if type_ is not None:
                ...
                if (supported_types is None) or (type_ not in supported_types):
                    raise TypeError(
                        f'`{parent!r}` does not supports elements of type `{type_!r}`.'
                    )
...
class _EventHandlerManagerRouter(_EventHandlerManager):
    ...
    def __call__(self, func=..., *args, **kwargs):
        ...
        **kwargs : Keyword parameters
        ...
        for handler, func_, args, kwargs in zip(
            handlers, routed_func, routed_args, routed_kwargs
        ):
    ...
    def __repr__(self):
        return (
            f'<{self.__class__.__name__} parent={self.parent!r}, getter={self._getter!r}, from_class_constructor='
...
def route_kwargs(kwargs, count):
    ...
    if kwargs is not None:
        for parameter_name, parameter_value in kwargs.items():
            for route_to, parameter in zip(
                result, route_parameter(parameter_value, count)
            ):
    ...
    if isinstance(name, tuple):
        for index, name_value in enumerate(name):
            if (
                ...
                and (name_value is not ...)
                and (not isinstance(name_value, str))
            ):
                raise TypeError(
        ...
        if name is None:
            ...
            raise TypeError(
                '`name` can be given as `None` or as `tuple` of (`None, `Ellipsis`, `str`), got: '
...
class eventlist(list):
    ...
    def __new__(cls, iterable=None, type_=None, **kwargs):
        ...
        **kwargs : Keyword parameters
        ...
        if (type_ is not None) and (not isinstance(type_, type)):
            raise TypeError(
                f'`type_` can be given as `None` or as `type` instance, got `{type_!r}`.'
            )
    ...
    def from_class(self, klass):
        ...
        if from_class is None:
            message = (
                f'The `eventlist`\'s type: `{type_!r}` is not supporting `.from_class`.'
            )
    ...
    def extend(self, iterable):
        ...
        if type(iterable) is type(self):
            if self.type is not iterable.type:
                raise ValueError(
                    f'Extending {self.__class__.__name__} with an other object of the same type, but with '
    ...
    def remove(self, func):
        ...
        raise ValueError(
            f'Did not find any element, what matched the passed func={func!r}, combination.'
        )
...
class EventWaitforMeta(type):
    ...
    async def _call_message_delete(
        self,
        client,
        message,
    ):
...
def EventWaitforMeta__new__(cls, class_name, class_parents, class_attributes):
    ...
    if event_name not in EVENT_HANDLER_NAME_TO_PARSER_NAMES:
        raise TypeError(
            f'`{class_name}.__event_name__` is not set, or not set correctly.'
        )
    ...
    if class_attributes.get('call_waitfors', None) is None:
        try:
            ...
            raise TypeError(
                f'The following event name: `{event_name!r}` has no auto `call_waitfor` added. Please '
                'define one.'
            )
...
class asynclist(list):
    ...
    def __getattribute__(self, name):
        ...
        if not isinstance(name, str):
            raise TypeError(
                f'Attribute name must be string, not `{name.__class__.__name__}`.'
            )
