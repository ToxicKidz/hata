...
from .intent import (
    INTENT_MASK_GUILDS,
    INTENT_MASK_GUILD_USERS,
    INTENT_MASK_GUILD_EMOJIS_AND_STICKERS,
    INTENT_MASK_GUILD_VOICE_STATES,
    INTENT_MASK_GUILD_PRESENCES,
    INTENT_MASK_GUILD_MESSAGES,
    INTENT_MASK_GUILD_REACTIONS,
    INTENT_MASK_DIRECT_MESSAGES,
    INTENT_MASK_DIRECT_REACTIONS,
)
...
def READY(client, data):
    ...
    try:
        ...
        for channel_private_data in channel_private_datas:
            CHANNEL_TYPES.get(channel_private_data['type'], ChannelGuildUndefined)(
                channel_private_data, client
            )
...
add_parser('READY', READY, READY, READY, READY)
del READY
...
add_parser('RESUMED', RESUMED, RESUMED, RESUMED, RESUMED)
del RESUMED
    ...
    def MESSAGE_DELETE__CAL_MC(client, data):
        ...
        try:
            ...
            clients = filter_clients(
                ...
                else INTENT_MASK_DIRECT_MESSAGES,
            )
...
if ALLOW_DEAD_EVENTS:
    ...
    def MESSAGE_REACTION_ADD__CAL_MC(client, data):
        ...
        clients = filter_clients(
            ...
            INTENT_MASK_GUILD_REACTIONS
            if isinstance(channel, ChannelGuildBase)
            else INTENT_MASK_DIRECT_REACTIONS,
        )
        if clients.send(None) is not client:
            clients.close()
            return
...
if CACHE_USER:
    ...
    def GUILD_MEMBER_UPDATE__OPT_MC(client, data):
        ...
        if (
            first_client_or_me(guild.clients, INTENT_MASK_GUILD_USERS, client)
            is not client
        ):
            return
    ...
    def GUILD_MEMBER_UPDATE__CAL_SC(client, data):
        ...
        Task(
            client.events.guild_user_edit(client, client, guild, old_attributes), KOKORO
        )
...
del GUILD_JOIN_REQUEST_DELETE__CAL, GUILD_JOIN_REQUEST_DELETE__OPT
...
del INTEGRATION_CREATE__CAL, INTEGRATION_CREATE__OPT
...
def INTEGRATION_DELETE__CAL(client, data):
    ...
    Task(
        client.events.integration_delete(client, guild, integration_id, application_id),
        KOKORO,
    )
...
del INTEGRATION_DELETE__CAL, INTEGRATION_DELETE__OPT
...
del INTEGRATION_UPDATE__CAL, INTEGRATION_UPDATE__OPT
...
def RELATIONSHIP_ADD__CAL(client, data):
    ...
    if old_relationship is None:
        ...
        coro = client.events.relationship_change(
            client, old_relationship, new_relationship
        )
    Task(coro, KOKORO)
...
add_parser('MESSAGE_ACK', MESSAGE_ACK, MESSAGE_ACK, MESSAGE_ACK, MESSAGE_ACK)
del MESSAGE_ACK
...
del INTERACTION_CREATE__CAL, INTERACTION_CREATE__OPT
...
def APPLICATION_COMMAND_CREATE__CAL(client, data):
    ...
    Task(
        client.events.application_command_create(client, guild_id, application_command),
        KOKORO,
    )
...
del APPLICATION_COMMAND_CREATE__CAL, APPLICATION_COMMAND_CREATE__OPT
...
def APPLICATION_COMMAND_UPDATE__CAL(client, data):
    ...
    Task(
        client.events.application_command_update(
            client, guild_id, application_command, old_attributes
        ),
        KOKORO,
    )
...
del APPLICATION_COMMAND_UPDATE__CAL, APPLICATION_COMMAND_UPDATE__OPT
...
def APPLICATION_COMMAND_DELETE__CAL(client, data):
    ...
    Task(
        client.events.application_command_delete(client, guild_id, application_command),
        KOKORO,
    )
...
del APPLICATION_COMMAND_DELETE__CAL, APPLICATION_COMMAND_DELETE__OPT
...
def APPLICATION_COMMAND_PERMISSIONS_UPDATE__CAL(client, data):
    ...
    Task(
        client.events.application_command_permission_update(
            client, application_command_permission
        ),
        KOKORO,
    )
...
del STAGE_INSTANCE_CREATE__CAL, STAGE_INSTANCE_CREATE__OPT
...
def THREAD_MEMBER_UPDATE__CAL_SC(client, data):
    ...
    Task(
        client.user_thread_profile_edit(client, thread_channel, client, old_attributes),
        KOKORO,
    )
    ...
    if (removed_user_ids is not None) and removed_user_ids:
        for user_id in removed_user_ids:
            ...
            if thread_user_deletion is not None:
                ...
                if event_handler is not DEFAULT_EVENT_HANDLER:
                    Task(
                        event_handler(client, thread_channel, *thread_user_deletion),
                        KOKORO,
                    )
...
def THREAD_MEMBERS_UPDATE__OPT_MC(client, data):
    ...
    if (
        first_client_or_me(thread_channel.clients, INTENT_MASK_GUILD_USERS, client)
        is not client
    ):
        return
