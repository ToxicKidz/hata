...
def parse_time(timestamp):
    ...
    if parsed is None:
        sys.stderr.write(
            f'Cannot parse timestamp: `{timestamp}`, returning `DISCORD_EPOCH_START`\n'
        )
...
def time_to_id(time):
    ...
    id_ `int`
    """
    return ((time.timestamp() * 1000.0).__int__() - DISCORD_EPOCH) << 22
...
def random_id():
    ...
    id_ `int`
    """
    return (((time_now() * 1000.0).__int__() - DISCORD_EPOCH) << 22) + (
        random.random() * 4194304.0
    ).__int__()
...
def log_time_converter(value):
    ...
    raise TypeError(
        f'Expected `int`, `{DiscordEntity.__name__}` instance, or a `datetime` object, got '
...
IS_MENTION_RP = re_compile(
    '@(?:everyone|here)|<(?:@[!&]?|#|/[a-zA-Z0-9_\-]{3,32}:)\d{7,21}>'
)
...
def now_as_id():
    ...
    snowflake : `int`
    """
    return ((time_now() * 1000.0) - DISCORD_EPOCH).__int__() << 22
...
if relativedelta is None:
    ...
    def elapsed_time(
        ...
        if isinstance(delta, datetime):
            ...
            raise TypeError(
                f'Expected, `relativedelta` or `datetime`, got {delta.__class__.__name__}.'
            )
...
class Relationship:
    ...
    def __repr__(self):
        ...
        return (
            f'<{self.__class__.__name__} {self.type.name} user={self.user.full_name!r}>'
        )
