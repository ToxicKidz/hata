...
from ..exceptions import (
    VOICE_CLIENT_DISCONNECT_CLOSE_CODE,
...
class VoiceClient:
    ...
    async def join_audience(self):
        ...
        data = {'suppress': True, 'channel_id': channel.id}
    ...
    def append(self, source):
        ...
        if not issubclass(source_type, AudioSource):
            raise TypeError(
                f'Expected {AudioSource.__name__} instance, received {source_type.__name__}.'
            )
            ...
            while True:
                ...
                try:
                    ...
                    if not (
                        ...
                        and (err.code == VOICE_CLIENT_RECONNECT_CLOSE_CODE)
                    ):
                    ...
                    try:
                        ...
                        if isinstance(err, ConnectionClosed):
                            ...
                            if code == 1000 or (
                                ...
                                and (not self._maybe_change_voice_region())
                            ):
                                self._reconnecting = False
                                await self.disconnect(force=False)
                                return
    ...
    async def _create_socket(self, data):
        ...
        transport, protocol = await KOKORO.create_datagram_endpoint(
            DatagramMergerReadProtocol(KOKORO), socket=socket
        )
