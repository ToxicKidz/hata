...
class LocalAudio(AudioSource):
    ...
    def _create_process_preprocess(source, executable, pipe, before_options, options):
        ...
        if pipe:
            try:
                ...
                raise TypeError(
                    'The given `source` not supports `.fileno()` method'
                ) from err
            try:
            ...
            if source_type is str:
                ...
                raise TypeError(
                    'The given `source` should be given as `str` or as `Path` instance, got '
                    f'{source_type}.'
                )
    ...
    async def __new__(
        ...
        TypeError
            ...
            - If `pipe` was given as `False`, meanwhile `source` was not given as `str` or `Path` instance.
        """
        args = cls._create_process_preprocess(
            source, executable, pipe, before_options, options
        )
...
try:
    ...
    class YTAudio(LocalAudio):
        ...
        def _preprocess(cls, url, stream):
            ...
            args = cls._create_process_preprocess(
                path, DEFAULT_EXECUTABLE, False, before_options, ('-vn',)
            )
        ...
        async def __new__(cls, url, stream=True):
            ...
            TypeError
                ...
                - If `pipe` was given as `False`, meanwhile `source` was not given as `str` or `Path` instance.
            """
            path, data, args = await KOKORO.run_in_executor(
                alchemy_incendiary(cls._preprocess, (cls, url, stream))
            )
