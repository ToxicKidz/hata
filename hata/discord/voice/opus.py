...
class OpusError(Exception):
    ...
    def __new__(cls, code):
        ...
        if opus is None:
            raise RuntimeError(
                f'{cls.__name__} cannot be created if opus is not loaded'
            )
    ...
    try:
        if sys.platform == 'win32':
            filename = os.path.join(
                ...
                'bin',
                f'libopus-0.{"x64" if sys.maxsize>(1<<36) else "x86"}.dll',
            )
        else:
        ...
        EncoderStructPtr = ctypes.POINTER(
            type(
                'EncoderStruct',
                (ctypes.Structure,),
                {},
        ...
        DecoderStructPtr = ctypes.POINTER(
            type(
                'DecoderStruct',
                (ctypes.Structure,),
                {},
...
class OpusEncoder:
    ...
    def __new__(cls):
        ...
        encoder = opus.opus_encoder_create(
            SAMPLING_RATE, CHANNELS, APPLICATION_AUDIO, ctypes.byref(ctypes.c_int())
        )
    ...
    def set_inband_fec(self, enabled):
        ...
        """
        opus.opus_encoder_control(
            self._encoder, SET_INBAND_FEC, int(enabled)
    ...
    def encode(self, data):
        ...
        buffer = self._buffer
        end = opus.opus_encode(
            self._encoder, data, SAMPLES_PER_FRAME, buffer, max_data_bytes
        )
        return buffer[:end]
...
class OpusDecoder:
    ...
    def __new__(cls):
        ...
        decoder = opus.opus_decoder_create(
            SAMPLING_RATE, CHANNELS, ctypes.byref(ctypes.c_int())
        )
    ...
    def _get_last_packet_duration(self):
        ...
        """
        obj = ctypes.c_int32()
        opus.opus_decoder_control(
            self._decoder, GET_LAST_PACKET_DURATION, ctypes.byref(obj)
        )
        return obj.value
    ...
    def decode(self, data):
        ...
        data : `bytes`
        """
        frame_size = self.packet_get_frame_count(
            data
        ) * self.packet_get_samples_per_frame(data)

        buffer = self._buffer
        buffer_ptr = ctypes.cast(buffer, ctypes.POINTER(ctypes.c_int16))

        end = opus.opus_decode(
            self._decoder, data, len(data), buffer_ptr, frame_size, True
        )
