...
class VerificationScreen:
    ...
    def from_data(cls, data):
        ...
        self.steps = tuple(
            VerificationScreenStep.from_data(field_data)
            for field_data in data['form_fields']
        )
        return self
    ...
    def __repr__(self):
        ...
        return (
            f'<{self.__class__.__name__} created_at={self.created_at:{DATETIME_FORMAT_CODE}}, '
...
class VerificationScreenStep:
        ...
        **kwargs : keyword parameters
        ...
        try:
            ...
            if not isinstance(title, str):
                raise TypeError(
                    f'`title` can be given as `str` instance, got {title.__class__.__name__}.'
                )
        ...
        try:
            ...
            if values is not None:
                if not isinstance(values, (list, tuple)):
                    raise TypeError(
                        f'`values` can be given as `tuple` or `list` instance, got '
                ...
                for index, value in enumerate(values):
                    if not isinstance(value, str):
                        raise TypeError(
                            f'`values` index `{index}` is not `str` instance expected, but got '
                            f'{value.__class__.__name__}; {value!r}.'
                        )

                    if not value:
                        raise ValueError(
                            f'`values` index `{index}` is an empty string.'
                        )
        ...
        try:
            ...
            type_ = preconvert_preinstanced_type(
                type_, 'type_', VerificationScreenStepType
            )
    ...
    def __repr__(self):
        ...
        return (
            f'<{self.__class__.__name__} title={self.title!r}, type={self.type.value}, required={self.required!r}, '
