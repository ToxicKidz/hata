...
class SingleUserChunker:
    ...
    def __init__(
        ...
        self.timer = KOKORO.call_at(
            LOOP_TIME() + USER_CHUNK_TIMEOUT, type(self)._cancel, self
        )
...
class DiscoveryTermRequestCacher:
    ...
    __slots__ = (
        '_last_cleanup',
        '_minimal_cleanup_interval',
        '_rate_limit_proxy_args',
        '_waiters',
        'cached',
        'func',
        'timeout',
    ...
    async def execute(self, client, arg):
        ...
        if arg_type is str:
            ...
            raise TypeError(
                f'The parameter can be given as `str` instance, got {arg_type.__class__}.'
            )
        ...
        if not free_count:
            ...
            for client_ in CLIENTS.values():
                ...
                free_count = RateLimitProxy(
                    client_, *self._rate_limit_proxy_args
                ).free_count
                if free_count:
                    requester = client_
                    break
...
def _check_is_client_duped(client, client_id):
    ...
    if other_client is not client:
        raise RuntimeError(
            f'Creating the same client multiple times is not allowed; {client!r} already exists:, '
            f'{other_client!r}.'
        )
    ...
    while True:
                ...
                if message_count:
                    if message_count == 1:
                        ...
                        delete_mass_task = Task(
                            client.http.message_delete_multiple(
        ...
        if delete_old_task is None:
            if message_group_old:
                ...
                delete_old_task = Task(
                    client.http.message_delete_b2wo(channel_id, message_id, reason),
                    KOKORO,
                )
        ...
        if delete_new_task is None:
            ...
            if group is not None:
                ...
                delete_new_task = Task(
                    client.http.message_delete(channel_id, message_id, reason), KOKORO
                )
                tasks.append(delete_new_task)
...
async def _request_members_loop(gateway, guilds):
    ...
    data = {'op': GATEWAY_OPERATION_CODE_REQUEST_MEMBERS, 'd': sub_data}
