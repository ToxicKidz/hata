...
from ..http import (
    ...
    RateLimitProxy,
    rate_limit_groups,
    VALID_ICON_MEDIA_TYPES,
    VALID_ICON_MEDIA_TYPES_EXTENDED,
    is_media_url,
    VALID_STICKER_IMAGE_MEDIA_TYPES,
)
from ..role import Role
from ..webhook import Webhook, create_partial_webhook_from_id
from ..gateway.client_gateway import (
    DiscordGateway,
    DiscordGatewaySharder,
    PRESENCE as GATEWAY_OPERATION_CODE_PRESENCE,
...
from ..interaction.application_command import (
    APPLICATION_COMMAND_LIMIT_GLOBAL,
    APPLICATION_COMMAND_LIMIT_GUILD,
...
from .functionality_helpers import (
    SingleUserChunker,
    MassUserChunker,
    DiscoveryCategoryRequestCacher,
    DiscoveryTermRequestCacher,
    MultiClientMessageDeleteSequenceSharder,
    WaitForHandler,
    _check_is_client_duped,
    _request_members_loop,
    _message_delete_multiple_private_task,
    _message_delete_multiple_task,
...
from .request_helpers import (
    ...
    validate_content_and_embed,
    add_file_to_message_data,
    get_user_id,
    get_channel_and_id,
    get_channel_id_and_message_id,
    get_role_id,
    get_channel_id,
    get_guild_and_guild_text_channel_id,
    get_guild_and_id,
    get_user_id_nullable,
    get_user_and_id,
    get_guild_id,
    get_achievement_id,
    get_achievement_and_id,
    get_guild_discovery_and_id,
    get_guild_id_and_role_id,
    get_guild_id_and_channel_id,
    get_stage_channel_id,
    get_webhook_and_id,
    get_webhook_and_id_token,
    get_webhook_id,
    get_webhook_id_token,
    get_reaction,
    get_emoji_from_reaction,
    get_guild_id_and_emoji_id,
    get_sticker_and_id,
)
...
MESSAGE_FLAG_VALUE_INVOKING_USER_ONLY = MessageFlag().update_by_keys(
    invoking_user_only=True
)
...
class Client(ClientUserPBase):
    ...
    __slots__ = (
        ...
        'verified',  # OAUTH 2
        '__dict__',
        '_additional_owner_ids',
        '_activity',
        '_gateway_requesting',
        '_gateway_time',
        '_gateway_url',
        '_gateway_max_concurrency',
        '_gateway_waiter',
        '_status',
        '_user_chunker_nonce',
        'application',
    ...
    def __new__(
        ...
        **kwargs : keyword parameters
        ...
        if type(token) is str:
            ...
            raise TypeError(
                f'`token` can be passed as `str` instance, got {token.__class__.__name__}.'
            )
        ...
        if (secret is None) or type(secret is str):
            ...
            raise TypeError(
                f'`secret` can be passed as `str` instance, got `{secret.__class__.__name__}`.'
            )
        ...
        # activity
        if (not isinstance(activity, ActivityBase)) or (
            type(activity) is ActivityCustom
        ):
            raise TypeError(
                f'`activity` should have been passed as `{ActivityBase.__name__} instance (except '
        ...
        if type(shard_count) is int:
            ...
            raise TypeError(
                f'`shard_count` should have been passed as `int` instance, got '
                f'{shard_count.__class__.__name__}.'
            )

        if shard_count < 0:
            raise ValueError(
                f'`shard_count` can be passed only as non negative `int`, got {shard_count!r}.'
            )
        ...
        if additional_owners is None:
            ...
            if isinstance(additional_owners, ClientUserBase):
                ...
                if iter_ is None:
                    raise TypeError(
                        '`additional_owners` should have been passed as `iterable`, got '
                ...
                for additional_owner in iter_(additional_owners):
                    if type(additional_owner) is int:
                        ...
                        raise TypeError(
                            f'`additional_owners` contains a non `int` or {ClientUserBase.__name__} '
    ...
    def _delete(self):
        ...
        if self.running:
            raise RuntimeError(
                f'`{self.__class__.__name__}._delete` called from a running client: {self!r}'
            )
        ...
        if __debug__:
            if not isinstance(url, (str, URL)):
                raise AssertionError(
                    f'`url` can be given as `str` or `{URL.__name__}` instance, got '
    ...
    async def download_attachment(self, attachment):
        ...
        if __debug__:
            if not isinstance(attachment, (Attachment, EmbedImage)):
                raise AssertionError(
                    f'`attachment` can be given as `{Attachment.__name__}` or `{EmbedImage.__name__}` '
        ...
        if name is not None:
            if __debug__:
                if not isinstance(name, str):
                    raise AssertionError(
                        f'`name` can be given as `str` instance, got {name.__class__.__name__}.'
                    )
        ...
        if avatar is not ...:
            if avatar is None:
                ...
                if not isinstance(avatar, (bytes, bytearray, memoryview)):
                    raise TypeError(
                        f'`avatar` can be passed as `bytes-like` or None, got {avatar.__class__.__name__}.'
                    )
                ...
                if __debug__:
                    ...
                    if media_type not in valid_icon_media_types:
                        raise AssertionError(
                            f'Invalid avatar type for the client: `{media_type}`.'
                        )
        ...
        if not self.is_bot:
            if __debug__:
                ...
                if not isinstance(password, str):
                    raise AssertionError(
                        '`password` can be passed as `str` instance, got '
            ...
            if email is not None:
                if __debug__:
                    if not isinstance(email, str):
                        raise AssertionError(
                            f'`email` can be given as `str` instance, got {email.__class__.__name__}.'
                        )
            ...
            if new_password is not None:
                if __debug__:
                    if not isinstance(new_password, str):
                        raise AssertionError(
                            '`new_password` can be passed as `str` instance, got '
    ...
    async def client_guild_profile_edit(
        self, guild, *, nick=..., avatar=..., reason=None
    ):
        """
        ...
        if __debug__:
            if nick is not None:
                if not isinstance(nick, str):
                    raise AssertionError(
                        f'`nick` can be given as `None` or `str` instance, got '
    ...
    async def client_edit_presence(self, *, activity=..., status=None, afk=False):
        ...
        if activity is ...:
            ...
            self._activity = ACTIVITY_UNKNOWN
        elif isinstance(activity, ActivityBase) and (
            not isinstance(activity, ActivityCustom)
        ):
            self._activity = activity
        else:
            raise TypeError(
        ...
        if __debug__:
            if not isinstance(afk, bool):
                raise AssertionError(
                    f'`afk` can be given as `bool` instance, got {afk.__class__.__name__}.'
                )
    ...
    async def activate_authorization_code(self, redirect_url, code, scopes):
        ...
        if __debug__:
            if not isinstance(redirect_url, str):
                raise AssertionError(
                    f'`redirect_url` can be given as `str` instance, got '
                    f'{redirect_url.__class__.__name__}.'
                )

            if not isinstance(code, str):
                raise AssertionError(
                    f'`code` can be given as `str` instance, got {code.__class__.__name__}.'
                )
        ...
        if isinstance(scopes, str):
            ...
            if __debug__:
                ...
                for index, scope in enumerate(scopes):
                    if not isinstance(scope, str):
                        raise AssertionError(
                            f'`scopes` element `{index}` is not `str` instance, but '
            ...
            raise TypeError(
                f'`scopes` can be given as `str` or `list` of `str` instances, got '
    ...
    async def user_info_get(self, access):
        ...
        if isinstance(access, (OA2Access, UserOA2)):
            ...
            raise TypeError(
                f'`access` can be given as `{OA2Access.__name__}`, `{UserOA2.__name__}` or `str`'
    ...
    async def renew_access_token(self, access):
        ...
        if __debug__:
            if not isinstance(access, (OA2Access, UserOA2)):
                raise AssertionError(
                    f'`access` can be given as `{OA2Access.__name__}` or `{UserOA2.__name__}` '
                    f'instance, but got {access.__class__.__name__}.'
                )
    ...
    async def guild_user_add(
        ...
        if roles is not None:
            if __debug__:
                if not isinstance(roles, list):
                    raise AssertionError(
                        f'`roles` can be given as `list` or `{Role.__name__}` instances, got '
            ...
            if roles:
                ...
                for index, role in enumerate(roles):
                    if isinstance(role, Role):
                        ...
                        if role_id is None:
                            raise TypeError(
                                f'`roles` element `{index}` is not `{Role.__name__}`, neither `int` '
        ...
        if __debug__:
            if not isinstance(mute, bool):
                raise AssertionError(
                    f'`mute` can be given as `bool` instance, got {mute.__class__.__name__}.'
                )
        ...
        if __debug__:
            if not isinstance(deaf, bool):
                raise AssertionError(
                    f'`deaf` can be given as `bool` instance, got {deaf.__class__.__name__}.'
                )
    ...
    async def user_guild_get_all(self, access):
        ...
        return [
            (
                create_partial_guild_from_data(guild_data),
                UserGuildPermission(guild_data),
            )
    ...
    async def achievement_get(self, achievement_id):
        ...
        if achievement_id_c is None:
            raise TypeError(
                f'`achievement_id` can be given as `int` instance, got '
    ...
    async def achievement_create(
        self, name, description, icon, *, secret=False, secure=False
    ):
        """
        ...
        if __debug__:
            ...
            if not isinstance(description, str):
                raise AssertionError(
                    f'`description` can be given as `str` instance, got '
                    f'{description.__class__.__name__}.'
                )

        if not isinstance(icon, (bytes, bytearray, memoryview)):
            raise TypeError(
                f'`icon` can be passed as `bytes-like`, got {icon.__class__.__name__}.'
            )
        ...
        if __debug__:
            ...
            if media_type not in VALID_ICON_MEDIA_TYPES_EXTENDED:
                raise AssertionError(
                    f'Invalid icon type for achievement: `{media_type}`.'
                )
        ...
        if __debug__:
            if not isinstance(secret, bool):
                raise AssertionError(
                    f'`secret` can be given as `bool` instance, got {secret.__class__.__name__}.'
                )

            if not isinstance(secure, bool):
                raise AssertionError(
                    f'`secure` can be given as `bool` instance, got {secure.__class__.__name__}.'
                )
    ...
    async def achievement_edit(
        ...
        *,
        ...
        if icon is not None:
            ...
            if not isinstance(icon, (bytes, bytearray, memoryview)):
                raise TypeError(
                    f'`icon` can be passed as `bytes-like`, got {icon_type.__name__}.'
                )
        ...
        data = await self.http.achievement_edit(
            self.application.id, achievement_id, data
        )
    ...
    async def user_achievement_update(self, user, achievement, percent_complete):
        ...
        if __debug__:
            if not isinstance(percent_complete, int):
                raise AssertionError(
                    f'`percent_complete` can be given as `int` instance, got '
        ...
        await self.http.user_achievement_update(
            user_id, self.application.id, achievement_id, data
        )
    ...
    async def application_get(self, application):
        ...
        if isinstance(application, Application):
            ...
            if application_id is None:
                raise TypeError(
                    f'`application_id` can be given as `{Application.__name__}` or as `int` instance, got '
    ...
    async def eula_get(self, eula):
        ...
        if isinstance(eula, EULA):
            ...
            if eula_id is None:
                raise TypeError(
                    f'`eula` can be given as `{EULA.__name__}` or as `int` instance, got '
        ...
        if name is not ...:
            if __debug__:
                if name is not None:
                    if not isinstance(name, str):
                        raise AssertionError(
                            f'`name` can be given as `None` or `str` instance, got '
        ...
        *users : ``ClientUserBase`` or `int` instances
        ...
        if __debug__:
            ...
            if user_ids_length < 2:
                raise AssertionError(
                    f'`{ChannelGroup.__name__}` can be created at least with at least `2` users, but '
    ...
    async def channel_move(
        ...
        *,
        ...
        if not isinstance(channel, ChannelGuildMainBase):
            raise TypeError(
                f'`channel` can be given as `{ChannelGuildMainBase.__name__}` instance, got '
        ...
        if isinstance(channel, ChannelCategory) and isinstance(parent, ChannelCategory):
            raise ValueError(
                f'Can not move category channel under category channel. Channel: {channel!r}; Category: '
                f'{parent!r}'
            )

        if not isinstance(visual_position, int):
            raise TypeError(
                f'`visual_position` can be given as `int` instance, got '
                f'{visual_position.__class__.__name__}.'
            )

        if not isinstance(lock_permissions, bool):
            raise TypeError(
                f'`lock_permissions` can be given as `bool` instance, got '
        ...
        if (
            channel.parent is parent
            and parent.channel_list.index(channel) == visual_position
        ):
            return
        ...
        for iter_channel in display_state:
            if isinstance(iter_channel, tuple):
                ...
                for sub_channel in sub_channels:
                    channel_key = (
                        sub_channel.ORDER_GROUP,
                        sub_channel.position,
                        sub_channel.id,
                        None,
                    )
            ...
            channel_key = (
                iter_channel.ORDER_GROUP,
                iter_channel.position,
                iter_channel.id,
                display_sub_channels,
            )
        ...
        *,
        ...
        if topic is not None:
            if __debug__:
                ...
                if not isinstance(topic, str):
                    raise AssertionError(
                        f'`topic` can be given as `str` instance, got {topic.__class__.__name__}.'
                    )
                ...
                if topic_length > topic_length_limit:
                    raise AssertionError(
                        f'`topic` length can be in range [0:{topic_length_limit}], got {topic_length}; '
                        f'{topic!r}.'
                    )
        ...
        if type_ is not None:
            if __debug__:
                ...
                if not isinstance(type_, int):
                    raise AssertionError(
                        f'`type_` can be given as `int` instance, got {type_.__class__.__name__}.'
                    )
        ...
        if nsfw is not None:
            if __debug__:
                ...
                if not isinstance(nsfw, bool):
                    raise AssertionError(
                        f'`nsfw` can be given as `bool` instance, got {nsfw.__class__.__name__}.'
                    )
        ...
        if slowmode is not None:
            if __debug__:
                ...
                if not isinstance(slowmode, int):
                    raise AssertionError(
                        '`slowmode` can be given as `int` instance, got '
        ...
        if bitrate is not None:
            if __debug__:
                ...
                if not isinstance(bitrate, int):
                    raise AssertionError(
                        '`bitrate` can be given as `int` instance, got '
                ...
                if bitrate < 8000 or bitrate > bitrate_limit:
                    raise AssertionError(
                        f'`bitrate` is out of the expected [8000:{bitrate_limit}] range, got '
                        f'{bitrate!r}.'
                    )
        ...
        if region is not ...:
            ...
            if region is None:
                ...
                raise TypeError(
                    f'`region` can be given either as `None`, `str` or as `{VoiceRegion.__name__}` '
        ...
        if video_quality_mode is not None:
            if __debug__:
                if channel is not None:
                    if not isinstance(channel, ChannelVoice):
                        raise AssertionError(
                            f'`video_quality_mode` is a valid parameter only for '
            ...
            if isinstance(video_quality_mode, VideoQualityMode):
                ...
                raise TypeError(
                    f'`video_quality_mode` can be given either as `None`, `str` or as '
    ...
    async def channel_create(
        self, guild, name, type_=ChannelText, *, reason=None, **kwargs
    ):
        """
        ...
        **kwargs : Keyword parameters
        ...
        if guild is not None:
            return CHANNEL_TYPES.get(data['type'], ChannelGuildUndefined)(
                data, self, guild
            )
    ...
    async def channel_follow(self, source_channel, target_channel):
        ...
        """
        source_channel, source_channel_id = get_channel_and_id(
        ...
        if source_channel is None:
            ...
            if __debug__:
                if source_channel.type != 5:
                    raise AssertionError(
                        f'`source_channel` must be type 5 (announcements) channel, got '
                        f'`{source_channel}`.'
                    )

        target_channel, target_channel_id = get_channel_and_id(
            target_channel, ChannelText
        )
        ...
        webhook = await Webhook._from_follow_data(
            data, source_channel, target_channel, self
        )
        return webhook
    ...
    async def message_get_chunk(
        self, channel, limit=100, *, after=None, around=None, before=None
    ):
        """
        ...
        if __debug__:
            if not isinstance(limit, int):
                raise AssertionError(
                    f'`limit` can be given as `int` instance, got {limit.__class__.__name__}.'
                )
    ...
    async def message_get(self, channel, message_id):
        ...
        if message_id_value is None:
            raise TypeError(
                f'`message_id` can be given as `int` instance, got {message_id.__class__.__name__}.'
            )
        ...
        *,
        ...
        if isinstance(channel, ChannelTextBase):
            ...
            if channel_id is not None:
                ...
                if snowflake_pair is None:
                    raise TypeError(
                        f'`channel` can be given as `{ChannelTextBase.__name__}`, `{Message.__name__}`, '
                    ...
                    if isinstance(sticker, (list, tuple)):
                        for sticker in sticker:
                            if isinstance(sticker, Sticker):
                                ...
                                if sticker_id is None:
                                    raise TypeError(
                                        f'`sticker` contains a non `{Sticker.__name__}` nor `int` element, '
                        ...
                        raise TypeError(
                            f'`sticker` can be given as `None`, `{Sticker.__name__}`, `int` or '
        ...
        if __debug__:
            if not isinstance(tts, bool):
                raise AssertionError(
                    f'`tts` can be given as `bool` instance, got {tts.__class__.__name__}.'
                )

            if (nonce is not None) and (not isinstance(nonce, str)):
                raise AssertionError(
                    f'`nonce` can be given either as `None` or as `str` instance, got '
                    f'{nonce.__class__.__name__}.'
                )

            if not isinstance(reply_fail_fallback, bool):
                raise AssertionError(
                    f'`reply_fail_fallback` can be given as `bool` instance, got '
    ...
    async def message_delete(self, message, *, reason=None):
        ...
        if (author is self) or (
            message_id > int((time_now() - 1209590.0) * 1000.0 - DISCORD_EPOCH) << 22
        ):
            # own or new
    ...
    async def message_delete_multiple(self, messages, *, reason=None):
        ...
        if __debug__:
            if not isinstance(messages, (list, set, tuple)):
                raise AssertionError(
                    f'`messages` can be given as `list`, `set` or `tuple` instance.'
                )
        ...
        bulk_delete_limit = (
            int((time_now() - 1209600.0) * 1000.0 - DISCORD_EPOCH) << 22
        )  # 2 weeks
    ...
    async def message_delete_sequence(
        ...
        if channel.message_history_reached_end or (
            not permissions.can_read_message_history
        ):
        ...
        while True:
            if should_request and (get_mass_task is None):
                ...
                get_mass_task = Task(
                    self.http.message_get_chunk(channel_id, request_data), KOKORO
                )
            ...
            if delete_mass_task is None:
                ...
                if message_limit:
                    time_limit = (
                        int((time_now() - 1209590.0) * 1000.0 - DISCORD_EPOCH) << 22
                    )  # 2 weeks -10s
                    collected = 0
                    ...
                    if collected == 0:
                        ...
                        delete_mass_task = Task(
                            self.http.message_delete_multiple(
                                channel_id, {'messages': message_ids}, reason
                            ),
                            KOKORO,
                        )
            ...
            if delete_new_task is None:
                ...
                if message_group_old_own:
                    ...
                    delete_new_task = Task(
                        self.http.message_delete(channel_id, message_id, reason), KOKORO
                    )
                    tasks.append(delete_new_task)
            ...
            if delete_old_task is None:
                if message_group_old:
                    ...
                    delete_old_task = Task(
                        self.http.message_delete_b2wo(channel_id, message_id, reason),
                        KOKORO,
                    )
    ...
    async def multi_client_message_delete_sequence(
        ...
        if __debug__:
            if not isinstance(channel, ChannelTextBase):
                raise AssertionError(
                    f'`channel` should have been given as `{ChannelTextBase.__name__}` instance, got '
                    f'{channel.__class__.__name__}.'
                )
        ...
        while True:
            if should_request and (get_mass_task is None):
                ...
                get_mass_task = Task(
                    sharder.client.http.message_get_chunk(channel_id, request_data),
                    KOKORO,
                )
                tasks.append(get_mass_task)
                    ...
                    if message_limit:
                        ...
                        if collected == 0:
                            ...
                            for sub_sharder in sharders:
                                if (sub_sharder.can_manage_messages) and (
                                    sharder.delete_new_task is None
                                ):
                                    # We collected 1 message -> We cannot use mass delete on this.
                                    who_s, message_id = message_group_new.popleft()
                                    delete_new_task = Task(
                                        sub_sharder.client.http.message_delete(
                                            channel_id, message_id, reason=reason
                                        ),
                                        KOKORO,
                                    )
                            ...
                            delete_mass_task = Task(
                                sharder.client.http.message_delete_multiple(
                                    channel_id, {'messages': message_ids}, reason
                                ),
                                KOKORO,
                            )
                        ...
                        if message_limit:
                            ...
                            while True:
                                ...
                                if who_s == -1:
                                    ...
                                    message_group_old_own.appendleft(
                                        (
                                            who_s,
                                            message_group_old,
                                        ),
            ...
            if message_group_old_own:
                ...
                if sharder.delete_new_task is None:
                    ...
                    delete_new_task = Task(
                        sharder.client.http.message_delete(
                            channel_id, message_id, reason
                        ),
                        KOKORO,
                    )
            ...
            if message_group_old:
                for sharder in sharders:
                    if sharder.delete_old_task is None:
                        message_id = message_group_old.popleft()
                        delete_old_task = Task(
                            sharder.client.http.message_delete_b2wo(
                                channel_id, message_id, reason
                            ),
                            KOKORO,
                        )
        ...
        *,
        ...
        if __debug__:
            if (suppress is not ...) and (not isinstance(suppress, bool)):
                raise AssertionError(
                    f'`suppress` can be given as `bool` instance, got {suppress.__class__.__name__}.'
                )
    ...
    async def message_suppress_embeds(self, message, suppress=True):
        ...
        await self.http.message_suppress_embeds(
            channel_id, message_id, {'suppress': suppress}
        )
    ...
    async def _load_messages_till(self, channel, index):
        ...
        while True:
            ...
            if ln:
                result = await self.message_get_chunk(
                    channel, planned, before=messages[ln - 1].id + 1
                )
            else:
    ...
    async def message_get_at_index(self, channel, index):
        ...
        if __debug__:
            if not isinstance(index, int):
                raise AssertionError(
                    f'`index` can be given as `int` instance, got {index.__class__.__name__}.'
                )
        ...
        channel, channel_id = get_channel_and_id(channel, ChannelTextBase)
        if channel is None:
            messages = await self.message_get_chunk_from_zero(
                channel_id, min(index + 1, 100)
            )
    ...
    async def message_get_all_in_range(self, channel, start=0, end=100):
        ...
        if __debug__:
            if not isinstance(start, int):
                raise AssertionError(
                    f'`start` can be given as `int` instance, got {start.__class__.__name__}.'
                )
            ...
            if not isinstance(end, int):
                raise AssertionError(
                    f'`end` can be given as `int` instance, got {end.__class__.__name__}.'
                )
        ...
        if (
            (end >= ln)
            and (not channel.message_history_reached_end)
            and channel.cached_permissions_for(self).can_read_message_history
        ):
    ...
    async def reaction_delete(self, message, emoji, user):
        ...
        if user_id == self.id:
            await self.http.reaction_delete_own(
                channel_id, message_id, emoji_as_reaction
            )
        else:
            await self.http.reaction_delete(
                channel_id, message_id, emoji_as_reaction, user_id
            )
    ...
    async def reaction_user_get_chunk(self, message, emoji, *, limit=None, after=None):
        ...
        if limit is None:
            ...
            if __debug__:
                if not isinstance(limit, int):
                    raise AssertionError(
                        f'`limit` can be given as `None` or `int` instance, got '
        ...
        data = await self.http.reaction_user_get_chunk(
            channel_id, message_id, emoji.as_reaction, data
        )
    ...
    async def reaction_user_get_all(self, message, emoji):
        ...
        while True:
            user_datas = await self.http.reaction_user_get_chunk(
                channel_id, message_id, reaction, data
            )
    ...
    async def welcome_screen_edit(
        self, guild, *, enabled=..., description=..., welcome_channels=...
    ):
        """
        ...
        if enabled is not ...:
            if __debug__:
                if not isinstance(enabled, bool):
                    raise AssertionError(
                        f'`enabled` can be given as `bool` instance, got '
        ...
        if description is not ...:
            if __debug__:
                if description is not None:
                    if not isinstance(description, str):
                        raise AssertionError(
                            f'`description` can be given as `None` or `str` instance, got '
                            f'{description.__class__.__name__}.'
                        )
                    ...
                    if description_length > 300:
                        raise AssertionError(
                            f'`description` length can be in range [0:140], got '
            ...
            if welcome_channel_datas is None:
                ...
                for index, welcome_channel in enumerate(welcome_channels):
                    if not isinstance(welcome_channel, WelcomeChannel):
                        raise TypeError(
                            f'Welcome channel `{index}` was not given as `{WelcomeChannel.__name__}` '
                ...
                raise TypeError(
                    f'`welcome_channels` can be given as `None`, `{WelcomeChannel.__name__}` or as '
    ...
    async def verification_screen_get(self, guild):
        ...
        if (guild is None) or (GuildFeature.verification_screen in guild.features):
            ...
            if verification_screen_data is None:
                ...
                verification_screen = VerificationScreen.from_data(
                    verification_screen_data
                )
        else:
            verification_screen = None
    ...
    async def verification_screen_edit(
        self, guild, *, enabled=..., description=..., steps=...
    ):
        """
        Requests the given guild's verification screen.
        ...
        if description is not ...:
            if __debug__:
                if description is not None:
                    ...
                    if description_length > 300:
                        raise AssertionError(
                            f'`description` length can be in range [0:300], got {description_length!r}; '
                            f'{description!r}.'
                        )
            ...
            if steps is None:
                ...
                for index, step in enumerate(steps):
                    if not isinstance(step, VerificationScreenStep):
                        raise TypeError(
                            f'`step` element `{index}` was not given as '
                ...
                raise TypeError(
                    f'`steps` can be given as `None`, `{VerificationScreenStep.__name__}` or as '
    ...
    async def guild_ban_add(self, guild, user, *, delete_message_days=0, reason=None):
        ...
        if __debug__:
            if not isinstance(delete_message_days, int):
                raise AssertionError(
                    '`delete_message_days` can be given as `int` instance, got '
    ...
    async def guild_create(
        ...
        *,
        ...
        """
        if __debug__:
            if len(self.guild_profiles) > (
                9 if self.is_bot else 199 if self.flags.staff else 99
            ):
                if self.is_bot:
        ...
        if __debug__:
            ...
            if name_length < 2 or name_length > 100:
                raise AssertionError(
                    f'`name` length can be in range [2:100], got {name_length}'
                )
        ...
        if isinstance(region, VoiceRegion):
            ...
            raise TypeError(
                f'`region` can be given either as `{VoiceRegion.__name__}` or `str` instance, got '
        ...
        if isinstance(verification_level, VerificationLevel):
            ...
            raise TypeError(
                f'`verification_level` can be given either as {VerificationLevel.__name__} or `int` '
        ...
        if isinstance(message_notification, MessageNotificationLevel):
            ...
            raise TypeError(
                f'`message_notification` can be given either as `{MessageNotificationLevel.__name__}`'
        ...
        if isinstance(content_filter, ContentFilterLevel):
            ...
            raise TypeError(
                f'`content_filter` can be given either as {ContentFilterLevel.__name__} or `int` '
        ...
        if afk_channel_id is not None:
            if __debug__:
                if not isinstance(afk_channel_id, int):
                    raise AssertionError(
                        f'`afk_channel_id` can be given as `int` instance, got '
        ...
        if system_channel_id is not None:
            if __debug__:
                if not isinstance(system_channel_id, int):
                    raise AssertionError(
                        f'`system_channel_id` can be given as `int` instance, got '
        ...
        if afk_timeout is not None:
            if __debug__:
                if not isinstance(afk_timeout, int):
                    raise AssertionError(
                        '`afk_timeout` can be given as `int` instance, got '
                        f'{afk_timeout.__class__.__name__}.'
                    )

                if afk_timeout not in (60, 300, 900, 1800, 3600):
                    raise AssertionError(
                        f'`afk_timeout` should be 60, 300, 900, 1800, 3600 seconds!, got '
                        f'`{afk_timeout!r}`'
                    )
    ...
    async def guild_prune(self, guild, days, *, roles=None, count=False, reason=None):
        ...
        if __debug__:
            if not isinstance(count, bool):
                raise AssertionError(
                    f'`count` can be given as `bool` instance, got {count.__class__.__name__}.'
                )
        ...
        if __debug__:
            if not isinstance(days, int):
                raise AssertionError(
                    f'`days can be given as `int` instance, got {days.__class__.__name__}.'
                )
        ...
        if roles is not None:
            if __debug__:
                if not isinstance(roles, list):
                    raise AssertionError(
                        f'`roles` can be given as `None` or `list` of `{Role.__name__}` and `int`'
            ...
            for index, role in enumerate(roles):
                if isinstance(role, Role):
                    ...
                    if role_id is None:
                        raise TypeError(
                            f'`roles` index {index} was expected to be {Role.__name__} or `int` instance,'
        ...
        *,
        ...
        if icon is not ...:
            if icon is None:
                ...
                if not isinstance(icon, (bytes, bytearray, memoryview)):
                    raise TypeError(
                        f'`icon` can be passed as `None` or `bytes-like`, got {icon.__class__.__name__}.'
                    )
                ...
                if __debug__:
                    ...
                    if media_type not in valid_icon_media_types:
                        raise AssertionError(
                            f'Invalid icon type for the guild: `{media_type}`.'
                        )
        ...
        if banner is not ...:
            if __debug__:
                if (guild is not None) and (GuildFeature.banner not in guild.features):
                    raise AssertionError(
                        'The guild has no `BANNER` feature, meanwhile `banner` is given.'
                    )
            ...
            if banner is None:
                ...
                if not isinstance(banner, (bytes, bytearray, memoryview)):
                    raise TypeError(
                        f'`banner` can be passed as `None` or `bytes-like`, got '
        ...
        if invite_splash is not ...:
            if __debug__:
                if (guild is not None) and (
                    GuildFeature.invite_splash not in guild.features
                ):
                    raise AssertionError(
                        'The guild has no `INVITE_SPLASH` feature, meanwhile `invite_splash` is '
                        f'given.'
                    )
            ...
            if invite_splash is None:
                ...
                if not isinstance(invite_splash, (bytes, bytearray, memoryview)):
                    raise TypeError(
                        f'`invite_splash` can be passed as `bytes-like`, got '
                ...
                if __debug__:
                    ...
                    if media_type not in VALID_ICON_MEDIA_TYPES:
                        raise AssertionError(
                            f'Invalid invite splash type: `{media_type}`.'
                        )
        ...
        if discovery_splash is not ...:
            if __debug__:
                if (guild is not None) and (
                    GuildFeature.discoverable not in guild.features
                ):
                    raise AssertionError(
                        'The guild is not discoverable, but `discovery_splash` was given.'
                    )
            ...
            if discovery_splash is None:
                ...
                if not isinstance(discovery_splash, (bytes, bytearray, memoryview)):
                    raise TypeError(
                        f'`discovery_splash` can be passed as `bytes-like`, got '
                ...
                if __debug__:
                    ...
                    if media_type not in VALID_ICON_MEDIA_TYPES:
                        raise AssertionError(
                            f'Invalid discovery_splash type: `{media_type}`.'
                        )
        ...
        if afk_channel is not ...:
            if afk_channel is None:
                ...
                if afk_channel_id is None:
                    raise TypeError(
                        f'`afk_channel` can be given as `None`, `{ChannelVoice.__name__}` or `int` '
        ...
        if system_channel is not ...:
            if system_channel is None:
                ...
                if system_channel_id is None:
                    raise TypeError(
                        f'`system_channel` can be given as `None`, `{ChannelText.__name__}` or `int` '
        ...
        if rules_channel is not ...:
            if __debug__:
                if (guild is not None) and (
                    not COMMUNITY_FEATURES.intersection(guild.features)
                ):
                    raise AssertionError(
                        'The guild is not Community guild and `rules_channel` was given.'
                    )
            ...
            if rules_channel is None:
                ...
                if rules_channel_id is None:
                    raise TypeError(
                        f'`rules_channel` can be given as `None`, `{ChannelText.__name__}` or `int` '
        ...
        if public_updates_channel is not ...:
            if __debug__:
                if (guild is not None) and (
                    ...
                    raise AssertionError(
                        'The guild is not Community guild and `public_updates_channel` was given.'
                    )
            ...
            if public_updates_channel is None:
                ...
                if public_updates_channel_id is None:
                    raise TypeError(
                        f'`public_updates_channel` can be given as `None`, `{ChannelText.__name__}` or '
        ...
        if owner is not None:
            ...
            if isinstance(owner, ClientUserBase):
                ...
                if owner_id is None:
                    raise TypeError(
                        f'`owner` can be given as `{UserBase.__name__}` instance or as `int`, got '
        ...
        if afk_timeout is not None:
            if __debug__:
                ...
                if afk_timeout not in (60, 300, 900, 1800, 3600):
                    raise AssertionError(
                        f'Afk timeout should be one of (60, 300, 900, 1800, 3600) seconds, got '
                        f'`{afk_timeout!r}`.'
                    )
        ...
        if description is not ...:
            if __debug__:
                if (guild is not None) and (
                    ...
                    raise AssertionError(
                        'The guild is not Community guild and `description` was given.'
                    )
            ...
            if description is None:
                ...
                raise TypeError(
                    '`description` can be either given as `None` or `str` instance, got '
                    f'{description.__class__.__name__}.'
                )
        ...
        if preferred_locale is not None:
            if __debug__:
                if (guild is not None) and (
                    ...
                    raise AssertionError(
                        'The guild is not Community guild and `preferred_locale` was given.'
                    )

                if not isinstance(preferred_locale, str):
                    raise AssertionError(
                        '`preferred_locale` can be given as `str` instance, got '
        ...
        if system_channel_flags is not None:
            if __debug__:
                if not isinstance(system_channel_flags, int):
                    raise AssertionError(
                        f'`system_channel_flags` can be given as `{SystemChannelFlag.__name__}` '
        ...
        if (add_feature is not None) or (remove_feature is not None):
            ...
            while True:
                ...
                if isinstance(add_feature, GuildFeature):
                    ...
                    if iter_func is None:
                        raise TypeError(
                            f'`add_feature` can be given as `str`, as `{GuildFeature.__name__}` or as '
            ...
            while True:
                ...
                if isinstance(remove_feature, GuildFeature):
                    ...
                    if iter_func is None:
                        raise TypeError(
                            f'`remove_feature` can be given as `str`, as `{GuildFeature.__name__}` or as '
                            f'`iterable` of (`str` or `{GuildFeature.__name__}`), got '
    ...
    async def guild_ban_get_all(self, guild):
        ...
        return [
            BanEntry(User(ban_data['user']), ban_data.get('reason', None))
    ...
    async def guild_discovery_edit(
        self, guild, *, primary_category=..., keywords=..., emoji_discovery=...
    ):
        """
        ...
        if type(guild) is Guild:
            ...
            raise TypeError(
                f'`guild` can be `{Guild.__name__}` or `{GuildDiscovery.__name__}` instance, '
            ...
            if primary_category is None:
                ...
                if primary_category_type is DiscoveryCategory:
                    ...
                    if primary_category.name and (not primary_category.primary):
                        raise ValueError(
                            f'The given `primary_category_id` was not given as a primary '
                    ...
                    raise TypeError(
                        f'`primary_category` can be given as `None`, `int` instance, or as '
        ...
        if keywords is not ...:
            if keywords is None:
                pass
            elif (not isinstance(keywords, str)) and hasattr(
                type(keywords), '__iter__'
            ):
                ...
                for keyword in keywords:
                    if type(keyword) is str:
                        ...
                        raise TypeError(
                            f'`keywords` can be `None` or `iterable` of `str`. Got `iterable`, but it\'s '
                ...
                raise TypeError(
                    f'`keywords` can be `None` or `iterable` of `str`. Got {keywords.__class__.__name__}.'
                )
        ...
        if emoji_discovery is not ...:
            if (emoji_discovery is None) or (type(emoji_discovery) is bool):
                ...
                if emoji_discovery == 0:
                    ...
                    raise ValueError(
                        f'`emoji_discovery` was given as `int` instance, but not as `0`, or `1`, got '
                        f'{emoji_discovery!r}.'
                    )
            else:
                raise TypeError(
                    f'`emoji_discovery` can be given as `None`, `bool` or as `int` instance as `0` or '
    ...
    async def guild_discovery_add_subcategory(self, guild, category):
        ...
        if category_type is DiscoveryCategory:
            ...
            raise TypeError(
                f'`category` can be given either as `int` or as `{DiscoveryCategory.__name__} instance, '
    ...
    discovery_category_get_all = DiscoveryCategoryRequestCacher(
        _discovery_category_get_all, 3600.0, cached=list(DISCOVERY_CATEGORIES.values())
    )
    ...
    discovery_validate_term = DiscoveryTermRequestCacher(
        discovery_validate_term, 86400.0, rate_limit_groups.discovery_validate_term
    )
    ...
    async def guild_get_all(self):
        ...
        while True:
            ...
            result.extend(
                create_partial_guild_from_data(guild_data) for guild_data in data
            )
            if len(data) < 100:
                break
    ...
    async def audit_log_get_chunk(
        self, guild, limit=100, *, before=None, after=None, user=None, event=None
    ):
        """
        ...
        if user is not None:
            if isinstance(user, ClientUserBase):
                ...
                if user_id is None:
                    raise TypeError(
                        f'`user` can be given as `{ClientUserBase.__name__}` or `int` instance, '
        ...
        if event is not None:
            if isinstance(event, AuditLogEvent):
                ...
                raise TypeError(
                    f'`event` can be given as `None`, `{AuditLogEvent.__name__}` or `int` instance, got '
    ...
    async def user_guild_profile_edit(
        ...
        *,
        ...
        if nick is not ...:
            ...
            if should_edit_nick:
                if self.id == user_id:
                    await self.http.client_guild_profile_edit(
                        guild_id, {'nick': nick}, reason
                    )
                else:
                    data['nick'] = nick
        ...
        if voice_channel is not ...:
            if voice_channel is None:
                ...
                if voice_channel_id is None:
                    raise TypeError(
                        f'`voice_channel` can be given either as `None`, `{ChannelVoiceBase.__name__}` '
            ...
            if roles is not None:
                if __debug__:
                    if not isinstance(roles, (list, set, tuple)):
                        raise AssertionError(
                            f'`roles` can be given either `None`, `list`, `set` or `tuple` instance, '
                ...
                for role in roles:
                    if isinstance(role, Role):
                        ...
                        if role_id is None:
                            raise TypeError(
                                f'`roles` contains not `{Role.__name__}` neither `int` instance element, '
    ...
    async def stage_create(
        self, channel, topic, *, privacy_level=StagePrivacyLevel.guild_only
    ):
        """
        ...
        if topic is None:
            ...
            if __debug__:
                if not isinstance(topic, str):
                    raise AssertionError(
                        f'`topic` can be given as `None` or `sts` instance, got '
        ...
        if isinstance(privacy_level, StagePrivacyLevel):
            ...
            raise TypeError(
                f'`privacy_level` can be given either as {StagePrivacyLevel.__name__} or `int` '
    ...
    async def thread_create(self, message, name, *, auto_archive_after=None):
        ...
        if isinstance(message, ChannelTextBase):
            ...
            if channel_id is not None:
                ...
                if snowflake_pair is None:
                    raise TypeError(
                        f'`message` can be given as `{ChannelTextBase.__name__}`, `{Message.__name__}`, '
                        f'`{MessageRepr.__name__}`, `{MessageReference.__name__}`, `int` or `tuple` (`int`, `int`), '
        ...
        if auto_archive_after is None:
            ...
            if __debug__:
                if not isinstance(auto_archive_after, int):
                    raise AssertionError(
                        f'`auto_archive_after` can be given as `None` or as `datetime` instance, got '
                        f'{auto_archive_after.__class__.__name__}.'
                    )

                if auto_archive_after not in AUTO_ARCHIVE_OPTIONS:
                    raise AssertionError(
                        f'`auto_archive_after` can be any of: '
    ...
    async def thread_get_all_active(self, channel):
        ...
        return await request_thread_channels(
            self, guild, channel_id, type(self.http).thread_get_chunk_active
        )
    ...
    async def thread_get_all_archived_private(self, channel):
        ...
        guild, channel_id = get_guild_and_guild_text_channel_id(channel)
        return await request_thread_channels(
            self, guild, channel_id, type(self.http).thread_get_chunk_archived_private
        )
    ...
    async def thread_get_all_archived_public(self, channel):
        ...
        guild, channel_id = get_guild_and_guild_text_channel_id(channel)
        return await request_thread_channels(
            self, guild, channel_id, type(self.http).thread_get_chunk_archived_public
        )
    ...
    async def thread_get_all_self_archived(self, channel):
        ...
        guild, channel_id = get_guild_and_guild_text_channel_id(channel)
        return await request_thread_channels(
            self, guild, channel_id, type(self.http).thread_get_chunk_self_archived
        )
    ...
    async def guild_user_search(self, guild, query, limit=1):
        ...
        if __debug__:
            if not isinstance(query, str):
                raise AssertionError(
                    f'`query` can be given as `str` instance, got {query.__class__.__name__}.'
                )
    ...
    if API_VERSION > 7:
        ...
        async def integration_get_all(self, guild):
            ...
            return [
                Integration(integration_data) for integration_data in integration_datas
            ]
        ...
        async def integration_get_all(self, guild):
            ...
            integration_datas = await self.http.integration_get_all(
                guild_id, {'include_applications': True}
            )
            return [
                Integration(integration_data) for integration_data in integration_datas
            ]
    ...
    async def integration_create(self, guild, integration_id, type_):
        ...
        if integration_id_value is None:
            raise TypeError(
                f'`integration_id` can be given as `int` instance, got '
        ...
        *,
        ...
        if __debug__:
            if not isinstance(integration, Integration):
                raise AssertionError(
                    f'`integration` can be given as `{Integration.__name__}` instance, got '
        ...
        if expire_behavior is not None:
            if __debug__:
                if not isinstance(expire_behavior, int):
                    raise AssertionError(
                        f'`expire_behavior` can be given either as `None` or as `int` instance, got '
                        f'{expire_behavior.__class__.__name__}.'
                    )

                if expire_behavior not in (0, 1):
                    raise AssertionError(
                        f'`expire_behavior` should be 0 for kick, 1 for remove role, got '
                        f'{expire_behavior!r}.'
                    )
        ...
        if expire_grace_period is not None:
            if __debug__:
                if not isinstance(expire_grace_period, int):
                    raise AssertionError(
                        f'`expire_grace_period` can be given either as `None` or as `int` instance, '
                        f'got {expire_grace_period.__class__.__name__}.'
                    )

                if expire_grace_period not in (1, 3, 7, 14, 30):
                    raise AssertionError(
                        f'`expire_grace_period` can be one of `(1, 3, 7, 14, 30)`, got '
        ...
        if enable_emojis is not None:
            if __debug__:
                if not isinstance(enable_emojis, bool):
                    raise AssertionError(
                        f'`enable_emojis` can be given either as `None` or as `bool` instance, '
    ...
    async def permission_overwrite_edit(
        self, channel, overwrite, *, allow=None, deny=None, reason=None
    ):
        """
        ...
        if __debug__:
            if not isinstance(overwrite, PermissionOverwrite):
                raise AssertionError(
                    f'`overwrite` can be given as `{PermissionOverwrite.__name__}` instance, got '
        ...
        if allow is None:
            ...
            if __debug__:
                if not isinstance(allow, int):
                    raise AssertionError(
                        f'`allow` can be given either as `None`, `{Permission.__name__}` or as other '
        ...
        if deny is None:
            ...
            if __debug__:
                if not isinstance(deny, int):
                    raise AssertionError(
                        f'`deny` can be given either as `None`, `{Permission.__name__}` or as other '
                        f'`int` instance, got {deny.__class__.__name__}.'
                    )

        data = {'allow': allow, 'deny': deny, 'type': overwrite.type.value}

        await self.http.permission_overwrite_create(
            channel_id, overwrite.target.id, data, reason
        )
    ...
    async def permission_overwrite_delete(self, channel, overwrite, *, reason=None):
        ...
        await self.http.permission_overwrite_delete(
            channel_id, overwrite.target.id, reason
        )

    async def permission_overwrite_create(
        self, channel, target, allow, deny, *, reason=None
    ):
        """
        ...
        if isinstance(target, Role):
            ...
            raise TypeError(
                f'`target` can be either `{Role.__name__}` or `{ClientUserBase.__name__}` '
        ...
        if __debug__:
            if not isinstance(allow, int):
                raise AssertionError(
                    f'`allow` can be given as `{Permission.__name__}` or as other `int` instance, '
                    f'got {allow.__class__.__name__}.'
                )

            if not isinstance(deny, int):
                raise AssertionError(
                    f'`deny` can be given as `{Permission.__name__}` or as other `int` instance, '
    ...
    async def webhook_create(self, channel, name, *, avatar=None):
        ...
        if avatar is not None:
            ...
            if not issubclass(avatar_type, (bytes, bytearray, memoryview)):
                raise TypeError(
                    f'`avatar` can be passed as `bytes-like`, got {avatar_type.__name__}.'
                )
        ...
        if channel is not None:
            if isinstance(channel, ChannelText):
                ...
                if channel_id is None:
                    raise TypeError(
                        f'`channel` can be given either as `{ChannelText.__name__}` or as `int` instance, got '
                        f'{channel.__class__.__name__}.'
                    )
        ...
        *,
        ...
        if thread is None:
            ...
            if thread_id is None:
                raise TypeError(
                    f'`thread` can be given as `None`, `{ChannelThread.__name__}`, or as `int` instance, '
        ...
        if __debug__:
            ...
            if not isinstance(wait, bool):
                raise AssertionError(
                    f'`wait` can be given as `bool` instance, got {wait.__class__.__name__}.'
                )
        ...
        if avatar_url is not None:
            if __debug__:
                if not isinstance(avatar_url, str):
                    raise AssertionError(
                        f'`avatar_url` can be given as `None` or `str` instance, got '
        ...
        if name is not None:
            if __debug__:
                if not isinstance(name, str):
                    raise AssertionError(
                        f'`name` cane be given either as `None` or `str instance, got '
                        f'{name.__class__.__name__}'
                    )
        ...
        message_data = await self.http.webhook_message_create(
            webhook_id, webhook_token, message_data, query_parameters
        )
        ...
        *,
        ...
        if isinstance(message, Message):
            ...
            if message_id is not None:
                ...
                raise TypeError(
                    f'`message` can be given as `{Message.__name__}`, `{MessageRepr.__name__}` or as '
        ...
        await self.http.webhook_message_edit(
            webhook_id, webhook_token, message_id, message_data
        )
    ...
    async def webhook_message_delete(self, webhook, message):
        ...
        if isinstance(message, Message):
            ...
            if message_id is not None:
                ...
                raise AssertionError(
                    '`message` parameter was given as `None`. Make sure to use '
    ...
    async def webhook_message_get(self, webhook, message_id):
        ...
        message_data = await self.http.webhook_message_get(
            webhook_id, webhook_token, message_id_value
        )
    ...
    async def emoji_get(self, guild, emoji):
        ...
        if isinstance(emoji, Emoji):
            ...
            if emoji_id is None:
                raise TypeError(
                    f'`emoji` can be given either as `{Emoji.__name__}` or as `int` instance, got '
    ...
    async def emoji_create(self, guild, name, image, *, roles=None, reason=None):
        ...
        data = {'name': name, 'image': image, 'roles': role_ids}
    ...
    async def emoji_edit(self, emoji, *, name=None, roles=..., reason=None):
        ...
        if __debug__:
            if not isinstance(emoji, Emoji):
                raise AssertionError(
                    f'`emoji` can be given as `{Emoji.__name__}` instance, got '
                    f'{emoji.__class__.__name__}.'
                )
        ...
        if name is None:
            ...
            if __debug__:
                if not isinstance(name, str):
                    raise AssertionError(
                        f'`name` can be given as `st` instance, got {name.__class__.__name__}.'
                    )
    ...
    async def vanity_invite_edit(self, guild, vanity_code, *, reason=None):
        ...
        if __debug__:
            if not isinstance(vanity_code, str):
                raise AssertionError(
                    f'`vanity_code` can be given as `str` instance, got '
    ...
    async def invite_create(
        self, channel, *, max_age=0, max_uses=0, unique=True, temporary=False
    ):
        """
        ...
        if __debug__:
            if not isinstance(max_age, int):
                raise AssertionError(
                    f'`max_age` can be given as `int` instance, got {max_age.__class__.__name__}.'
                )

            if not isinstance(max_uses, int):
                raise AssertionError(
                    f'`max_uses` can be given as `int` instance, got {max_uses.__class__.__name__}.'
                )

            if not isinstance(unique, bool):
                raise AssertionError(
                    f'`unique` can be given as `bool` instance, got {unique.__class__.__name__}.'
                )

            if not isinstance(temporary, bool):
                raise AssertionError(
                    f'`temporary` can be given as `bool` instance, got '
    ...
    async def stream_invite_create(
        self, guild, user, *, max_age=0, max_uses=0, unique=True, temporary=False
    ):
        """
        ...
        if __debug__:
            if not isinstance(guild, Guild):
                raise AssertionError(
                    f'`guild` can be given as `{Guild.__name__}` instance, got '
        ...
        try:
            ...
            raise ValueError(
                'The user must stream at a voice channel of the guild!'
            ) from None
    ...
    async def application_invite_create(
        ...
        *,
        ...
        if isinstance(application, Application):
            ...
            if application_id is None:
                raise TypeError(
                    f'`application` can be given as `{Application.__name__}` or as `int` instance, got '
        ...
        **kwargs : Keyword parameters
        ...
        while True:
            if not guild.channels:
                raise ValueError(
                    'The guild has no channels (yet?), try waiting for dispatch or create a channel'
                )
            ...
            for channel_type in (0, 2):
                ...
                raise ValueError(
                    'The guild has only category channels and cannot create invite from them!'
                )
            break
    ...
    async def invite_get(self, invite, *, with_count=True):
        ...
        if __debug__:
            if not isinstance(with_count, bool):
                raise AssertionError(
                    f'`with_count` can be given as `str` instance, got '
                    f'{with_count.__class__.__name__}.'
                )

        invite_data = await self.http.invite_get(
            invite_code, {'with_counts': with_count}
        )
        ...
        *,
        ...
        if name is not None:
            if __debug__:
                if not isinstance(name, str):
                    raise AssertionError(
                        f'`name` can be given as `None` or as `str` instance, got '
                        f'{name.__class__.__name__}.'
                    )
        ...
        if color is not None:
            if __debug__:
                if not isinstance(color, int):
                    raise AssertionError(
                        f'`color` can be given as `None`, `{Color.__name__}` or as other `int` '
        ...
        if separated is not None:
            if __debug__:
                if not isinstance(separated, bool):
                    raise AssertionError(
                        f'`separated` can be given as `None` or as `bool` instance, got '
        ...
        if mentionable is not None:
            if __debug__:
                if not isinstance(mentionable, bool):
                    raise AssertionError(
                        f'`mentionable` can be given as `None` or as `bool` instance, got '
        ...
        if permissions is not None:
            if __debug__:
                if not isinstance(permissions, int):
                    raise AssertionError(
                        f'`permissions` can be given as `None`, `{Permission.__name__}` or as other '
    ...
    async def role_move(self, role, position, *, reason=None):
        ...
        if isinstance(role, Role):
            ...
            if snowflake_pair is None:
                raise TypeError(
                    f'`role` can be given as `{Role.__name__}`, or as `tuple` (`int`, `int`), got '
        ...
        data = change_on_switch(
            guild.role_list,
            role,
            position,
            key=lambda role_, pos: {'id': role_.id, 'position': pos},
        )
        if not data:
            return
    ...
    async def _role_reorder_roles_element_validator(self, item):
        ...
        if not isinstance(item, tuple):
            raise TypeError(
                f'`roles` item can be given as `tuple`, got {item.__class__.__name__}.'
            )
        ...
        if isinstance(role, Role):
            ...
            if snowflake_pair is None:
                raise TypeError(
                    f'`roles` item[0] can be given as `{Role.__name__}`, or as `tuple` (`int`, `int`), got '
                    f'{role.__class__.__name__}.'
                )
        ...
        if not isinstance(position, int):
            raise TypeError(
                f'`roles` item[1] should be `int` instance, but got {position.__class__.__name__}.'
            )
        ...
        async for element in self._role_reorder_roles_validator(roles):
            ...
            if guild is None:
                ...
                if guild is not maybe_guild:
                    raise ValueError(
                        f'roles are from different guilds, got guild={guild!r}; other_guild={guild!r}.'
                    )
    ...
    async def application_command_global_get(self, application_command):
        ...
        if isinstance(application_command, ApplicationCommand):
            ...
            if application_command_id is None:
                raise TypeError(
                    f'`application_command` can be given as `{ApplicationCommand.__name__}`, or as `int` '
        ...
        application_command_data = await self.http.application_command_global_get(
            application_id, application_command_id
        )
    ...
    async def application_command_global_get_all(self):
        ...
        return [
            ...
            for application_command_data in data
        ]
    ...
    async def application_command_global_create(self, application_command):
        ...
        if __debug__:
            if not isinstance(application_command, ApplicationCommand):
                raise AssertionError(
                    f'`application_command` can be given as `{ApplicationCommand.__name__}`, got '
    ...
    async def application_command_global_edit(
        self, old_application_command, new_application_command
    ):
        """
        ...
        if isinstance(old_application_command, ApplicationCommand):
            ...
            if application_command_id is None:
                raise TypeError(
                    f'`old_application_command` can be given as `{ApplicationCommand.__name__}` or `int` '
        ...
        if __debug__:
            if not isinstance(new_application_command, ApplicationCommand):
                raise AssertionError(
                    f'`new_application_command` can be given as `{ApplicationCommand.__name__}`, got '
        ...
        if (old_application_command is not None) and (
            old_application_command.name == data['name']
        ):
            del data['name']

        await self.http.application_command_global_edit(
            application_id, application_command_id, data
        )
        return ApplicationCommand._from_edit_data(
    ...
    async def application_command_global_delete(self, application_command):
        ...
        if isinstance(application_command, ApplicationCommand):
            ...
            if application_command_id is None:
                raise TypeError(
                    f'`application_command` can be given as `{ApplicationCommand.__name__}` or `int` '
                    f'instance, got {application_command.__class__.__name__}.'
                )

        await self.http.application_command_global_delete(
            application_id, application_command_id
        )
    ...
    async def application_command_global_update_multiple(self, application_commands):
        ...
        if __debug__:
            if getattr(type(application_commands), '__iter__', None) is None:
                raise AssertionError(
                    f'`application_command_datas` can be given as an `iterable`, got '
        ...
        for application_command in application_commands:
            if __debug__:
                if not isinstance(application_command, ApplicationCommand):
                    raise AssertionError(
                        f'An application commands can be given as an `iterable` of '
        ...
        if application_command_datas:
            application_command_datas = (
                await self.http.application_command_global_update_multiple(
    ...
    async def application_command_guild_create(self, guild, application_command):
        ...
        data = application_command.to_data()
        data = await self.http.application_command_guild_create(
            application_id, guild_id, data
        )
        return ApplicationCommand.from_data(data)

    async def application_command_guild_edit(
        self, guild, old_application_command, new_application_command
    ):
        """
        ...
        await self.http.application_command_guild_edit(
            application_id, guild_id, application_command_id, data
        )
    ...
    async def application_command_guild_delete(self, guild, application_command):
        ...
        await self.http.application_command_guild_delete(
            application_id, guild_id, application_command_id
        )

    async def application_command_guild_update_multiple(
        self, guild, application_commands
    ):
        """
        ...
        if application_command_datas:
            application_command_datas = (
                await self.http.application_command_guild_update_multiple(
    ...
    async def application_command_permission_edit(
        self, guild, application_command, overwrites
    ):
        """
        ...
        if overwrites is not None:
            if __debug__:
                if not isinstance(overwrites, (list, set, tuple)):
                    raise AssertionError(
                        f'`overwrites` can be given as `None`, `list`, `tuple` or `set`, got '
            ...
            if __debug__:
                if len(overwrite_datas) > APPLICATION_COMMAND_PERMISSION_OVERWRITE_MAX:
                    raise AssertionError(
                        f'`overwrites` can contain up to '
    ...
    async def application_command_permission_get_all_guild(self, guild):
        ...
        permission_datas = await self.http.application_command_permission_get_all_guild(
            application_id, guild_id
        )

        return [
            ApplicationCommandPermission.from_data(permission_data)
            for permission_data in permission_datas
        ]

    async def interaction_application_command_acknowledge(
        self, interaction, *, show_for_invoking_user_only=False
    ):
        """
        ...
        if __debug__:
            if not isinstance(interaction, InteractionEvent):
                raise AssertionError(
                    f'`interaction` can be given as `{InteractionEvent.__name__}` instance, got '
        ...
        with InteractionResponseContext(interaction, True, show_for_invoking_user_only):
            await self.http.interaction_response_message_create(
                interaction.id, interaction.token, data
            )
    ...
    async def interaction_response_message_create(
        ...
        *,
        ...
        if interaction.is_unanswered():
            ...
            return await self.interaction_response_message_edit(
                interaction, content, embed=embed, allowed_mentions=allowed_mentions
            )
            ...
            return await self.interaction_followup_message_create(
                interaction,
                content,
                embed=embed,
        ...
        if __debug__:
            ...
            if not isinstance(show_for_invoking_user_only, bool):
                raise AssertionError(
                    f'`show_for_invoking_user_only` can be given as `bool` instance, got '
        ...
        with InteractionResponseContext(
            interaction, is_deferring, show_for_invoking_user_only
        ):
            await self.http.interaction_response_message_create(
                interaction.id, interaction.token, data
            )
    ...
    async def interaction_response_message_edit(
        ...
        *,
        ...
        if interaction.is_unanswered():
            ...
            return await self.interaction_response_message_create(
                interaction, content, embed=embed, allowed_mentions=allowed_mentions
            )
    ...
    async def interaction_response_message_delete(self, interaction):
        ...
        await self.http.interaction_response_message_delete(
            application_id, interaction.id, interaction.token
        )
    ...
    async def interaction_followup_message_create(
        ...
        *,
        ...
        with InteractionResponseContext(
            interaction, False, show_for_invoking_user_only
        ):
    ...
    async def sticker_pack_get_all(self, force_update=False):
        ...
        if force_update or (not STICKER_PACK_CACHE.synced):
            ...
            if force_update:
                sticker_packs = [
                    StickerPack._create_and_update(sticker_pack_data)
                    for sticker_pack_data in sticker_pack_datas
                ]
            else:
                sticker_packs = [
                    StickerPack(sticker_pack_data)
                    for sticker_pack_data in sticker_pack_datas
                ]
    ...
    async def sticker_guild_create(
        self, guild, name, image, emoji_representation, description=None, *, reason=None
    ):
        """
        ...
        if isinstance(emoji_representation, str):
            ...
            if __debug__:
                if emoji_representation.is_custom_emoji():
                    raise AssertionError(
                        f'Only unicode (builtin) emojis can be used as tags, got '
            ...
            raise TypeError(
                f'`emoji_representation` can be either `str` or `{Emoji.__name__}` instance, got '
        ...
        if __debug__:
            if not isinstance(image, (bytes, bytearray, memoryview)):
                raise TypeError(
                    f'`image` can be passed as `bytes-like` or None, got {image.__class__.__name__}.'
                )
        ...
        form_data.add_field(
            'file', image, filename=f'file.{extension}', content_type=media_type
        )
    ...
    async def relationship_delete(self, relationship):
        ...
        if isinstance(relationship, Relationship):
            ...
            if user_id is None:
                raise TypeError(
                    f'`relationship` can be given as `{Relationship.__name__}`, `{User.__name__}`, '
    ...
    async def relationship_create(self, user, relationship_type=None):
        ...
        if relationship_type is None:
            ...
            raise TypeError(
                f'`relationship_type` can be given as `None`, `{RelationshipType.__name__}` or as `int`'
    ...
    async def relationship_friend_request(self, user):
        ...
        data = {'username': user.name, 'discriminator': str(user.discriminator)}
    ...
    async def client_gateway(self):
        ...
        try:
            ...
            try:
                ...
                gateway_max_concurrency = session_start_limit_data.get(
                    'max_concurrency', 1
                )
    ...
    async def client_gateway_url(self):
        ...
        self._gateway_url = (
            gateway_url
        ) = f'{data["url"]}?encoding=json&v={API_VERSION}&compress=zlib-stream'
    ...
    async def hypesquad_house_change(self, house):
        ...
        if isinstance(house, HypesquadHouse):
            ...
            raise TypeError(
                f'`house` can be given as `int` or `{HypesquadHouse.__name__}` instance, got '
    ...
    async def _connect(self):
        ...
        try:
            ...
            if isinstance(err, InvalidToken) or (
                isinstance(err, DiscordGatewayException)
                and (err.code in INTENT_ERROR_CODES)
            ):
    ...
    async def join_voice(self, channel):
        ...
        if isinstance(channel, ChannelVoiceBase):
            ...
            if channel_id is None:
                raise TypeError(
                    f'`channel` can be given as `{ChannelVoiceBase.__name__}` or `int` instance, got '
                    f'{channel.__class__.__name__}.'
                )
            ...
            try:
                ...
                raise RuntimeError(
                    f'Cannot join partial channel: {channel!r}'
                ) from None

            if not isinstance(channel, ChannelVoiceBase):
                raise TypeError(
                    f'Can join only to `{ChannelVoiceBase.__name__}`, got {channel.__class__.__name__}.'
                )
    ...
    async def join_speakers(self, channel, *, request=False):
        ...
        if isinstance(channel, ChannelStage):
            ...
            if channel_id is None:
                raise TypeError(
                    f'`channel` can be given as `{ChannelStage.__name__}` or `int` instance, got '
                    f'{channel.__class__.__name__}.'
                )
            ...
            if not isinstance(channel, ChannelStage):
                raise TypeError(
                    f'Can join only to `{ChannelStage.__name__}`, got {channel.__class__.__name__}.'
                )
    ...
    async def join_audience(self, channel):
        ...
        data = {'suppress': True, 'channel_id': channel_id}
    ...
    async def _request_members2(self, guilds):
        ...
        event_handler.waiters['0000000000000000'] = waiter = MassUserChunker(
            len(guilds)
        )
        ...
        if shard_count:
            ...
            for index in range(shard_count):
                task = Task(
                    _request_members_loop(gateways[index], guilds_by_shards[index]),
                    KOKORO,
                )
                tasks.append(task)
    ...
    async def request_members(self, guild, name, limit=1):
        ...
        if isinstance(guild, Guild):
            ...
            if guild_id is None:
                raise TypeError(
                    f'`guild` can be given as ``{Guild.__name__}`` or `int` instance, got '
                    f'{guild.__class__.__name__}.'
                )
    ...
    def get_guild(self, name, default=None):
        ...
        if __debug__:
            if not isinstance(name, str):
                raise AssertionError(
                    f'`name` should have be given as `str` instance, got {name.__class__.__name__}.'
                )
        ...
        *users : `int` or ``UserBase`` instances
        ...
        while True:
            ...
            if not isinstance(user, (int, UserBase)):
                raise TypeError(
                    f'User {index} was not passed neither as `int` or as `{UserBase.__name__}` instance, '
                    f'got {user.__class__.__name__}.'
                )
