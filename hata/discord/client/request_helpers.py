        ...
        if isinstance(components, ComponentBase):
            ...
            for component in components:
                if isinstance(component, ComponentBase):
                    ...
                    raise TypeError(
                        f'`components` contains a non `{ComponentBase.__name__}` or as `list`, `tuple` of '
            ...
            raise TypeError(
                f'`components` can be given as `{ComponentBase.__name__}` or as `list`, `tuple` of '
    ...
    if isinstance(message, Message):
        ...
        if isinstance(message, MessageRepr):
            ...
            if snowflake_pair is None:
                raise TypeError(
                    f'`message` should have be given as `{Message.__name__}` or as '
...
def create_file_form(data, file):
    ...
    if len(files) == 1:
        name, io = files[0]
        form.add_field(
            'file', io, filename=name, content_type='application/octet-stream'
        )
...
def validate_content_and_embed(content, embed, is_edit):
    ...
    if embed is None:
                ...
                for embed_element in embed:
                    if not isinstance(embed_element, EmbedBase):
                        raise AssertionError(
                            f'`embed` was given as a `list` or `tuple`, but it\'s it contains a non '
        ...
        raise TypeError(
            f'`embed` was not given as `{EmbedBase.__name__}` instance, neither as a `list`  or `tuple` of '
    ...
    if content is None:
        ...
        if __debug__:
            if embed is not (... if is_edit else None):
                raise AssertionError(
                    f'Multiple embeds were given, got content={content!r}, embed={embed!r}.'
                )
...
def get_channel_id(channel, channel_type):
    ...
    if isinstance(channel, channel_type):
        ...
        if channel_id is None:
            raise TypeError(
                f'`channel` can be either given as `{channel_type.__name__}` or as `int` instance, '
...
def get_guild_and_guild_text_channel_id(channel):
    ...
    while True:
        ...
        raise TypeError(
            f'`channel` can be either given as `{ChannelText.__name__}` or as `int` instance, '
            f'got {channel.__class__.__name__}.'
        )
...
def get_guild_id_and_channel_id(channel, channel_type):
    ...
    if isinstance(channel, channel_type):
        ...
        if snowflake_pair is None:
            raise TypeError(
                f'`channel` can be given as `{channel_type.__name__}`, or as '
...
def get_stage_channel_id(stage):
    ...
    if isinstance(stage, Stage):
        ...
        if channel_id is None:
            raise TypeError(
                f'`stage` can be given as `{Stage.__name__}`, `{ChannelStage.__name__}`, or as '
...
def get_user_id(user):
    ...
    if isinstance(user, ClientUserBase):
        ...
        if user_id is None:
            raise TypeError(
                f'`user` can be either given as `{ClientUserBase.__name__}` or as `int` instance, '
...
def get_guild_discovery_and_id(guild):
    ...
    if isinstance(guild, Guild):
        ...
        if guild_id is None:
            raise TypeError(
                f'`guild` can be given as `{Guild.__name__}`, `{GuildDiscovery.__name__}` or `int` '
...
def get_achievement_id(achievement):
    ...
    if isinstance(achievement, Achievement):
        ...
        if achievement_id is None:
            raise TypeError(
                f'`achievement` can be given as `{Achievement.__name__}` or `int` instance, got '
...
def get_channel_id_and_message_id(message):
    ...
    if isinstance(message, Message):
        ...
        raise TypeError(
            '`message` was given as `None`. Make sure to use `Client.message_create` with giving '
        ...
        if snowflake_pair is None:
            raise TypeError(
                f'`message` can be given as `{Message.__name__}` or as '
...
def get_role_id(role):
    ...
    if isinstance(role, Role):
        ...
        if role_id is None:
            raise TypeError(
                f'`role` can be given as `{Role.__name__}` or `int` instance, got '
...
def get_webhook_id(webhook):
    ...
    if isinstance(webhook, Webhook):
        ...
        if webhook_id is None:
            raise TypeError(
                f'`webhook` can be given as `{Webhook.__name__}` or `int` instance, got '
...
def get_webhook_id_token(webhook):
    ...
    if isinstance(webhook, Webhook):
        ...
        if snowflake_token_pair is None:
            raise TypeError(
                f'`webhook` can be given either as `{Webhook.__name__}` or as `tuple` (`int`, `str`), '
...
def get_reaction(emoji):
    ...
    if isinstance(emoji, Emoji):
        ...
        raise TypeError(
            f'`emoji` can be given as ``{Emoji.__class__}`` or as `str` instance, got '
...
def get_guild_id_and_emoji_id(emoji):
    ...
    if isinstance(emoji, Emoji):
        ...
        if snowflake_pair is None:
            raise TypeError(
                f'`emoji` can be given as `{Emoji.__name__}`, or as `tuple` (`int`, `int`), got '
                f'{emoji.__class__.__name__}.'
            )
...
def get_sticker_and_id(sticker):
    ...
    while True:
        ...
        raise TypeError(
            f'`sticker` can be either given as `{Sticker.__name__}` or as `int` instance, '
