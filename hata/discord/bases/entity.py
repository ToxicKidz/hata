...
def _get_direct_parents_and_merge_slots(class_name, class_parents, class_attributes):
    ...
    if direct_parent is not None:
        ...
        while index < parent_count:
            ...
            final_slots.update(
                getattr(class_parent, f'_{class_parent.__name__}__slots', ())
            )
            index += 1

    slots = class_attributes.get(
        '__slots__',
    )
...
def _process_set_slot(class_attributes, final_slots):
    ...
    while slotters:
        ...
        type(slotter).__set_slot__(
            slotter, attribute_name, class_attributes, final_slots
        )
...
class Slotted(type):
    ...
    def __new__(cls, class_name, class_parents, class_attributes):
        ...
        class_attributes : `dict` of (`str`, `Any`) items
            The class attributes of the created type.
        """
        direct_parent, final_slots = _get_direct_parents_and_merge_slots(
            class_name, class_parents, class_attributes
        )
...
class DiscordEntityMeta(Slotted):
    ...
    def __new__(cls, class_name, class_parents, class_attributes, immortal=False):
        ...
        if (
            ...
            and ('id' not in class_attributes)
            and (direct_parent.id is DiscordEntity.id)
        ):
            final_slots.add('id')
