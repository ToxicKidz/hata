...
class IconSlot:
    ...
    def __new__(
        ...
        exec(
            ...
            cls._compile_globals,
            locals_,
        )

        added_class_attributes.append(
            (func_name, locals_[func_name]),
        )
        ...
        self.added_instance_attributes = (
            added_internal_attribute_name_type,
            added_instance_attribute_name_hash,
        )
    ...
    def preconvert(self, kwargs, processable):
        ...
        try:
            ...
            if type(icon_hash) is int:
                ...
                raise TypeError(
                    f'`{icon_hash_name}` can be passed as `int` instance, got '
                    f'{icon_hash.__class__.__name__}.'
                )

            if icon_hash < 0 or icon_hash > ((1 << 128) - 1):
                raise ValueError(
                    f'`{icon_hash_name}` cannot be negative or longer than 128 bits, got {icon_hash}.'
                )
            ...
            try:
                ...
                if type(icon_type) is not IconType:
                    raise TypeError(
                        f'`{icon_type_name}` can be passed as `{IconType.__name__}` instance, got '
            ...
            if icon is None:
                ...
                raise TypeError(
                    f'`{self.internal_name!r}` can be passed as `None`, `{Icon.__name__}` or as `str` '
