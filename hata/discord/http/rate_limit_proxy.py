...
class RateLimitProxy:
    ...
    def __new__(cls, client, group, limiter=None, keep_alive=False):
        ...
        if type(group) is not RateLimitGroup:
            raise TypeError(
                f'`group` should be type `{RateLimitGroup.__name__}`, got {group}.__class__.__name__.'
            )
        ...
        while True:
            ...
            if group_limiter is LIMITER_GLOBAL:
                ...
                if limiter is not None:
                    ...
                    if isinstance(
                        limiter, (ChannelGuildBase, Message, Role, Webhook, WebhookRepr)
                    ):
            ...
            raise ValueError(
                f'Cannot cast rate limit group\'s: `{group!r}` rate limit_id of: `{limiter!r}`.'
            )
    ...
    async def wait_till_limits_expire(self):
        ...
        if handler is self._key:
            raise RuntimeError(
                'Cannot use `.wait_till_limits_expire` meanwhile `keep_alive` is `True`.'
            )

        future = Future(current_thread())
        self._handler = WeakReferer(
            handler, self._wait_till_limits_expire_callback(future)
        )
        await future
