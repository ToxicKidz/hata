...
API_ENDPOINT = (
    f'https://discord.com/api/v{API_VERSION}'
    if (CUSTOM_API_ENDPOINT is None)
    else CUSTOM_API_ENDPOINT
)
CDN_ENDPOINT = (
    'https://cdn.discordapp.com'
    if (CUSTOM_CDN_ENDPOINT is None)
    else CUSTOM_CDN_ENDPOINT
)
DIS_ENDPOINT = (
    'https://discord.com' if (CUSTOM_DIS_ENDPOINT is None) else CUSTOM_DIS_ENDPOINT
)
...
VALID_ICON_FORMATS_EXTENDED = frozenset(
    (
        *VALID_ICON_FORMATS,
        'gif',
    )
...
def user_avatar_url_as(user, ext=None, size=None):
    ...
    return (
        f'{CDN_ENDPOINT}/avatars/{user.id}/{prefix}{user.avatar_hash:0>32x}.{ext}{end}'
    )
...
def user_banner_url_as(user, ext=None, size=None):
    ...
    return (
        f'{CDN_ENDPOINT}/banners/{user.id}/{prefix}{user.banner_hash:0>32x}.{ext}{end}'
    )
...
def user_avatar_url_for_as(user, guild, ext=None, size=None):
    ...
    return (
        f'{CDN_ENDPOINT}/guilds/{guild.id}/users/{user.id}/avatars/{prefix}{guild_profile.avatar_hash:0>32x}.'
        f'{ext}{end}'
    )
...
def team_icon_url_as(team, ext=None, size=None):
    ...
    return (
        f'{CDN_ENDPOINT}/team-icons/{team.id}/{prefix}{team.icon_hash:0>32x}.{ext}{end}'
    )
    ...
    if size is None:
        ...
        if format is StickerFormat.lottie:
            ...
            if size in VALID_ICON_SIZES:
                ...
                raise ValueError(
                    f'Size must be in {sorted(VALID_ICON_SIZES)!r}, got {size}.'
                )
