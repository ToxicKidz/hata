    ...
    def exit(self, headers):
        ...
        if optimistic:
            ...
            delay1 = (
                ...
                - parse_date_header_to_datetime(headers[DATE])
            ).total_seconds()
...
class RateLimitHandlerCTX:
    ...
    def exit(self, headers):
        ...
        assert (
            not self.exited
        ), '`RateLimitHandlerCTX.exit` or `StaticRateLimitHandler.exit` was already called.'
        self.exited = True
...
class StackedStaticRateLimitHandler:
    ...
    def __new__(cls, parents, limiter_id):
        ...
        self = object.__new__(cls)
        self.stack = tuple(
            StaticRateLimitHandler(parent, limiter_id) for parent in parents
        )
        return self
