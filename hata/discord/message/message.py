__all__ = (
    'EMBED_UPDATE_EMBED_ADD',
    'EMBED_UPDATE_EMBED_REMOVE',
    'EMBED_UPDATE_NONE',
    'EMBED_UPDATE_SIZE_UPDATE',
    'Message',
)
...
class Message(DiscordEntity, immortal=True):
    ...
    def _finish_init(self, data, channel):
        ...
        if webhook_id is None:
            ...
            if data.get('message_reference', None) is not None:
                ...
                if (cross_mention_datas is None) or (not cross_mention_datas):
                    ...
                    cross_mentions = tuple(
                        sorted(
                            (
                                UnknownCrossMention(cross_mention_data)
                                for cross_mention_data in cross_mention_datas
                            ),
                            key=id_sort_key,
                        )
                    )
            ...
            if author_data is None:
                author = create_partial_webhook_from_id(
                    webhook_id, '', type_=webhook_type
                )
            else:
                author = WebhookRepr(
                    author_data, webhook_id, type_=webhook_type, channel=channel
                )
        ...
        if referenced_message_data is None:
            ...
            referenced_message = channel._create_unknown_message(
                referenced_message_data
            )
        ...
        if (attachment_datas is not None) and attachment_datas:
            attachments = tuple(
                Attachment(attachment) for attachment in attachment_datas
            )
        else:
            attachments = None
        ...
        if (component_datas is None) or (not component_datas):
            ...
            components = tuple(
                create_component(component_data) for component_data in component_datas
            )
        ...
        if sticker_datas is None:
            ...
            stickers = tuple(
                Sticker._create_partial(sticker_data) for sticker_data in sticker_datas
            )
        ...
        try:
            ...
            thread = CHANNEL_TYPES.get(thread_data['type'], ChannelGuildUndefined)(
                thread_data, None, guild
            )
        self.thread = thread
    ...
    def custom(cls, base, validate=True, **kwargs):
        ...
        **kwargs : keyword parameters
        ...
        if (base is not None) and (type(base) is not cls):
            raise TypeError(
                f'`base` should be either `None`, or type `{cls.__name__}`, got `{base!r}`'
            )
        ...
        try:
            ...
            if base is None:
                raise TypeError(
                    'Expected to be called as method, but was called as a classmethod and `channel` was '
                    f'not passed.'
                )
            ...
            if not isinstance(channel, ChannelTextBase):
                raise TypeError(
                    f'`channel` should be `{ChannelTextBase.__name__}` subclass\'s instance, got '
                    f'`{channel!r}`'
                )
        ...
        try:
            ...
            if (activity is not None) and (type(activity) is not MessageActivity):
                raise TypeError(
                    f'`activity` should be `None` or type `{MessageActivity.__name__}`, got `{activity!r}`'
                )
        ...
        try:
            ...
            if (application is not None) and (
                type(application) is not MessageApplication
            ):
                raise TypeError(
                    f'`application` should be `None` or type `{MessageApplication.__name__}`, got '
                    f'`{application!r}`'
                )
        ...
        try:
            ...
            if attachments is not None:
                if not isinstance(attachments, (list, tuple)):
                    raise TypeError(
                        f'`attachments` should be `None` or `tuple`, `list` of `{Attachment.__name__}` '
                ...
                if validate:
                    if attachments_length > 10:
                        raise ValueError(
                            f'`attachments` should have maximal length of `10`, got `{attachments_length!r}`'
                        )
                ...
                if attachments_length:
                    for attachment in attachments:
                        if not isinstance(attachment, Attachment):
                            raise TypeError(
                                f'`attachments` `list` contains a non `{Attachment.__name__}` '
        ...
        try:
            ...
            if (referenced_message is not None) and (
                type(referenced_message) not in (Message, MessageReference)
            ):
                raise TypeError(
                    f'`referenced_message` should be `None` or type `{Message.__name__}`, '
        ...
        try:
            ...
            if cross_mentions is not None:
                if not isinstance(cross_mentions, (tuple, list)):
                    raise TypeError(
                        f'`cross_mentions` should be `None` or `tuple`, `list` of '
                ...
                for channel_ in cross_mentions:
                    if not isinstance(
                        channel_, (ChannelGuildBase, UnknownCrossMention)
                    ):
                        raise TypeError(
                            f'`cross_mentions` contains a non `{ChannelGuildBase.__name__}` or '
        ...
        try:
            ...
            if (edited_at is not None) and (type(edited_at) is not datetime):
                raise TypeError(
                    f'`edited_at` can be `None` or `datetime`, got `{edited_at.__class__.__name__}`.'
                )
        ...
        try:
            ...
            if embeds is not None:
                if not isinstance(embeds, (list, tuple)):
                    raise TypeError(
                        f'`embeds` can be `None` or `list` of type `{EmbedCore.__name__}`, got '
                ...
                if validate:
                    if len(embeds) > 10:
                        raise ValueError(
                            f'`embeds` can have maximal length of `10`, got `{embeds_length!r}`.'
                        )
                ...
                if embeds_length:
                    for index in range(embeds_length):
                        ...
                        raise TypeError(
                            f'`embeds` `list` contains a non `{EmbedBase.__name__}` instance: '
                            f'`{embeds.__class__.__name__}`.'
                        )
        ...
        try:
            ...
            if nonce is not None:
                if type(nonce) is str:
                    ...
                    raise TypeError(
                        f'`nonce` should be `None` or type `str` instance, got `{nonce!r}`.'
                    )

                nonce_length = len(nonce)
                if nonce_length > 32:
                    raise TypeError(
                        f'`nonce`\'s length can be be in range [1:32], got: `{nonce_length!r}`; '
                        f'`{nonce!r}`.'
                    )
                elif nonce_length == 0:
                    nonce = None
        ...
        try:
            ...
            if reactions is None:
                ...
                raise TypeError(
                    f'`reactions`, should be type `{reaction_mapping.__name__}`, got `{reactions}`'
                )
        ...
        try:
            ...
            if role_mentions is not None:
                if not isinstance(role_mentions, (tuple, list)):
                    raise TypeError(
                        f'`role_mentions` should be `None` or `list`, `tuple` of `{Role.__name__}` '
                ...
                if role_mentions:
                    for role in role_mentions:
                        if not isinstance(role, Role):
                            raise TypeError(
                                f'`role_mentions` contains a non `{Role.__name__}` instance, '
        ...
        if validate:
            if (role_mentions is not None) and (
                not isinstance(channel, ChannelGuildBase)
            ):
                raise ValueError(
                    '`role_mentions` are set as not `None`, meanwhile the `channel` is not '
        ...
        try:
            ...
            if not isinstance(stickers, (list, tuple)):
                raise TypeError(
                    f'`stickers` should be `None` or ` tuple`, `list` of `{Sticker.__name__}` instances, '
                    f'got `{stickers!r}`'
                )
            ...
            if stickers_length:
                for sticker in stickers:
                    if not isinstance(sticker, Sticker):
                        raise TypeError(
                            f'`stickers` contains a non `{Sticker.__name__}` instance, '
                            f'`{sticker!r}`'
                        )
            else:
                # We should not have empty attachment list, lets fix it
        ...
        try:
            ...
            if user_mentions is not None:
                if not isinstance(user_mentions, (tuple, list)):
                    raise TypeError(
                        f'`user_mentions` should be `None` or `tuple`, `list` of `{UserBase.__name__}` '
                ...
                if user_mentions:
                    for user in user_mentions:
                        if not isinstance(user, UserBase):
                            raise TypeError(
                                f'`user_mentions` contains at least 1 non `{UserBase.__name__}` '
        ...
        try:
            ...
            if (interaction is not None) and (
                not isinstance(interaction, MessageInteraction)
            ):
                raise TypeError(
                    f'`interaction` can be given as `None` or as `{MessageInteraction.__name__}` '
        ...
        try:
            ...
            if components is not None:
                if not isinstance(components, (list, tuple)):
                    raise TypeError(
                        f'`components` should be `None` or `tuple`, `list` of `{ComponentBase.__name__}` '
                ...
                if components:
                    for component in components:
                        if not isinstance(component, ComponentBase):
                            raise TypeError(
                                f'`components` contains at least 1 non `{ComponentBase.__name__}` '
        ...
        try:
            ...
            if (thread is not None) and (not isinstance(thread, ChannelThread)):
                raise TypeError(
                    f'`thread` can be given as `None` or as `{ChannelThread.__name__}` '
        ...
        if flag_difference:
            ...
            if MessageFlag(flag_difference).embeds_suppressed:
                try:
                    ...
                    if embed_datas:
                        embeds = tuple(
                            EmbedCore.from_data(embed) for embed in embed_datas
                        )
                    else:
                        embeds = None
    ...
    def _update_no_return(self, data):
        ...
        if flag_difference:
            ...
            if MessageFlag(flag_difference).embeds_suppressed:
                try:
                    ...
                    if embed_datas:
                        embeds = tuple(
                            EmbedCore.from_data(embed_data)
        ...
        try:
            ...
            if embed_datas:
                embeds = tuple(
                    EmbedCore.from_data(embed_data) for embed_data in embed_datas
                )
            else:
                embeds = None
            self.embeds = embeds
    ...
    def _update_embed(self, data):
        ...
        if embeds is None:
            ...
            embeds = (
                *embeds,
                *(
                    EmbedCore.from_data(embed_datas[index])
                    for index in range(embeds_length_actual, embeds_length_new)
                ),
            )
